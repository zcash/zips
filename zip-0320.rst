...
It is only required to support a transparent-source-only form of P2PKH addresses;
P2SH address support is not necessary.
...
If a user desires to spend shielded funds to a TEX Address, a Sender supporting
this ZIP MUST create two transactions: one that unshields the funds to an
ephemeral transparent address, and one that spends from that ephemeral address
to the destination TEX Address. This does not defeat the intent of the ZIP,
because it is still possible for a Recipient to return the funds to the Sender
by sending them back to the ephemeral address.
...
the relevant transparent viewing keys.
...
TEX addresses are also supported by the ``zcash_address_wasm`` library [#zcash_address_wasm]_.
...
As required by Binance, it will possible to convert between a TEX address and an
...
Some of the design elements of that approach that apply to metadata in general have
