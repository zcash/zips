...
The off-chain payments is also split in three phases: setup, update state, and unmask.
 1. in the setup phase,
  a. the customer creates a new private channel state , corresponding to the customer and merchant new balances, a state nonce and revocation lock (``<channel-state>_{i+1} = (<channel-id>, <channel-nonce>__{i+1}, <revocation-lock>_{i+1}, <balance-merch>_{i+1}, <balance-cust>_{i+1})`` with ``<balance-merch>_{i+1}=<balance-merch>_i + <epsilon>``, ``<balance-cust>_{i+1}=<balance-cust>_i - <epsilon>`` and ``<revocation-lock> = HASH(<revocation-secret>));
