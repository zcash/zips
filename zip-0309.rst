...
- if context.tx.tze_out_count == 0 AND context.tx.tx_out_count = 2: // mutual close
  ...
  - if amount(context.tx_out[0]) != cust-balance OR owner(context.tx_out[0]) != cust-pk:

    - Return False

  - if amount(context.tx_out[1]) != merch-balance OR owner(context.tx_[1]) != merch-pk:
...
- if context.tx.tze_out_count == 1 AND context.tze_out[0].precondition.mode == MERCH-CLOSE-MODE:

  - // *Define variables*
  - cid-wtn || cust-pk-wtn || merch-pk-wtn || MERCH-PK-wtn || cust-sig = witness
  - cid || cust-pk || merch-pk || MERCH-PK = context.tze_out[0].precondition.tze_data_payload
  - msg = cid || cust-pk || merch-pk || MERCH-PK
  ...
  - Return True
- if tze_out_count == 1 AND context.tze_out[0].precondition.mode = CUST-CLOSE-MODE:
