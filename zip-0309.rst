...
- if mutual-close TODO
  ...
  - if sender(context.mutual-close) != cust-pk AND sender(context.mutual-close-tx) != merch-pk:
...
- if context.expiry-out.precondition.mode == MERCH-CLOSE:
  ...
  - // *Verify transaction correctness*
  - if sender(context.expiry-tx) != merch-pk:
...
- witness =
  - tze_id = <zk-Channels-tze-id>
  - tze_mode = <expiry-mode> // we need to keep the same mode
  - tze_data_payload_len =
...
- precondition =
 ...
 - tze_data_payload_len =  
 - tze_data_payload = <cust-pk> || <merch-pk> || <cid> || <rl>_i || <cust-balance>_i // potentially adding <n_i> for reconciliation, as well as <delay> if channel specific
...
- if <cust-close-out>.precondition.mode = <cust-close-mode>
  ...
  - if sender != <cust-pk>:
    - Return False
  - <msg> = <cust-pk> || <merch-pk> || <cid> || <rl>_i || <merch-balance>_i || <cust-balance>_i
  - if Verify(<merch-pk>, <msg>, <signature_M>) = 0
    - Return False
  // Check that amounts and receiver are correct
  - if <merch-output>.amount != <merch-balance>_i OR <merch-output>.owner != <merch-pk>:
    - Return False
  - if <cust-close-out>.amount != <cust-balance>_i OR <cust-close-out>.owner != <cust-pk> OR <rl>_i NOT IN <cust-close-out>.precondition:
    - Return False
  ...
  - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<cid>] != <cust-close-out>.precondition.tze_data_payload[<cid>]:
    - Return False
 - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<cust-pk>] != <cust-close-out>.precondition.tze_data_payload[<cust-pk>]:
    - Return False
  - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<merch-pk>] != <cust-close-out>.precondition.tze_data_payload[<merch-pk>]:
    - Return False
...
- if <expiry-out>.precondition.mode = <expiry-mode>
   ...
   - if Verify(<merch-pk>, <msg>, <expiry-signature_C>) = 0
    - Return False
   // Check that information is transferred correctly from tze_in to tze_out
   - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<cid>] != <expiry-out>.precondition.tze_data_payload[<cid>]:
    - Return False
   - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<cust-pk>] != <expiry-out>.precondition.tze_data_payload[<cust-pk>]:
    - Return False
   - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<merch-pk>] != <expiry-out>.precondition.tze_data_payload[<merch-pk>]:
    - Return False
   - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<rl>] != <expiry-out>.precondition.tze_data_payload[<rl>]:
    - Return False
   - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<cust-balance>] != <expiry-out>.precondition.tze_data_payload[<cust-balance>]:
    - Return False
   - if <escrow-tx-id>.tze_in[prevout_in].precondition.tze_data_payload[<merch-balance>] != <expiry-out>.precondition.tze_data_payload[<merch-balance>]:
    - Return False
  - Return True
- if <cust-close-out>.precondition.mode = <cust-close-mode>
   // Standard check (add delay if channel specific)
