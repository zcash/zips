...
We define the issuance authorization signature scheme :math:`\mathsf{IssueAuthSig}` identically to the Orchard spend authorization signature scheme, :math:`\mathsf{SpendAuthSig^{\mathsf{Orchard}}`, that is, as :math:`RedPallas` with key re-randomization and with generator :math:`\mathcal{P}_{\mathbb{G}} = \mathcal{G}^{\mathsf{Orchard}}` [#protocol-concretespendauthsig]_.
