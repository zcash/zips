...
In order to verify the balance of the different Assets, the verifier MUST perform a similar process as for the Orchard protocol [#protocol-binding]_, with an additional term for the `burn mechanism`_.
...
.. math:: \mathsf{bvk} =(\sum_{i=1}^{n}  \mathsf{cv^{net}}_{i})  - \mathsf{ValueCommit_0^{OrchardZSA}(v^{balanceOrchard}, \mathcal{V}^{\mathsf{Orchard}})} - \mathsf{bvk}^{\mathsf{assetBurn}}

where :math:`\mathsf{bvk}^{\mathsf{assetBurn}}` is defined in the `burn mechanism`_ section.
...
The sender includes a :math:`\mathsf{v^{AssetBase}}` variable for every Asset Identifier that is being burnt. 
This represents the amount of that Asset that is being burnt.
As we will show in the `ZSA Transaction Structure`_, this is separate from the regular :math:`\mathsf{valueBalance^Orchard}` that is the default transparent value for the ZEC Asset, and represents either the transaction fee, or the amount of ZEC changing pools (eg: to Sapling or Transparent).
...
.. math:: \mathsf{bvk}^{\mathsf{assetBurn}} = \sum_{k=1}^{L} \mathsf{ValueCommit_0^{OrchardZSA}}(\mathsf{v}_{k}, \mathsf{AssetBase}_{k})

The validator uses this definition and verifies the Balance and Binding Signature as described in the `Value Balance Verification`_ section.
...
Note that in the case that the balance of all the Action values related to a specific Asset will be zero, there will be no value added to the vector. This way, neither the number of Assets nor their Asset Identifiers will be revealed, except in the case that an Asset is burnt.
