...
of the cryptosystem used for note encryption, since an adversary would have
to know $\TransmitPublic$ in order to exploit a hypothetical weakness in that
cryptosystem.
...
\begin{itemize}
    \item $\AuthPublic$ is the \payingKey of the recipient.
    \item $\Value$ is an unsigned integer representing the value of the
        \note in \zatoshi (1 \ZEC = 10\textsuperscript{8} \zatoshi).
    \item $\NoteAddressRand$ is a $\PRFnf{\AuthPrivate}$ preimage.
...
$\MerkleCRH$ \changed{and $\GeneralCRH$} are collision-resistant hash functions.
...
\begin{equation*}
\begin{aligned}
&\setchanged \PRFaddr{x}(t) \\
\nf =\;& \PRFnf{\AuthPrivate}(\NoteAddressRand) \\
\h{i} =\;& \PRFpk{\AuthPrivate}(i, \hSig) \\
...
\subsubsection{Merkle root validity}
...
\subsubsection{\JoinSplitCircuit{} and Proofs}
...
$\MerkleCRH$ is a collision-resistant hash function. In \Zcash, the $\SHAName$ function
...
\subsection{\NoteCommitments} \label{comm}
