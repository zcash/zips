...
\newcommand{\NoteCommitmentLeadNibble}{\mathbf{0}}
...
$\Leading{k}(x)$, where $k$ is an integer, converts the input $x$ to a bit
sequence using little-endian order, and returns the leading (initial) $k$ bits
...
represents the application of $\BlakeHashName$ to a 16-byte personalization string
$p$ and input $x$, as defined in \cite{blake2}.
}
...
and $\NoteCommitRand$ using the collision-resistant hash function \changed{$\FullHash$}.
...
\begin{bytefield}[bitwidth=0.036em]{840}
    ...
    \bitbox{24}{1} &
    \bitbox{24}{1} &
    \bitbox{112}{4 bit $\NoteCommitmentLeadNibble$} &
...
\begin{equation*}
\begin{bytefield}[bitwidth=0.028em]{1600}
    \bitbox{180}{$\Value$ (8 bytes)} &
    \bitbox{256}{$\NoteAddressRand$ (32 bytes)} &
    \bitbox{256}{$\NoteCommitRand$ (\changed{32} bytes)} &
    \changed{\bitbox{1024}{$\Memo$ (128 bytes)}}
\end{bytefield}
\end{equation*}
...
\raisebox{-1.5ex}{\pbox{10.5em}{\type{JoinSplitDescription} \type{[$\nJoinSplit$]}}} &
The \sequenceOfJoinSplitDescriptions in this \transaction. \\ \hline

33 & $\joinSplitPubKey$ & \type{char[33]} & An encoding of a ECDSA public verification key,
using the secp256k1 curve and parameters defined in \cite{sec2-ecdsa} and
\cite{secp256k1}. \\ \hline

64 & $\joinSplitSig$ & \type{char[64]} & A signature on a prefix of the \transaction encoding,
...
432 & $\encCiphertexts$ & \type{char[216][$\NNew$]} & A sequence of ciphertext
...
\transaction up to and not including the $\joinSplitPubKey$ and $\joinSplitSig$ fields.
...
\Leading{256}(\BlakeHash(\kdftag, \kdfinput))$
