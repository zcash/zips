...
the encryption scheme based on $\SymSpecific$ \cite{rfc7539} which internally
uses length fields encoded as little-endian).
...
\subparagraph{\changed{$\DiscloseCiphertext{}$ integrity}}
...
\setchanged
\begin{bytefield}[bitwidth=0.045em]{320}
...
$\DisclosePlaintext{i} = \Justthebox{\disclosebox}{-1.3ex}$ for $i$ in $\{2..\NNew\}$.
}
...
\SymEncrypt{\TransmitKey{i}}(\TransmitPlaintext{i})$.
    \end{itemize}
  \item Let $\DisclosePlaintext{1} := \Justthebox{\sharedbox}{-5.5ex}$ \linebreak
        and let $\DisclosePlaintext{i} := \Justthebox{\disclosebox}{-1.3ex}$
        for $i$ in $\{2..\NNew\}$.
  \item For $i$ in $\{1..\NOld\}$,
    \begin{itemize}
      \item Let $\DerivedKey{i} := \PRFdk{\DiscloseKeyOld{i}}(i, \hSig)$.
      \item Let $\DiscloseCiphertext{i} := \SymEncrypt{\DerivedKey{i}}(\DisclosePlaintext{i})$.
    \end{itemize}
}
\end{itemize}
...
\begin{enumerate}
  ...
  \item If $k = 1$ then let $\SharedPlaintext := \DisclosePlaintext{1}$.
  \item If $2 \leq k \leq \NOld$ then let
$\SharedPlaintext := \SymDecrypt{\DisclosePlaintext{k}}(\DisclosePlaintext{1})$.
  \item If $k = \bot$ or else $\SharedPlaintext = \bot$ then
set $\CoinPlaintext{i} = \bot$ for $i$ in $\{1..\NNew\}$ and return
