...
to give $a^* = \vproduct{i=0}{n-1} a_i$; then, constrain $a^*$ to be nonzero.
...
\begin{pnotes}
  \item The bit length $n$ is not limited by the field element size
        (for example, the case $n = 512$ is used in the definition
        of $\PRFnr{}$ in \crossref{concreteprfs}).
...
\begin{formulae}
  ...
  \item let $\Acc^{\vv}\_0hairspace = k_0 \bchoose B^{\vv} : 1$
        \vspace{1ex}
