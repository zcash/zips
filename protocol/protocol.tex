...
$\PRFexpand{}    $&$\typecolon\; \SpendingKeyType   $&$\times\; \byte  $& &$\rightarrow \PRFOutputExpand $\\
$\PRFock{}       $&$\typecolon\; \OutViewingKeyType $&$\times\; \ReprJ \times \ReprJ \times \ReprJ $& &$\rightarrow \Keyspace$\\
$\PRFnfSapling{} $&$\typecolon\; \ReprJ             $&$\times\; \ReprJ $& &$\rightarrow \PRFOutputNfSapling $
\end{tabular}
...
\vspace{-4ex}
\pnote{$\PRFnf{}$ was called $\PRFsn{}$ in \Zerocash \cite{BCGGMTV2014}.}
...
\pnote{
We need separate signing key generation and verifying key derivation algorithms,
...
\vspace{-4ex}
\pnote{The order of randomizer and key arguments to $\SigRandomizePrivate$ and $\SigRandomizePublic$
are reversed relative to \cite[section 3]{FKMSSS2016}.}
...
this is very similar to the definition of a \quotedterm{Signature with Secret Key to Public Key Homomorphism}
in \cite[Definition 13]{DS2016}.
...
\begin{pnotes}
  \item Assume that $\PRFexpand{}$ is a PRF with output range $\PRFOutputExpand$, and define
        $f \typecolon \SpendingKeyType \times \byte \rightarrow \GF{\ParamJ{r}}$ by
        \begin{formulae}
        ...
        distributions $\PRFexpand{\SpendingKey}(0) : \SpendingKey \leftarrowR \SpendingKeyType$
        and $\SpendAuthSigGenPrivate()$ (defined in \crossref{concretespendauthsig}) are computationally
        indistinguishable.
  \item $\AuthProvePublicRepr$ will be used as a key for $\PRFnfSapling{}$ (see
        \crossref{commitmentsandnullifiers} and \crossref{spendstatement}) to derive
        \nullifiers for \Sapling{} \notes.
        The output of $\reprJ$ is approximately uniformly distributed on
        representations of points in the subgroup of order $\ParamJ{r}$, but not on
        $\ReprJ$. However this can cause a security loss of at most a factor
        of $2^{\ellJ} / \ParamJ{r} \simeq 17.666$ (and is not expected to cause any
        problem for the instantiation of $\PRFnfSapling{}$ in \crossref{concreteprfs}).
...
\begin{consensusrules}
  ...
  \item The \spendAuthSignature{} \MUST be a valid $\SpendAuthSig$ signature over $\dataToBeSigned$
        as defined in \crossref{spendauthsig}, using $\AuthSignRandomizedPublic$ as the public key.
\end{consensusrules}
} %sapling
...
\begin{enumerate}
  ...
  \item Generate a proof $\ProofOutput$ for the \outputStatement described in \crossref{outputstatement}.
...
\begin{itemize}
  ...
  \item Choose uniformly random $\NoteCommitRandOld{i} \leftarrowR \NoteCommitSproutTrapdoor$.
...
\begin{bytefield}[bitwidth=0.046em]{512}
    \sbitbox{256}{$\LEBStoOSPOf{256}{\reprJOf{\BindingPublic}\hairspace}$}
...
Let $\BindingSig$, $\combplus$, $\combminus$, $\scombsum{i=1}{\rmN}$, $\grpplus$, $\grpminus$,
and $\sgrpsum{i=1}{\rmN}$ be as specified in \crossref{abstractsighom} and instantiated
in \crossref{concretebindingsig}.
...
\introlist
The \txBindingVerificationKey is calculated as:
\begin{formulae}
...
\introlist
The corresponding signing key is calculated as:
\begin{formulae}
...
\begin{pnotes}
  \item The spender \MAY reveal any strict subset of the \valueCommitment randomnesses to
  ...
  \item The technique of checking signatures using a public key derived from a sum of
...
This could have been proven directly in the \spendStatement, similar to the
check in \crossref{sproutspendauthority} that is part of the \joinSplitStatement.
...
key is a randomization of the \spendAuthAddressKey $\AuthSignPublic$ with a randomizer
...
Let $\dataToBeSigned := \Justthebox{\spendauthsigmsgbox}$.
...
\begin{formulae}
  ...
  \item Generate a proof $\ProofSpend$ of the \spendStatement (\crossref{spendstatement})
        with $\AuthSignRandomizer$ in the \auxiliaryInput.
...
$\PRFnfSapling{\AuthProvePublicRepr}(\NoteAddressRand)$.
} %sapling
...
\begin{formulae}
  \item $\oparen\rt \typecolon \MerkleHashSapling,\\
         ...
         \hparen\AuthSignRandomizedPublic \typecolon \GroupJ\cparen$,
\end{formulae}
...
\begin{formulae}
  \item $\oparen\TreePath{} \typecolon \typeexp{\MerkleHash}{\MerkleDepthSapling},\\
         ...
         \hparen\ValueCommitRandOld{} \typecolon \binaryrange{\ScalarLength},\\ %\ValueCommitTrapdoor,\\
         \hparen\cmOld{} \typecolon \NoteCommitSaplingOutput,\\
         \hparen\NoteCommitRandOld{} \typecolon \binaryrange{\ScalarLength},\\ %\NoteCommitSaplingTrapdoor,\\
         \hparen\AuthSignRandomizer \typecolon \binaryrange{\ScalarLength},\\
         \hparen\AuthSignPublic \typecolon \GroupJ,\\
...
%  $\AuthSignRandomizedPublic \typecolon \GroupJ$ &$= \abstJOf{\AuthSignRandomizedPublicRepr}$ &$\neq \bot$ \\
%  $\DiversifiedTransmitBase  \typecolon \GroupJ$ &$= \abstJOf{\DiversifiedTransmitBaseRepr}$  &$\neq \bot$ \\
%  $\AuthSignPublic           \typecolon \GroupJ$ &$= \abstJOf{\AuthSignPublicRepr}$           &$\neq \bot$
%\end{tabular}
such that the following conditions hold:

\snarkcondition{Note commitment integrity} \label{spendnotecommitmentintegrity}

$\pack(\cmOld{}) = \NoteCommitSapling{\NoteCommitRandOld{}}(\reprJOf{\DiversifiedTransmitBase},
                                                            \reprJOf{\DiversifiedTransmitPublic},
                                                            \vOld{})$.
...
\begin{itemize}
  \item $\vOld{} = 0$; or
  \item $(\TreePath{}, \NotePosition)$ is a valid \merklePath of depth $\MerkleDepthSapling$,
...
$\AuthSignRandomizedPublic = \AuthSignPublic + \scalarmult{\AuthSignRandomizer}{\AuthSignBase}$.
...
$\InViewingKey = \CRHivk(\AuthSignPublicRepr, \AuthProvePublicRepr)$.
...
\begin{formulae}
  \item $(\DiversifiedTransmitBase \typecolon \GroupJ,\\
   \hparen\DiversifiedTransmitPublic \typecolon \GroupJ,\\
...
%\begin{tabular}{@{\hskip 2em}r@{\;}l@{\;}l}
%  $\DiversifiedTransmitBase \typecolon \GroupJ$ &$= \abstJOf{\DiversifiedTransmitBaseRepr}$ &$\neq \bot$ \\
%  $\EphemeralPublic         \typecolon \GroupJ$ &$= \abstJOf{\EphemeralPublicRepr}$         &$\neq \bot$
%\end{tabular}
%
such that the following conditions hold:
...
\snarkcondition{Small order checks} \label{outputnonsmall}

$\DiversifiedTransmitBase$ and $\DiversifiedTransmitPublic$ are not of small order,
i.e.\ $\scalarmult{8}{\DiversifiedTransmitBase} \neq \ZeroJ$
and $\scalarmult{8}{\DiversifiedTransmitPublic} \neq \ZeroJ$.
...
\begin{itemize}
  ...
  \item Let $\OutPlaintext = \LEBStoOSPOf{512}{\reprJOf{\DiversifiedTransmitPublicNew} \bconcat \ItoLEBSP{256}(\EphemeralPrivate)}$.
...
\begin{formulae}
  ...
  \item \tab \tab Attempt to decrypt the \noteCiphertext component
        ...
        \tab \tab \crossref{saplingdecryptivk}. If this succeeds giving $\NotePlaintext{}$:
...
\begin{formulae}
  \item $\PRFexpand{\SpendingKey}(t) := \BlakeTwobOf{512}{\ascii{Zcash\_ExpandSeed}, \Justthebox{\expandbox}}$
\end{formulae}
...
$\NoteAddressRand$. Note that $\AuthProvePublicRepr$ is a representation of a
point in a subgroup of the \jubjubCurve and therefore is not uniformly
distributed on $\ReprJ$.
}
} %sapling
...
\changed{
The \SproutOrNothing{} \keyAgreementScheme specified in \crossref{abstractkeyagreement}
is instantiated using $\KASproutCurve$, described in \cite{Bernstein2006}, as follows.
...
It is based on the scheme from \cite[section 3]{FKMSSS2016}, with some ideas from
...
\introlist
Define $\RedDSARandomizePrivate \typecolon \RedDSAPrivate \times \RedDSARandom \rightarrow \RedDSAPrivate$ by:
\begin{formulae}
  \item $\RedDSARandomizePrivate(\sk, \RedDSARandomizer) := \sk + \RedDSARandomizer \pmod{\ParamG{r}}$.
\end{formulae}

\introlist
Define $\RedDSARandomizePublic \typecolon \RedDSAPublic \times \RedDSARandom \rightarrow \RedDSAPublic$ as:
\begin{formulae}
  \item $\RedDSARandomizePrivate(\vk, \RedDSARandomizer) := \vk + \scalarmult{\RedDSARandomizer}{\GenG{}}$.
\end{formulae}
...
\begin{formulae}
  ...
  \item Let $\RedDSAReprS = \LEBStoOSPOf{\bitlength(\ParamG{r})}{\RedDSASigS}$.
...
\begin{tabular}{@{\hskip 1.5em}r@{\;}l}
  $\RedDSADerivePublic(\sk_1 \grpplus \sk_2)$
    ...
    &$= \scalarmult{\sk_1}{\GenG{}} + \scalarmult{\sk_2}{\GenG{}}$ \\
...
Let $\LEOStoIP{} \typecolon (\ell \typecolon \Nat \suchthat \ell \bmod 8 = 0) \times \byteseq{\ell/8} \rightarrow \binaryrange{\ell}$
be as defined in \crossref{endian}.
...
\begin{bytefield}[bitwidth=0.07em]{344}
    ...
    \sbitbox{256}{$\LEBStoOSPOf{256}{\reprJOf{\DiversifiedTransmitPublic}\hairspace}$}
\end{bytefield}
\end{equation*}
...
$\InViewingKey$ is a $\KASaplingPrivate$ key for use with the encryption scheme
defined in \crossref{saplinginband}. It is derived as described in \crossref{saplingkeycomponents}.
...
and in the multi-party computation used to obtain the \Sapling parameters given in
...
A \sequenceOfJoinSplitDescriptions{} using PHGR13 proofs, each encoded as in \crossref{joinsplitencoding}. \\ \hline
} %sprout
\notsprout{
...
A \sequenceOfJoinSplitDescriptions{} using Groth16 proofs, each encoded as in \crossref{joinsplitencoding}. \\ \hline
} %notsprout
...
\sapling{$\ddagger$ Groth16 proofs are used when the \transaction version is $\geq 4$, i.e.\ after
\Sapling activation.}
}
...
\cite[section 1.3]{BCGGMTV2014}). Since \Zcash combines ``Mint'' and ``Pour''
...
Mary Maller, and no doubt others.
