...
\section{Pour Transfers and Descriptions} \label{pourdesc}
...
\subsection{Commitments and Serial Numbers}
...
(described in \crossref{trstructure}) consists only of Pours.
A Pour is generalized to take a transparent UTXO as input, allowing
Pours to subsume the functionality of Mints. An advantage of this
is that a transaction that takes input from an UTXO can produce
up to $\NNew$ output coins, improving the indistinguishability
properties of the protocol. A related change conceals the input
arity of the Pour: an unused (zero-value) input is
indistinguishable from an input that takes value from a coin.
...
\subsection{Memo fields}

\Zerocash adds a memo field sent from the creator of a Pour to
