...
\newcommand{\NUSixOneSpec}{NU6.1 proposal}
\newtoggle{issapling}
...
$\cb$ \MUST contain at least one distinct output for each of the following:
\begin{itemize}
  \item for each \fundingStream $\fs$ active at that \blockHeight with a recipient
        ...
        an output that pays $\fsValue(\BlockHeight)$ \zatoshi in the \prescribedWay to the
        address represented by that recipient identifier\notnusixone{.}\notbeforenusixone{;}
  \nusixoneonwarditem{if the \blockHeight is $\ZIPTwoSevenOneActivationHeight$, an output
        that pays $\ZIPTwoSevenOneDisbursementAmount$ \zatoshi in the \prescribedWay
        to the Key-Holder Organizations' P2SH multisig address specified in \cite{ZIP-271}.}
\end{itemize}
...
\begin{pnotes}
  \item The \fundingStream addresses are not treated specially in any other way, and
        ...
        other outputs, possibly to the same address, that do not meet the criterion
        in the above consensus rule, as long as there is at least one distinct output
        that meets it for each funding item.
