...
\newcommand{\LockboxDisbursementAmount}{\mathsf{LockboxDisbursementAmount}}
...
at \blockHeight $2726400$ \cite{Zcash-Nu6}. Its specifications are described in this document,
\cite{ZIP-253}, \cite{ZIP-236}, and \cite{ZIP-2001}, with updates to \cite{ZIP-207} and \cite{ZIP-214}.
...
\nusixone{Let $\ZIPTwoSevenOneActivationHeight$ and $\LockboxDisbursementAmount$ be as defined in \cite{ZIP-271}.}
...
\nusixone{
  ...
  \item $\totalDeferredInput(\BlockHeight) := \begin{cases}
           \LockboxDisbursementAmount,&\caseif \BlockHeight = \ZIPTwoSevenOneActivationHeight \\
           0,&\caseotherwise.
         \end{cases}$
} %nusixone
...
and Lockbox Disbursements}}{fundingstreams}}
...
\begin{itemize}
  ...
  \nusixoneonwarditem{if the \blockHeight is $\mathsf{ZIPTwoSevenOneActivationHeight}$,
        an output that pays $\mathsf{LockboxDisbursementAmount}$ \zatoshi in the \prescribedWay
        to the Key-Holder Organization P2SH multisig address specified in \cite{ZIP-271}.}
\end{itemize}
...
\historyentry{2025.6.0}{2025-08-20}
