...
\newcommand{\Sprout}{\notsprout{\termbf{Sprout}}\sprout{\Zcash}}
...
\newcommand{\AuthProveKey}{\mathsf{rsk}}
\newcommand{\PRFKey}{\mathsf{rk}}
\newcommand{\Diversifier}{\mathsf{d}}
\newcommand{\DiversifiedTransmitPublic}{\mathsf{pk}_\mathsf{d}}
...
\newcommand{\ParamJ}[1]{{{#1}_\mathbb{\hskip 0.03em J}}}
\newcommand{\ParamJexp}[2]{{{#1}_\mathbb{\hskip 0.03em J}\!}^{#2}}
\newcommand{\GroupJ}{\mathbb{J}}
\newcommand{\GroupJHash}{\mathsf{GH}^\mathbb{J}}
\newcommand{\CurveJ}{\Curve_{\mathbb{J}}}
\newcommand{\ZeroJ}{\Zero_{\mathbb{J}}}
\newcommand{\GenJ}{\Generator_{\mathbb{J}}}
\newcommand{\ellJ}{\ell_{\mathbb{J}}}
...
% Sapling circuits
%\newcommand{\SaplingSpend}{\mathsf{SaplingSpend}}
%\newcommand{\SaplingOutput}{\mathsf{SaplingOutput}}
...
\newcommand{\ValueCommit}[1]{\mathsf{ValueCommit}_{#1}}
...
\nsubsubsection{\Sprout \JoinSplitStatement} \label{sproutstatement}
...
\sapling{
\introsection
\nsubsubsection{\Sapling \SpendStatement} \label{saplingstatement}
...
over $\GF{\ParamJ{q}}$ with equation $\ParamJ{a} \mult u^2 + v^2 = 1 + \ParamJ{d} \mult u^2 \mult v^2$.
...
\notsprout{hash evaluation for the commitment: $\FullHashName$ for \Sprout,
and \sapling{$\PedersenHash$ for \Sapling}.}
...
\begin{itemize}
  \item The \Zerocash paper did not specify the curve to be used.
        ...
        similar design process following the ``Safe curves'' criteria \cite{safecurves}.
  ...
  \item \sproutonly{The scheme used by \Sprout includes an optimization that reuses
...
in \Zerocash and \Sprout, which \emph{are} collision-resistant assuming
that $\SHA$ is.
...
The \Sapling circuit implements an abstract statement described in
\crossref{saplingstatement}.
At the next lower level, the \Sapling circuit is defined in terms of a
...
The names given to components are those used in the Rust implementation.
...
A \quadraticArithmeticProgram consists of a set of constraints over the
variables, each of the form:

$(A) \times (B) = (C)$

where $(A)$, $(B)$, and $(C)$ are \linearCombinations in $\GF{\ParamS{r}}$.
...
$\AffineEdwardsJubjub = (u \typecolon \GF{\ParamS{r}}) \times (v \typecolon \GF{\ParamS{r}}) : ...$
...
$\CompressedEdwardsJubjub = (\tilde{u} \typecolon \bit) \times (v \typecolon  \GF{\ParamS{r}})$
...
To this end, we ... called the ``distinct-x criterion'':
...
Define $\DecompressValidate \typecolon \CompressedEdwardsJubjub \rightarrow \AffineEdwardsJubjub \union \setof{\bot}$
as follows:
...
\nsubsubsection{Edwards to Montgomery conversion} \label{cctmonttoed}


\nsubsubsection{Montgomery to Edwards conversion} \label{cctedtomont}



\nsubsubsection{Affine-Montgomery arithmetic} \label{cctmontarithmetic}

The affine-Montgomery addition formulae are given in \cite{BL2017} as follows:


Addition of distinct inputs can be implemented as:


In practice we will ensure that inputs are guaranteed to be distinct
when we use this formula.

Doubling can be implemented as:

...

\nsubsubsection{Affine-Edwards arithmetic} \label{cctedarithmetic}

The affine-Edwards addition formulae are given in \cite{BBJLP2008} as follows:

Addition can be implemented as:


We do not need Edwards doubling. For completeness, it would be implemented as:

...


\nsubsubsection{Fixed-base affine-Montgomery scalar multiplication} \label{cctfixedmontscalarmult}
...
\nsubsubsection{Variable-base affine-Montgomery scalar multiplication} \label{cctvarmontscalarmult}

When the base point $B$ is not fixed, the method in the preceding section
cannot be used. Instead we use a naive double-and-add method:
...
for i from 0 up to ...:
    ...
    // select [2] Acc or [2] Acc + B depending on the bit ki
    ()

\pnote{
It would be slightly more efficient to use 2-bit fixed windows, but there are only
two instances of variable-base scalar multiplication in the SaplingSpend circuit
and none in the SaplingOutput circuit, so the additional complexity was not considered
justified.
}
...
within each segment: the distinct-x criterion is met because the scalar ...
...
Checking a Merkle authentication path, as described in ..., requires
boolean-constraining the previous-layer and sibling hashes in addition to
the cost of ...

Therefore, the cost of each layer is $2 \mult 255 + ... = ...$ constraints.

Note that it is not necessary to ensure that the previous-layer or sibling
...
There are $10 \mult 4 \mult 2$ such message word additions.

Each $G$ evaluation requires 260 constraints. There are $10 \mult 8$ instances
of $G$:
