...
\newcommand{\noteAnonymitySet}{\term{note anonymity set}}
...
\transaction in which it was created. Unlike other proposals for private
payment systems that are based on mixing of a limited number of transactions,
such as CoinJoin \cite{Bitcoin-CoinJoin} or CryptoNote \cite{vanS2013}, the
\noteAnonymitySet is all previously created \notes. The \nullifiers are
necessary to prevent double-spending: each note only has one valid \nullifier,
and so attempting to spend a \note twice would reveal the \nullifier twice,
which would cause the second \transaction to be rejected.
