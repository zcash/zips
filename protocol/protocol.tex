...
% The Orchard-like pools are Orchard and Ironwood, which share the Orchard shielded protocol.
\newcommand{\OrchardLikePoolIds}{\OrchardPoolId\notbeforenusixthree{, \IronwoodPoolId}}
...
in \crossref{sproutbalance}\sapling{, \crossref{saplingbalance}}\nufive{,\! and\! \crossref{orchardbalance}}
respectively.
...
\sapling{There is no equivalent of interstitial \treestates for the \SaplingPool\nufive{ nor the \OrchardPool}.}
...
\begin{itemize}
  \item $\cvNet{} \typecolon \ValueCommitOutput{Orchard}$ is the \valueCommitment to the value of the
        ...
        \vspace{-0.5ex}
  \item $\rt{\OrchardPoolId} \typecolon \MerkleHashOrchard$ is an \anchor, as defined in \crossref{transactions},
...
\vspace{-1.5ex}
\pnote{The $\rt{\OrchardPoolId}$, $\enableSpends$, and $\enableOutputs$ components are the same for all
...
In order to send \OrchardPoolAdj \shielded value, the sender constructs a \transaction
...
and optionally perform an output. Similarly to \Sapling, the net value of \OrchardPool
...
$\vBalance{\OrchardPoolId}$ is encoded in a \transaction as the field \valueBalance{Orchard}.
If a \transaction has no \actionDescriptions, $\vBalance{\OrchardPoolId}$ is implicitly zero.
Transaction fields are described in \shortcrossref{txnencoding}.

A positive $\orchardBalancingValue$ takes net value from the \OrchardChainValuePool
and adds it to the \transparentTxValuePool. A negative $\orchardBalancingValue$ does the
reverse. As a result, positive $\vBalance{\OrchardPoolId}$ is treated like an \emph{input} to the
\transparentTxValuePool, whereas negative $\vBalance{\OrchardPoolId}$ is treated like an \emph{output}
from that pool.

\vspace{2ex}
Consistency of $\vBalance{\OrchardPoolId}$ with the \valueCommitments in \actionDescriptions
...
the total value produced) by \actionTransfers is consistent with the $\vBalance{\OrchardPoolId}$
...
\begin{itemize}
  ...
  \item \orchardBalancingValue $\vBalance{\OrchardPoolId}$.
\end{itemize}

\vspace{-1ex}
In a correctly constructed \transaction, $\vBalance{\OrchardPoolId} = \ssum{i=1}{n} \vNet{i}$,
but validators cannot check this directly because the values are hidden by the commitments.
...
\begin{formulae}
  \item $\BindingPublic{Orchard} := \Bigg(\!\vcombsum{i=1}{n}\kern 0.2em \cvNet{i}\kern 0.05em\Bigg) \combminus
                                    \ValueCommit{Orchard}{0}\big(\vBalance{\OrchardPoolId}\big)$.
\end{formulae}
\vspace{-1ex}
(This key is not encoded explicitly in the \transaction and must be recalculated.)
...
\begin{tabular}{@{\hskip 2em}r@{\;}l}
  $\BindingPublic{Orchard}$ &$= \Biggscalarmult{\Bigg(\!\vgrpsum{i=1}{n} \vNet{i}\Bigg) \grpminus \vBalance{\OrchardPoolId}}{\ValueCommitValueBase{Orchard}}\, \combplus
                            ...
                            &$= \ValueCommit{Orchard}{\BindingPrivate{Orchard}}\Bigg(\!\vsum{i=1}{n} \vNet{i} - \vBalance{\OrchardPoolId}\Bigg)$.
\end{tabular}

\introlist
Let $\vSum = \vsum{i=1}{n} \vNet{i} - \vBalance{\OrchardPoolId}$.
...
are in $\SignedValueDifferenceType$. $\vBalance{\OrchardPoolId}$ is encoded in the \transaction as a
...
\pnote{
The spender \MAY reveal any strict subset of the \OrchardPoolAdj \valueCommitment randomnesses to
...
\heartwood{
  ...
  \nufiveonwarditem{The flags in \flagsOrchard{} allow a version 5 \transaction to declare that no funds are
        spent from \OrchardPoolAdj \notes (by setting \enableSpendsOrchard{} to $0$), or that no new \OrchardPoolAdj \notes
        with nonzero values are created (by setting \enableOutputsOrchard{} to $0$). This has two primary
        purposes. First, the value of the \enableSpendsOrchard{} flag is required to be $0$ in version 5 \coinbaseTransactions to
        ensure that they cannot spend from existing \Orchard outputs. This maintains a restriction present
        in \coinbaseTransactions for \TransparentPoolAdj, \SproutPoolAdj, or \SaplingPoolAdj funds, which
        would not otherwise be enforceable in the combined \actionTransfer design.
  ...
  \nufiveonwarditem{Because \enableSpendsOrchard{} is set to $0$ in version 5 \coinbaseTransactions\ --which
        disables non-zero-valued \OrchardPoolAdj spends-- the $\valueBalance{Orchard}$ field of a \coinbaseTransaction
