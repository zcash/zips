...
\begin{formulae}
  \item \begin{flushleft}
    ...
    $\MAXMONEY$,\blossom{ $\BlossomActivationHeight$,}\canopy{ $\CanopyActivationHeight$,} $\SlowStartInterval$, $\PreBlossomHalvingInterval$,
...
onward, this procedure differs from that in \crossref{saplingdecryptivk} only in checking that the
ephemeral \privateKey $\EphemeralPrivate'$ derived from $\NoteSeedBytes$ is identical to that obtained
by decrypting the \outputCiphertext $\OutCiphertext$.}
...
\begin{itemize}
  ...
  \item $\EdDSAReprR{}$ and $\EdDSAReprA{}$ \MUST be encodings of points $R$ and $A$ respectively
        on the \EdSpecific curve;
  \precanopyitem{$\EdDSAReprR{}$ and $\EdDSAReprA{}$ \MUSTNOT be in $\ExcludedPointEncodings$;}
...
of the points represented by $\EdDSAReprR{}$ or $\EdDSAReprA{}$ are less than $2^{255}-19$.
  \item Appendix \crossref{ed25519batchvalidate} describes an optimization that \MAY be used to speed up
...
from $\EdDSAReprR{}$ and $\EdDSAReprA{}$ is due to a quirk of version 1.0.15 of the
...
\lsubsection{Ed25519 batch validation}{ed25519batchvalidate}

The reference validation algorithm for Ed25519 signatures is defined in \crossref{concreteed25519}.
...
\sprout{
The performance benefit of this approach arises partly from replacing the per-signature
scalar multiplication of the base $\EdDSABase$ with one such multiplication per batch,
and partly from using an efficient algorithm for multiscalar multiplication such
as Pippinger's method \cite{Bernstein2001} or the Bos--Coster method \cite{deRooij1995}, as explained in
\cite[section 5]{BDLSY2012}.
}
