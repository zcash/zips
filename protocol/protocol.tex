...
\begin{itemize}
  ...
  \item a verifying key derivation algorithm $\SigDerivePublic \typecolon \SigPrivate \rightarrow \SigPublic$;
...
such that for any key pair $(\sk \leftarrowR \SigGenPrivate(), \vk = \SigDerivePublic(\sk))$, and
...
\introsection
\securityrequirement{
For $N \typecolon \PosInt$ and a sequence of private keys $\sk_{\alln}$ where
$\sk_i \leftarrowR \SigGenPrivate()$, the distribution of $\sgrpsum{i=1}{n} \sk_i$ given
knowledge of $\sk_{\oneto{n-1}}$ is computationally indistinguishable from that of
$\SigGenPrivate()$.
} %securityrequirement
} %sapling
...
is instantiated using Curve25519, described in \cite{Bernstein2006}, as follows.
...
\changed{
  ...
  \item \changed{$32$ bytes} specifying $\TransmitPublic$, \changed{using the
        normal encoding of a Curve25519 public key \cite{Bernstein2006}}.
\end{itemize}
...
\begin{itemize}
  ...
  \item $32$ bytes specifying $\TransmitPrivate$, using the normal encoding
        of a Curve25519 private key \cite{Bernstein2006}.
\end{itemize}
