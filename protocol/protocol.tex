...
Bits within a byte are shown with the \emph{most significant} bit on the left
of a diagram. Where bit fields are used, the text will clarify their position
in each case. In cases where a field starts at a position that is not a byte
boundary, that field will always be a bit sequence, and its initial bits will
occupy the less significant bits of the overlapping first byte.
