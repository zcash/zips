...
% No automatic hyphenation and keep existing hyphens unbroken (use for names, hyphenated terms).
\newcommand{\nh}[1]{\nohyphens{#1}}
...
% Fonts
\usepackage{lmodern}
\usepackage[nosfdefault,type1,medium,scale=0.91]{raleway}
% The Raleway Tabular Lining Figures (TLF) fonts used for pool names (see
% \ralewayitalictype) are vendored under protocol/texmf/, because newer TeX Live
% (e.g. 2025 under `nix develop`) ships raleway without the TLF family or its
% italic Type1 metrics. Register the vendored font map so pdfTeX embeds them.
\ifdefined\pdfmapfile\pdfmapfile{+Raleway.map}\fi
...
% Quattrocento is beautiful but doesn't have an italic face. So for emphasis, we
% scale New Century Schoolbook italic to fit in with slanted Quattrocento and match
% its x height. (This has the side effect of making all emphasized text into boxes,
% so it won't linebreak, which has pluses and minuses.)
%
% XeTeX and LuaLaTeX require the \fontencoding{T1}:
% <https://tex.stackexchange.com/a/426300/78411>
%
% New Century Schoolbook italic bold is too wide, and there is no math sans italic
% font, so we override \strongvartype to \strongvaritalictype, and \mathsansvartype
% to \mathsansitalictype to provide better alternatives while in \emph.
\newcommand{\schoolbook}[1]{{\fontencoding{T1}\fontfamily{pnc}\selectfont{#1}}}
\newcommand{\schoolbookitalic}[1]{\scalebox{1.02}[0.999]{\schoolbook{#1}}}
\renewcommand{\emph}[1]{\hspace{0.12em}{\schoolbookitalic{\renewcommand{\mathsansvartype}{\mathsansitalictype}\renewcommand{\strongvartype}{\strongitalictype}\textit{#1}}}\hspace{0.12em}}

% <https://tex.stackexchange.com/a/63188/78411> Bruno Le Floch; retrieved 2026-06-15; CC BY-SA 4.0
\newsavebox{\argumentbox}
\newcommand{\slantbox}[2][0.21]{\mbox{\sbox{\argumentbox}{#2}\hskip\wd\argumentbox\pdfsave\pdfsetmatrix{1 0 #1 1}\llap{\usebox{\argumentbox}}\pdfrestore}}

% There is no math italic font available. \textit\textsf has noticeably different
% metrics and glyphs (wider, lower weight, and the g glyph is open, which occurs
% frequently in crossrefs containing "Sapling"). Instead we use a slanted variant of
% real mathsf, which is fine for a sans-serif font in the specific contexts we're
% using it.
\newcommand{\mathsansitalictype}[1]{\kern-0.08em\slantbox{$\mathsf{#1}$}\kern 0.06em}

% Use Raleway Italic for pool names. This is a deliberately idiosyncratic stylistic
% choice (I like the curved crossed staves of the w in Ironwood), consistent with
% the ZIPs as of zips#1301 and zips#1309.
\newcommand{\ralewaytype}[1]{{\fontencoding{T1}\fontfamily{Raleway-TLF}\selectfont{#1}}}
\newcommand{\ralewayitalictype}[1]{{\fontencoding{T1}\fontfamily{Raleway-TLF}\selectfont\textsl{#1}}}
...
\makeatletter
\renewcommand*{\@fnsymbol}[1]{\ensuremath{\ifcase#1\or \dagger\or \ddagger\or \mathsection\or \medlozenge\or \mathparagraph\else\@ctrerr\fi}}
\makeatother
...
\newcommand{\zcashdref}[1]{\href{https://github.com/zcash/zcash/releases/tag/#1}{\softwaretype{zcashd\;#1}}}
\newcommand{\zebraref}[1]{\href{https://github.com/ZcashFoundation/zebra/releases/tag/#1}{\softwaretype{zebra\;#1}}}
...
% We just want fivedots and medlozenge symbols from MnSymbol.
\DeclareSymbolFont{MnSyC}{U}{MnSymbolC}{m}{n}
\DeclareFontFamily{U}{MnSymbolC}{}
\DeclareFontShape{U}{MnSymbolC}{m}{n}{<->MnSymbolC10}{}
\DeclareMathSymbol{\fivedots}{\mathbin}{MnSyC}{15}
\DeclareMathSymbol{\medlozenge}{\mathord}{MnSyC}{197}
...
\iftoggle{darkmode}{
  ...
  \colorlet{foregroundcolor}{lighttext}
  ...
  \colorlet{foregroundcolor}{black}
...
\newcommand{\todo}[1]{{\color{foregroundcolor}{$\{$\,\schoolbook{TODO: #1}\,$\}$}}}
...
% Typefaces: Presentation

\newcommand{\strongtype}{\textbf}
\newcommand{\strongitalictype}[1]{\fakebold{\textit{#1}}}
\newcommand{\strong}[1]{\strongvartype{#1}\kern0.2em}

% \strongvartype is normally \strongtype, but is overridden to be \strongitalictype while in \emph.
\newcommand{\strongvartype}{\strongtype}

% \indextype is normally normalstyle (plain), but is overridden to be strongitalictype while in \defining.
% This is what makes the page numbers in the index italic bold for defining instances.
...
\newcommand{\mathsanstype}[1]{\ensuremath{\mathsf{#1}}}

% \mathsansvartype is normally mathsanstype, but is overridden to be mathsansitalictype while in \emph.
\newcommand{\mathsansvartype}{\mathsanstype}

\newcommand{\slantedtype}[1]{\textsl{#1}}

% Typefaces: Semantics

% Conformance keywords
\newcommand{\conformancetype}{\strongvartype}

% Network upgrades (Sprout is included)
\newcommand{\upgradetype}{\strongvartype}

% Top-level protocols (Zcash, Bitcoin, etc.)
\newcommand{\topprotocoltype}{\strongvartype}

% Sub-protocols (Sprout, Sapling, Orchard)
\newcommand{\subprotocoltype}{\mathsansvartype}

% Pools (transparent, Sprout, Sapling, Orchard, Ironwood)
\newcommand{\pooltype}{\ralewayitalictype}

% Software packages and libraries
\newcommand{\softwaretype}{\mathsansitalictype}

% Major currency units (ZEC, TAZ)
\newcommand{\majorcurrencyunittype}{\strongvartype}

% Minor currency units (zatoshi)
\newcommand{\minorcurrencyunittype}{\textsf}

% Field names
\newcommand{\fieldnametype}{\texttt}

% Filenames
\newcommand{\filenametype}{\texttt}

% Pairings
\newcommand{\pairingtype}{\mathsansvartype}

% Elliptic curves
\newcommand{\ellipticcurvetype}{\mathsansvartype}

% The arguments are: {anchor_as_formatted_in_text}{index_sort_key}{index_as_formatted}.
%
...
\newcommand{\termandindex}[2]{\indexlink{\rawterm{#1}}{#2}{#2}}  % index formatted as sort key
\newcommand{\termandindexx}[2]{\indexlink{#1}{#2}{#1}}  % index formatted as anchor
\newcommand{\term}[1]{\termandindex{#1}{#1}}
\newcommand{\terms}[1]{\termandindex{#1s}{#1}}
\newcommand{\termes}[1]{\termandindex{#1es}{#1}}
\newcommand{\termps}[1]{\termandindex{#1(s)}{#1}}
...
\newcommand{\termbfandindex}[2]{\termandindexx{\textbf{#1}}{#2}}
\newcommand{\termsf}[1]{\termandindexx{\textsf{#1}}{#1}}
\newcommand{\termsfandindex}[2]{\termandindexx{\textsf{#1}}{#2}}

% Define only the single-argument cases here; others don't help conciseness.
\newcommand{\termconformance}[1]{\termandindexx{\conformancetype{#1}}{#1}}
\newcommand{\termtopprotocol}[1]{\termandindexx{\topprotocoltype{#1}}{#1}}
\newcommand{\termsubprotocol}[1]{\termandindexx{\subprotocoltype{#1}}{#1}}
\newcommand{\termpoolandindex}[2]{\indexlink{\pooltype{#1}}{#2}{#2}}
\newcommand{\termsoftware}[1]{\termandindexx{\softwaretype{#1}}{#1}}
\newcommand{\termmajorcurrencyunit}[1]{\termandindexx{\majorcurrencyunittype{#1}}{#1}}
\newcommand{\termminorcurrencyunit}[1]{\termandindexx{\minorcurrencyunittype{#1}}{#1}}
...
% Use `\readas[unicode]` to resolve ambiguity for listeners ("Sapling upgrade", "Orchard pool", etc.)
% TODO: does `\readas` work in headings?
%
% The `*Text` macros are for headings, and should be used with `{}` for correct spacing.
% FIXME: allow headings to use the semantic macros rather than `*Text{}`.

\newcommand{\Zcash}{\termtopprotocol{Zcash}}
\newcommand{\Zerocash}{\termtopprotocol{Zerocash}}
\newcommand{\ZerocashText}{\topprotocoltype{Zerocash}}

% Network Upgrades
\newcommand{\NUOverwinter}{\termandindexx{\upgradetype{Overwinter}}{Overwinter (Network Upgrade 0)}}
\newcommand{\NUSapling}{\readas{Sapling upgrade}{\termandindexx{\upgradetype{Sapling}}{Sapling (Network Upgrade 1)}}\xspace}
\newcommand{\NUBlossom}{\termandindexx{\upgradetype{Blossom}}{Blossom (Network Upgrade 2)}}
\newcommand{\NUHeartwood}{\termandindexx{\upgradetype{Heartwood}}{{Heartwood (Network Upgrade 3)}}}
\newcommand{\NUCanopy}{\termandindexx{\upgradetype{Canopy}}{Canopy (Network Upgrade 4)}}
% "N<zwsp>U<zwsp>5"
\newcommand{\NUFive}{\readasunicode{004e200b0055200b0035}{\termandindexx{\upgradetype{NU5}}{NU5 (Network Upgrade 5)}}\xspace}
% "N<zwsp>U<zwsp>6"
\newcommand{\NUSix}{\readasunicode{004e200b0055200b0036}{\termandindexx{\upgradetype{NU6}}{NU6 (Network Upgrade 6)}}\xspace}
% "N<zwsp>U<zwsp>6.1"
...
% names render as mathsans everywhere.
\newcommand{\SproutProto}{\subprotocoltype{Sprout}}
\newcommand{\SaplingProto}{\subprotocoltype{Sapling}}
\newcommand{\OrchardProto}{\subprotocoltype{Orchard}}

% The Sprout shielded protocol.
\newcommand{\SproutShieldedProtocol}{\termandindex{\SproutProto{} shielded protocol}{Sprout shielded protocol}}
\newcommand{\SproutProtocolAdj}{\termandindex{\SproutProto{}-protocol}{Sprout shielded protocol}}
\newcommand{\Sprout}{\readas{Sprout protocol}{\SproutProto}\xspace}
\newcommand{\SproutText}{\texorpdfstring{\SproutProto}{Sprout}}

% The Sapling shielded protocol
\newcommand{\SaplingShieldedProtocol}{\termandindex{\SaplingProto{} shielded protocol}{Sapling shielded protocol}}
\newcommand{\SaplingProtocolAdj}{\termandindex{\SaplingProto{}-protocol}{Sapling shielded protocol}}
\newcommand{\Sapling}{\readas{Sapling protocol}{\SaplingProto}\xspace}
\newcommand{\SaplingText}{\texorpdfstring{\SaplingProto}{Sapling}}

% The Orchard shielded protocol (used for both Orchard and Ironwood pools)
\newcommand{\OrchardShieldedProtocol}{\termandindex{\OrchardProto{} shielded protocol}{Orchard shielded protocol}}
\newcommand{\OrchardProtocolAdj}{\termandindex{\OrchardProto{}-protocol}{Orchard shielded protocol}}
\newcommand{\Orchard}{\readas{Orchard protocol}{\OrchardProto}\xspace}
\newcommand{\OrchardText}{\texorpdfstring{\OrchardProto}{Orchard}}

% The Sapling or Orchard shielded protocols
\newcommand{\SaplingOrOrchard}{\Sapling{}\nufive{ or \Orchard{}}\xspace}
\newcommand{\SaplingOrOrchardText}{\SaplingText{}\notbeforenufive{ or \OrchardText{}}}
...
% TODO: better readas
%\newcommand{\SaplingOrOrchard}{\notnufive{\Sapling}\notbeforenufive{\readas{Sapling or Orchard protocol}{\llSapling{}\nufive{ or \llOrchard{}}}}\xspace}
%\newcommand{\SaplingAndOrchard}{\notnufive{\Sapling}\notbeforenufive{\readas{Sapling and Orchard protocol}{\llSapling{}\nufive{ and \llOrchard{}}}}\xspace}

% The Sprout pool
\newcommand{\SproutPoolId}{\pooltype{SproutPool}}
\newcommand{\SproutChainValuePool}{\termpoolandindex{Sprout chain value pool}{Sprout pool}}
\newcommand{\SproutPool}{\termpoolandindex{Sprout pool}{Sprout pool}}
\newcommand{\SproutPoolAdj}{\termpoolandindex{Sprout-pool}{Sprout pool}}
% There is no Sprout transaction value pool, since JoinSplits are balanced individually.

% The Sapling pool
\newcommand{\SaplingPoolId}{\pooltype{SaplingPool}}
\newcommand{\SaplingChainValuePool}{\termpoolandindex{Sapling chain value pool}{Sapling pool}}
\newcommand{\SaplingPool}{\termpoolandindex{Sapling pool}{Sapling pool}}
\newcommand{\SaplingPoolAdj}{\termpoolandindex{Sapling-pool}{Sapling pool}}

% The Orchard pool
\newcommand{\OrchardPoolId}{\pooltype{OrchardPool}}
\newcommand{\OrchardChainValuePool}{\termpoolandindex{Orchard chain value pool}{Orchard pool}}
\newcommand{\OrchardPool}{\termpoolandindex{Orchard pool}{Orchard pool}}
\newcommand{\OrchardPoolAdj}{\termpoolandindex{Orchard-pool}{Orchard pool}}

% The Ironwood pool
\newcommand{\IronwoodPoolId}{\pooltype{IronwoodPool}}
\newcommand{\IronwoodChainValuePool}{\termpoolandindex{Ironwood chain value pool}{Ironwood pool}}
\newcommand{\IronwoodPool}{\termpoolandindex{Ironwood pool}{Ironwood pool}}
\newcommand{\IronwoodPoolAdj}{\termpoolandindex{Ironwood-pool}{Ironwood pool}}

% Sapling or Orchard pool, excluding Ironwood.
\newcommand{\SaplingOrOrchardPool}{\sapling{\SaplingPool{}}\nufive{ or \OrchardPool{}}\xspace}
\newcommand{\SaplingOrOrchardPoolAdj}{\sapling{\SaplingPoolAdj{}}\nufive{ or \OrchardPoolAdj{}}\xspace}
\newcommand{\SaplingAndOrchardPools}{\sapling{\SaplingPool{}}\nufive{ and \OrchardPool{}}\xspace}
\newcommand{\SaplingAndOrchardPoolsAdj}{\sapling{\SaplingPoolAdj{}}\nufive{ and \OrchardPoolAdj{}}\xspace}
...
\newcommand{\SaplingLikeChainValuePool}{\notnufive{\SaplingChainValuePool{}}%
  \notbeforenufive{\termpoolandindex{Sapling or Orchard chain value pool}{Sapling or Orchard pool}}%
  \notbeforenusixthree{\termpoolandindex{Sapling, Orchard, or Ironwood chain value pool}{Sapling, Orchard, or Ironwood pool}}\xspace}
\newcommand{\SaplingLikePool}{\SaplingPool{}\nufive{ or \OrchardPool{}}\nusixthree{ or \IronwoodPool{}}\xspace}
\newcommand{\SaplingLikePoolAdj}{\SaplingPoolAdj{}\nufive{ or \OrchardPoolAdj{}}\nusixthree{ or \IronwoodPoolAdj{}}\xspace}

\newcommand{\SaplingLikeChainValuePools}{\notnufive{\SaplingChainValuePool{}}%
  \notbeforenufive{\termpoolandindex{Sapling and Orchard chain value pools}{Sapling or Orchard pool}}%
  \notbeforenusixthree{\termpoolandindex{Sapling, Orchard, and Ironwood chain value pools}{Sapling, Orchard, or Ironwood pool}}\xspace}
\newcommand{\SaplingLikePools}{\SaplingPool{}\nufive{ and \OrchardPool{}}\nusixthree{ and \IronwoodPool{}}\xspace}
\newcommand{\SaplingLikePoolsAdj}{\SaplingPool{}\nufive{ and \OrchardPool{}}\nusixthree{ and \IronwoodPool{}}\xspace}
\newcommand{\TheSaplingLikePoolsNegative}{the \SaplingPool{}\nufive{ nor the \OrchardPool{}}\nusixthree{ nor the \IronwoodPool{}}\xspace}

% Either \OrchardPoolId or a generic \Pool in {\OrchardPoolId, \IronwoodPoolId}.
\newcommand{\OrchardLikePoolId}{\notnusixthree{\OrchardPoolId}\nusixthree{\Pool}}
\newcommand{\OrchardLikePoolIdList}{\OrchardPoolId\nusixthree{, \IronwoodPoolId}}

\newcommand{\OrchardLikeChainValuePool}{\notnusixthree{\OrchardChainValuePool{}}\notbeforenusixthree{\termpoolandindex{Orchard \nusixthree{or Ironwood} chain value pool}{Orchard or Ironwood pool}\xspace}}
\newcommand{\OrchardLikePool}{\OrchardPool{}\nusixthree{ or \IronwoodPool{}}\xspace}
\newcommand{\OrchardLikePoolAdj}{\OrchardPoolAdj{}\nusixthree{ or \IronwoodPoolAdj{}}\xspace}

\newcommand{\OrchardLikeChainValuePools}{\notnusixthree{\OrchardChainValuePool{}}\notbeforenusixthree{\termpoolandindex{Orchard \nusixthree{and Ironwood} chain value pool}{Orchard or Ironwood pool}\xspace}}
\newcommand{\OrchardLikePools}{\OrchardPool{}\nusixthree{ and \IronwoodPool{}}\xspace}
\newcommand{\AllOrchardLikePools}{\nusixthree{both }the \OrchardPool{}\nusixthree{ and the \IronwoodPool{}}\xspace}
\newcommand{\TheOrchardLikePoolsNegative}{the \OrchardPool{}\nusixthree{ nor the \IronwoodPool{}}\xspace}

% Transparent pool
\newcommand{\TransparentPoolId}{\pooltype{TransparentPool}}
\newcommand{\TransparentChainValuePool}{\termpoolandindex{transparent chain value pool}{transparent pool}}
\newcommand{\TransparentPool}{\termpoolandindex{transparent pool}{transparent pool}}
\newcommand{\TransparentPoolAdj}{\termpoolandindex{Transparent-pool}{transparent pool}}

% Deferred development fund pool
\newcommand{\DeferredPoolId}{\pooltype{DeferredPool}}
\newcommand{\DeferredChainValuePool}{\termpoolandindex{deferred development fund chain value pool}{deferred development fund pool}}
\newcommand{\DeferredPool}{\termpoolandindex{deferred development fund pool}{deferred development fund pool}}

\newcommand{\transparentTxValuePool}{\term{transparent transaction value pool}}

\newcommand{\Bitcoin}{\termtopprotocol{Bitcoin}}
\newcommand{\BitcoinText}{\topprotocoltype{Bitcoin}\xspace}

\newcommand{\CryptoNote}{\termsubprotocol{CryptoNote}}
\newcommand{\Mimblewimble}{\termsubprotocol{Mimblewimble}}
\newcommand{\Bulletproofs}{\termsubprotocol{Bulletproofs}}

\newcommand{\ZEC}{\termmajorcurrencyunit{ZEC}}
\newcommand{\TAZ}{\termmajorcurrencyunit{TAZ}}
\newcommand{\zatoshi}{\termminorcurrencyunit{zatoshi}}

\newcommand{\zcashd}{\termsoftware{zcashd}}
\newcommand{\zebra}{\termsoftware{zebra}}
\newcommand{\librustzcash}{\termsoftware{librustzcash}}
\newcommand{\BitcoinCore}{\termandindex{\softwaretype{Bitcoin Core}}{Bitcoin Core}}

\newcommand{\Makefile}{\filenametype{Makefile}\xspace}

\newcommand{\MUST}{\termconformance{MUST}}
\newcommand{\MUSTNOT}{\termconformance{MUST NOT}}
\newcommand{\SHOULD}{\termconformance{SHOULD}}
\newcommand{\SHOULDNOT}{\termconformance{SHOULD NOT}}
\newcommand{\RECOMMENDED}{\termconformance{RECOMMENDED}}
\newcommand{\MAY}{\termconformance{MAY}}
\newcommand{\OPTIONAL}{\termconformance{OPTIONAL}}
\newcommand{\ALLCAPS}{\conformancetype{ALL CAPS}\xspace}
...
\newcommand{\randomOracleAdj}{\termandindex{random-oracle}{random oracle}}
...
\newcommand{\smallOrderAdj}{\termandindex{small-order}{small order (of a group element)}}
\newcommand{\primeOrder}{\termandindex{prime order}{prime order (of a group element)}}
\newcommand{\primeOrderAdj}{\termandindex{prime-order}{prime order (of a group element)}}
...
\newcommand{\DecentralizedAnonymousPaymentScheme}{\term{Decentralized Anonymous Payment scheme}}
...
\newcommand{\bitSequenceAdj}{\termandindex{bit-sequence}{bit sequence}}
\newcommand{\byteSequence}{\term{byte sequence}}
\newcommand{\byteSequences}{\terms{byte sequence}}
\newcommand{\byteSequenceAdj}{\termandindex{byte-sequence}{byte sequence}}
...
\newcommand{\shieldedPool}{\term{shielded pool}}
\newcommand{\xShieldedPool}{\termx{shielded pool}}
\newcommand{\shieldedPools}{\terms{shielded pool}}
\newcommand{\shieldedPoolps}{\termps{shielded pool}}
...
\newcommand{\ironwoodBindingSignature}{\termandindex{Ironwood binding signature}{binding signature (Ironwood)}}
\newcommand{\ironwoodBindingSignatures}{\termandindex{Ironwood binding signatures}{binding signature (Ironwood)}}
\newcommand{\orchardLikeBindingSignature}{\termandindex{Orchard\notbeforenusixthree{ or Ironwood} binding signature}{binding signature (Orchard-like)}}
\newcommand{\orchardLikeBindingSignatures}{\termandindex{\Orchard\notbeforenusixthree{ or Ironwood} binding signatures}{binding signature (Orchard-like)}}
...
\newcommand{\saplingBalancingValue}{\termandindex{Sapling-pool balancing value}{balancing value (Sapling pool)}}
\newcommand{\orchardBalancingValue}{\termandindex{Orchard-pool balancing value}{balancing value (Orchard pool)}}
\newcommand{\ironwoodBalancingValue}{\termandindex{Ironwood-pool balancing value}{balancing value (Ironwood pool)}}
\newcommand{\orchardLikeBalancingValue}{\termandindex{Orchard- or Ironwood-pool balancing value}{balancing value (Orchard-like pool)}}
...
\newcommand{\BCTV}{\termsubprotocol{BCTV14}}
\newcommand{\BCTVText}{\texorpdfstring{\subprotocoltype{BCTV14}}{BCTV14}}
\newcommand{\Groth}{\termsubprotocol{Groth16}}
\newcommand{\GrothText}{\texorpdfstring{\subprotocoltype{Groth16}}{Groth16}}
\newcommand{\llHaloTwo}{\subprotocoltype{Halo\;2}}
\newcommand{\HaloTwo}{\termandindexx{\llHaloTwo}{Halo 2}}
\newcommand{\HaloTwoText}{\texorpdfstring{\llHaloTwo}{Halo 2}}
\newcommand{\halotwo}{\termandindexx{\softwaretype{halo2}}{halo2 (library)}}
\newcommand{\PLONK}{\termsubprotocol{PLONK}}

% Pairings
\newcommand{\llBNPairing}{\pairingtype{BN\mhyphen254}}
\newcommand{\BNPairing}{\termandindex{\ensuremath{\llBNPairing}}{BN-254 pairing}}
\newcommand{\BNPairingText}{\texorpdfstring{\llBNPairing}{BN-254}}

\newcommand{\llBLSPairing}{\pairingtype{BLS12\mhyphen381}}
\newcommand{\BLSPairing}{\termandindex{\ensuremath{\llBLSPairing}}{BLS12-381 pairing}}
\newcommand{\BLSPairingText}{\texorpdfstring{\llBLSPairing}{BLS12-381 pairing}}

% Elliptic curves
\newcommand{\Jubjub}{\termandindex{\ellipticcurvetype{Jubjub}}{Jubjub elliptic curve}}
\newcommand{\jubjubCurve}{\termandindex{Jubjub curve}{Jubjub elliptic curve}}
\newcommand{\JubjubText}{\texorpdfstring{\ellipticcurvetype{Jubjub}}{Jubjub}}

\newcommand{\Pallas}{\termandindex{\ellipticcurvetype{Pallas}}{Pallas elliptic curve}}
\newcommand{\pallasCurve}{\termandindex{Pallas curve}{Pallas elliptic curve}}
\newcommand{\PallasText}{\texorpdfstring{\ellipticcurvetype{Pallas}}{Pallas}}
\newcommand{\llIsoPallas}{\ellipticcurvetype{iso\kern0.05em\mhyphen\kern-0.05em Pallas}}
\newcommand{\IsoPallas}{\termandindex{\llIsoPallas}{iso-Pallas elliptic curve}}

\newcommand{\Vesta}{\termandindex{\ellipticcurvetype{Vesta}}{Vesta elliptic curve}}
\newcommand{\vestaCurve}{\termandindex{Vesta curve}{Vesta elliptic curve}}
\newcommand{\VestaText}{\texorpdfstring{\ellipticcurvetype{Vesta}}{Vesta}}
\newcommand{\llIsoVesta}{\ellipticcurvetype{iso\kern0.05em\mhyphen\kern-0.05em Vesta}}
\newcommand{\IsoVesta}{\termandindex{\llIsoVesta}{iso-Vesta elliptic curve}}

\newcommand{\PallasAndVestaText}{\texorpdfstring{\ellipticcurvetype{Pallas}{} and \ellipticcurvetype{Vesta}{}}{Pallas and Vesta}}

\newcommand{\secpCurve}{\termandindex{secp256k1 curve}{secp256k1 elliptic curve}}
...
\newcommand{\chain}{\termandindex{chain}{block chain}}
...
\newcommand{\xNotePlaintexts}{\termxs{note plaintext}}
...
\newcommand{\allowedLeadBytes}[1]{\mathsf{allowedLeadBytes}^{\kern0.1em\text{#1}}}
...
\newcommand{\DeriveInternalFVKOrchard}{\mathsf{DeriveInternalFVK}^{\OrchardProto}}
\newcommand{\DeriveDkAndOvkOrchard}{\mathsf{DeriveDkAndOvk}^{\OrchardProto}}
...
\newcommand{\DeriveNoteCommitRandAlg}{\mathsf{Derive\_rcm}}
\newcommand{\DeriveNoteCommitRand}[2]{\DeriveNoteCommitRandAlg^{#1}_{#2}}
...
\newcommand{\ChainValuePoolBalance}[1]{\ensuremath{\mathsf{ChainValuePoolBalance}^{#1}}}
...
\newcommand{\vBalance}[1]{\mathsf{v}^{\text{#1\pooltype{Balance}}}}
\newcommand{\vBalanceOrchardLikePoolId}{\mathsf{v}^{\text{\notnusixthree{\OrchardPoolId}\nusixthree{\Pool Balance}}}}
...
\newcommand{\DeferredPoolRecipient}{\mathsf{DEFERRED\_POOL}}
...
\newcommand{\overwintertype}[1]{\type{#1}}
...
\newcommand{\gdStar}{\mathsf{g}\mathord{\star}_{\mathsf{d}}}
\newcommand{\pkdStar}{\mathsf{pk}\mathord{\star}_{\mathsf{d}}}
...
\newcommand{\nufiveprenusixthreeitem}[1]{\nufive{\item {[\NUFive onward\notbeforenusixthree{, pre-\NUSixThree}]}\, {#1}}}
...
\newcommand{\heartwoodonlyitem}[1]{\heartwood{\item {[\NUHeartwood only\notbeforecanopy{, pre-\NUCanopy}]}\, {#1}}}
\newcommand{\heartwoodonwarditem}[1]{\heartwood{\item {[\NUHeartwood onward]}\, {#1}}}
\newcommand{\heartwoodandcanopyitem}[1]{\heartwood{\item \heartwoodandcanopy{#1}}}
\newcommand{\preblossomitem}[1]{\item \preblossom{#1}}
\newcommand{\blossomonlyitem}[1]{\blossom{\item {[\NUBlossom only\notbeforeheartwood{, pre-\NUHeartwood}]}\, {#1}}}
\newcommand{\blossomonwarditem}[1]{\blossom{\item {[\NUBlossom onward]}\, {#1}}}
\newcommand{\presaplingitem}[1]{\item \presapling{#1}}
\newcommand{\saplingonlyitem}[1]{\sapling{\item {[\NUSapling only\notbeforeblossom{, pre-\NUBlossom}]}\, {#1}}}
\newcommand{\saplingonwarditem}[1]{\sapling{\item {[\NUSapling onward]}\, {#1}}}
...
\newcommand{\overwinteronlyitem}[1]{\overwinter{\item {[\NUOverwinter only, pre-\NUSapling]}\, {#1}}}
\newcommand{\overwinteronwarditem}[1]{\overwinter{\item {[\NUOverwinter onward]}\, {#1}}}
...
\newcommand{\precanopy}[1]{\notbeforecanopy{\canopy{[Pre-\NUCanopy\!]\,}} {#1}}
\newcommand{\canopyonly}[1]{\canopy{[\NUCanopy only\notbeforenufive{, pre-\NUFive}]\, {#1}}}
\newcommand{\canopyonward}[1]{\canopy{[\NUCanopy onward]\, {#1}}}
\newcommand{\preheartwood}[1]{\notbeforeheartwood{\heartwood{[Pre-\NUHeartwood\!]\,}} {#1}}
\newcommand{\heartwoodonward}[1]{\heartwood{[\NUHeartwood onward]\, {#1}}}
\newcommand{\heartwoodonly}[1]{\heartwood{[\NUHeartwood only\notbeforecanopy{, pre-\NUCanopy}]\, {#1}}}
\newcommand{\heartwoodandcanopy}[1]{\heartwood{[\NUHeartwood\notbeforecanopy{ and \NUCanopy}\notbeforenufive{ only, pre-\NUFive}]\, {#1}}}
\newcommand{\preblossom}[1]{\notbeforeblossom{\blossom{[Pre-\NUBlossom\!]\,}} {#1}}
\newcommand{\blossomonly}[1]{\blossom{[\NUBlossom only\notbeforeheartwood{, pre-\NUHeartwood}]\, {#1}}}
\newcommand{\blossomonward}[1]{\blossom{[\NUBlossom onward]\, {#1}}}
\newcommand{\presapling}[1]{\sapling{[Pre-\NUSapling\!]\,} {#1}}
\newcommand{\saplingonly}[1]{\sapling{[\NUSapling only\notbeforeblossom{, pre-\NUBlossom}]\, {#1}}}
\newcommand{\saplingonward}[1]{\sapling{[\NUSapling onward]\, {#1}}}
\newcommand{\saplingandblossom}[1]{\sapling{[\NUSapling\notbeforeblossom{ and \NUBlossom}\notbeforeheartwood{ only, pre-\NUHeartwood}]\, {#1}}}
\newcommand{\saplingprenufive}[1]{\sapling{[\NUSapling \notnufive{onward}\notbeforenufive{ to \NUCanopy inclusive, pre-\NUFive}]\, {#1}}}
\newcommand{\preoverwinter}[1]{\overwinter{[Pre-\NUOverwinter\!]\,} {#1}}
\newcommand{\overwinteronly}[1]{\overwinter{[\NUOverwinter only, pre-\NUSapling]\, {#1}}}
\newcommand{\overwinteronward}[1]{\overwinter{[\NUOverwinter onward]\, {#1}}}
\newcommand{\overwinterprenufive}[1]{\overwinter{[\NUOverwinter \notnufive{onward}\notbeforenufive{ to \NUCanopy inclusive, pre-\NUFive}]\, {#1}}}
...
\newcommand{\canopyonwardnnote}[1]{\canopy{\callout{[\NUCanopy onward]\,\,}{Non-normative note:}{#1}}}
\newcommand{\canopyonwardpnote}[1]{\canopy{\callout{[\NUCanopy onward]\,\,}{Note:}{#1}}}
\newcommand{\precanopypnote}[1]{\callout{\notbeforecanopy{\canopy{[Pre-\NUCanopy\!]\,\,}}}{Note:}{#1}}
\newcommand{\preheartwoodpnote}[1]{\callout{\notbeforeheartwood{\heartwood{[Pre-\NUHeartwood\!]\,\,}}}{Note:}{#1}}
\newcommand{\presaplingpnote}[1]{\callout{\sapling{[Pre-\ NUSapling\!]\,\,}}{Note:}{#1}}
\newcommand{\preoverwinterpnote}[1]{\callout{\overwinter{[Pre-\NUOverwinter\!]\,\,}}{Note:}{#1}}
\newcommand{\overwinteronlypnote}[1]{\callout{\overwinter{[\NUOverwinter only\sapling{, pre-\NUSapling}]\,\,}}{Note:}{#1}}
\newcommand{\overwinteronwardpnote}[1]{\callout{\overwinter{[\NUOverwinter onward]\,\,}}{Note:}{#1}}
...
\newcommand{\affiliation}[1]{\hairspace\thanks{\;#1}\;}
...
\Large \docversion}
\author{
\Large \leadauthor\affiliation{Zcash Open Development Lab} \\
\Large \coauthora\affiliation{Project Tachyon} — \coauthorb\affiliation{Independent consultant} — \coauthorc\affiliation{Shielded Labs}}
\date{\today}
\maketitle
\vspace{-6ex}
...
\defining{\Zcash} is an implementation of the \DecentralizedAnonymousPaymentScheme
...
\newcommand{\thisspecdefinesandproposes}[2]{This specification defines the \Zcash consensus protocol at launch; after each of the upgrades codenamed {#1}; and proposed changes for {#2}. }
\noindent \notblossom{\sapling{\thisspecdefines{\NUOverwinter and \NUSapling}}}%
\notheartwood{\blossom{\thisspecdefines{\NUOverwinter, \NUSapling, and \NUBlossom}}}%
\notcanopy{\heartwood{\thisspecdefines{\NUOverwinter, \NUSapling, \NUBlossom, and \NUHeartwood}}}%
...
\Zcash is an implementation of the \defining{\DecentralizedAnonymousPaymentScheme
...
Changes before the \NUSapling upgrade are not highlighted.

Changes specific to the \NUSapling upgrade following \NUOverwinter
are highlighted in \sapling{\saplingcolorname}.

\notbeforeblossom{Changes specific to the \NUBlossom upgrade following \NUSapling
are highlighted in \blossom{\blossomcolorname}.}

\notbeforeheartwood{Changes specific to the \NUHeartwood upgrade following \NUBlossom
are highlighted in \heartwood{\heartwoodcolorname}.}

\notbeforecanopy{Changes specific to the \NUCanopy upgrade following \NUHeartwood
are highlighted in \canopy{\canopycolorname}.}

\notbeforenufive{Changes specific to the \NUFive upgrade following \NUCanopy
are highlighted in \nufive{\nufivecolorname}.}
...
\notbeforenusixthree{Changes specific to the proposed \NUSixThree upgrade following \NUSixTwo
are highlighted in \nusixthree{\nusixthreecolorname}.}

All of these are also changes from \Zerocash.

The name ``Sprout'' refers to the \Zcash protocol prior to \NUSapling
(both before and after \NUOverwinter), and in particular its \SproutShieldedProtocol.
The name ``Sapling'' refers to the \NUSapling \networkUpgrade, and also the
\SaplingShieldedProtocol which is used for the \SaplingPool.
\nufive{The name ``Orchard'' refers to the \OrchardShieldedProtocol, used for the
\OrchardPool}\nusixthree{ and the \IronwoodPool}.
...
please file an issue at \url{https://github.com/zcash/zips/issues}, or if
security-relevant please submit a GitHub Security Advisory to the relevant
repository (see \url{https://github.com/zcash/zips/security/policy}).
...
\introsection
All value in \Zcash belongs to some \defining{\chainValuePool}. The current \chainValuePools
are the \defining{\TransparentPool, \SproutPool, \SaplingLikePoolList\nusix{, and
\DeferredPool}}. \nusixthree{The \IronwoodPool uses the same \shieldedProtocol
as the \OrchardPool.}
...
\spendingKey\sapling{ (or the \nullifierDerivingKey for \SaplingLikePoolAdj \notes)}.
...
\begin{itemize}
  \item \saplingonward{there is a revealed \valueCommitment to the same value as
        ...
        the \OrchardShieldedProtocol, each Action reveals a single \valueCommitment to
        the net value spent by the Action, rather than one \valueCommitment for the input
...
the \bitSequenceAdj constants $\Uncommitted{Sprout} \typecolon \bitseq{\smash{\MerkleHashLength{Sprout}}}$\sapling{ and
...
\saplingonward{
\notnufive{\Sapling provides}\notbeforenufive{\SaplingAndOrchard provide}
...
\introsection
\extralabel{poolsandnotes}{\lsubsection{Shielded Pools and Notes}{notes}}

A \defining{\note} is a representation of value held in a \defining{\shieldedPool}.

Each \shieldedPool operates according to the rules of a \shieldedProtocol, and has
its own \treestate (see \crossref{transactions}). The currently deployed \shieldedPools
are the \SproutPool\sapling{ and \SaplingLikePools}.

Each \note (denoted $\NoteTuple{}$) belongs to one of these \shieldedPools.
It represents that a value $\Value$ is spendable by the recipient who
holds the \spendingKey corresponding to a given \shieldedPaymentAddress.
The form of a \shieldedPaymentAddress is defined by a \shieldedProtocol;
it can be used to send funds represented by new \notes to the address, in
the \shieldedPoolps supported by that protocol.

\nusixthreeonward{The \OrchardShieldedProtocol supports both the \OrchardPool and the
\IronwoodPool. However, there are restrictions on sending to the \OrchardPool after
\NUSixThree, and so new payments to \OrchardProtocolAdj \shieldedPaymentAddresses will
go to the \IronwoodPool after that point \cite{ZIP-326}.}
...
\introlist
Let $\NoteType{Sprout}$ be the type of a \SproutPoolAdj \note, i.e.
\begin{formulae}
...
\introlist
Let $\NoteType{Sapling}$ be the type of a \SaplingPoolAdj \note, i.e.
\begin{formulae}
...
\introlist
Let $\NoteType{Orchard}$ be the type of an \OrchardLikePoolAdj \note, i.e.
\begin{formulae}
...
\introlist
Each \defining{\SaplingLikePoolAdj \notePlaintext} (denoted $\NotePlaintext{}$) consists of
...
\vspace{-1ex}
The field $\NotePlaintextLeadByte$ indicates the version of the encoding of this \notePlaintext.
The allowed $\notePlaintextLeadBytes$ differ based on the \shieldedPool and \blockHeight, as
described below.

Let the constants $\CanopyActivationHeight$ and $\ZIPTwoOneTwoGracePeriod$ be as defined in \crossref{constants}.

Let $\Pool \typecolon \setof{\SaplingLikePoolIds}$ be the \chainValuePool of the \note.
...
Define $\allowedLeadBytes{\Pool}(\BlockHeight, \TxVersion) := \notcanopy{\setof{\hexint{01}}}$\notcanopy{.}
\canopy{
\begin{formulae}
        \vspace{-1ex}
  \item $\begin{cases}
           ...
           \setof{\hexint{02}},               &\notnusixthree{\caseotherwise.}
                                               \nusixthree{\caseif \CanopyActivationHeight + \ZIPTwoOneTwoGracePeriod \leq \BlockHeight\text{ and }\Pool \neq \IronwoodPoolId} \\
\notbeforenusixthree{
           \nusixthree{\setof{\hexint{03}},}  &\nusixthree{\caseotherwise.}
}
         \end{cases}$
\end{formulae}
} %canopy

The $\NotePlaintextLeadByte$ of a \SaplingLikePoolAdj \notePlaintext \MUST satisfy the condition
$\NotePlaintextLeadByte \in$ $\allowedLeadBytes{\Pool}$ $(\BlockHeight, \TxVersion)$.
Senders \SHOULD choose the highest $\NotePlaintextLeadByte$ allowed under this condition.

\begin{nnotes}
\nufive{
  \item Since \Orchard was introduced after the end of the $\ZIPTwoOneTwoGracePeriod$,
        \notePlaintexts for \OrchardPoolAdj \notes \MUST have $\NotePlaintextLeadByte \geq \hexint{02}$.
} %nufive
\nusixthreeonwarditem{\xNotePlaintexts for \IronwoodPoolAdj \notes \MUST have $\NotePlaintextLeadByte \geq \hexint{03}$.}
  \item It is intentional that the definition of $\allowedLeadBytes{}$ does not
        currently depend on\notnusixthree{ \Pool{} or} $\TxVersion$. It might do so in future.
\end{nnotes}
...
\introlist
\vspace{1.5ex}
An \OrchardLikePoolAdj{} \defining{\noteCommitment} on a \note
...
Unlike in \Sapling, the definition of an \OrchardLikePoolAdj \note includes the
...
An \OrchardLikePoolAdj \noteCommitment is represented in an \actionDescription by the
$x$-coordinate of a \Pallas curve point, as specified in \crossref{actiondesc}.
} %nufive
...
The effects of each \shieldedProtocol on the \transparentTxValuePool are described in the
following sections:

\begin{itemize}
    \item \SproutPoolAdj \joinSplitTransfers in \crossref{sproutbalance};
\sapling{
    \item \SaplingPoolAdj \spendTransfers and \outputTransfers in \crossref{saplingbalance}\notnufive{;}\notbeforenufive{.}
} %sapling
\nufive{
    \item \OrchardLikePoolAdj \actionTransfers in \crossref{orchardbalance}.
} %nufive
\end{itemize}
\vspace{-2ex}

As in \Bitcoin, the remaining value in the \transparentTxValuePool of a non-coinbase \transaction
is available to miners as a \defining{\transactionFee}. That is, the sum of those values for non-coinbase \transactions
...
for the \SproutPool\sapling{ and \SaplingLikePools}. \sapling{Each} \treestate consists of:
...
An \defining{\anchor} is a Merkle tree root of one of these \noteCommitmentTrees. It
uniquely identifies a \noteCommitmentTree state given the assumed security properties of
the Merkle tree's \hashFunction. Since the \nullifierSet is always updated together with
the \noteCommitmentTree, this also identifies a particular state of the associated
\nullifierSet.

\introsection
In a given \blockChain, for \sapling{each of} the \SproutPool\sapling{ and \SaplingLikePools,}
...
\treestates for the \SproutPool, explained in the following section.
\sapling{There is no equivalent of interstitial \treestates for \TheSaplingLikePoolsNegative.}
...
\begin{consensusrules}
  \item For the first \joinSplitDescription of a \transaction, the \anchor \MUST
        be the output \SproutPoolAdj \treestate of a previous \block.
        \vspace{-0.5ex}
  \item The \anchor of each \joinSplitDescription in a \transaction \MUST refer
        to either some earlier \block's final \SproutPoolAdj \treestate, or to
...
\vspace{-1ex}
\joinSplitTransfers are not used for \SaplingPoolAdj \notes. \defining{Instead, there is a
...
A \spendDescription specifies an \anchor, which refers to the output \SaplingPoolAdj \treestate
...
\NUSixThree, there can be a \bindingSignature for \AllOrchardLikePools.}
This approach allows all of the \zkSNARK \statements to be
independent of each other, potentially increasing opportunities for precomputation.
...
$\anchorField{Orchard}$\nusixthree{ and/or $\anchorField{Ironwood}$} field of the
\transaction{}.
...
\begin{consensusrules}
  \item The \OrchardPoolAdj \actionTransfers of a \transaction \MUST be consistent
        with its $\vBalance{\OrchardPoolId}$ value as specified in \crossref{orchardbalance}.
  \item The $\anchorField{Orchard}$ field of the \transaction, whenever it exists
        (i.e.\ when there are any \OrchardPoolAdj \actionDescriptions), \MUST refer to some
        earlier \block's final \OrchardPoolAdj \treestate.
  \nusixthreeonwarditem{The \IronwoodPoolAdj \actionTransfers of a \transaction \MUST be consistent
        with its $\vBalance{\IronwoodPoolId}$ value as specified in \crossref{orchardbalance}.}
  \nusixthreeonwarditem{The $\anchorField{Ironwood}$ field of the \transaction, whenever it exists
        (i.e.\ when there are any \IronwoodPoolAdj \actionDescriptions), \MUST refer to some
        earlier \block's final \IronwoodPoolAdj \treestate.}
\end{consensusrules}
} %nufive
...
\defining{A \noteCommitmentTree is an \incrementalMerkleTree}, of fixed depth defined
by each \shieldedProtocol, used to store \noteCommitments that \joinSplitTransfers\sapling{ or
\spendTransfers}\nufive{ or \actionTransfers} produce. Just as the \defining{\utxoSetFull}
used in \Bitcoin, it is used to express the existence of value and the capability to spend it.
However, unlike the \utxoSet, it is \emph{not} the job of this tree to protect against
\doubleSpending, as it is append-only.

A \defining{\merkleRoot} of a \noteCommitmentTree is associated with the \treestate
(\crossref{transactions}) for each \shieldedPool.
...
Each \fullValidator maintains a \defining{\nullifierSet} logically associated with the
\treestate for each \shieldedPool. As valid \transactions containing \joinSplitTransfers\sapling{ or
\spendTransfers}\nufive{ or \actionTransfers} are processed, the \defining{\nullifiers}
revealed in \joinSplitDescriptions\sapling{ and \spendDescriptions}\nufive{ and\nusixthree{ (\OrchardLikePools)}
...
in a \validBlockChain. \sapling{\SproutPoolAdj and \SaplingLikePoolsAdj \nullifiers are
...
The production \Zcash{} \defining{\network}, which supports the \ZEC token, is called \defining{\Mainnet}.
Governance of its protocol is by social consensus on which \fullValidator implementations are
considered to be faithful implementations of the intended \Mainnet consensus rules, and on how those
implementations should be modified. Currently these implementations are \zebra maintained by the
...
(or planned version) of the \Zcash protocol.
...
\begin{poetry}
  ...
  \item \emph{\hphantom{`}purpose of abstracting is \strong{not} to be vague, but to create a new semantic level in which}
...
to derive the unique $\NoteUniqueRand$ value for a \SaplingPoolAdj \note. It is also used
...
\notnufive{
  \item \sapling{sending (\crossref{saplingsend}) and receiving (\shortcrossref{saplingandorchardinband}) \SaplingPoolAdj \notes,
...
\begin{itemize}
  \item \BCTV (\crossref{bctv}) is used with the
        ...
        (\crossref{joinsplitstatement}) before \NUSapling activation.
        \vspace{-0.5ex}
  \item \Groth (\crossref{groth}) is used with the
        ...
        and verify the \joinSplitStatement after \NUSapling activation.
        \vspace{-0.5ex}
...
appears in a \block before or after \NUSapling activation.
...
creation of many \diversifiedPaymentAddresses with the same \spendingAuthority.
...
\begin{itemize}
  ...
  \item $\ProofJoinSplit \typecolon \JoinSplitProof$ is a \zkProof with
        ...
        a \BCTV proof before \NUSapling activation, and a \Groth proof after \NUSapling
        activation)};
...
\begin{itemize}
  ...
  \item $\ProofSpend \typecolon \SpendProof$ is a \zkSNARKProof with \primaryInput
        $(\cv, \rt{\SaplingPoolId}\!, \nf, \AuthSignRandomizedPublic)$ for the \Sapling \spendStatement defined in
...
\begin{consensusrules}
  ...
  \item The proof $\Proof{\Spend}$ \MUST be valid given a \primaryInput formed
        ...
        i.e.\ $\SpendVerify\big(\kern-0.1em(\cv, \rt{\SaplingPoolId}, \nf, \AuthSignRandomizedPublic), \Proof{\Spend}\big) = 1$.
...
\begin{nnotes}
  \item As stated in \shortcrossref{concretehomomorphiccommit}, an implementation of $\HomomorphicPedersenCommit{Sapling}{}$
        ...
        \vspace{-0.25ex}
  \item The rule that $\cv$ and $\AuthSignRandomizedPublic$ \MUST not be \smallOrderAdj has the effect
...
\begin{nnotes}
  \item As stated in \shortcrossref{concretehomomorphiccommit}, an implementation of $\HomomorphicPedersenCommit{Sapling}{}$
        \MAY resample the \commitmentTrapdoor until the resulting commitment is not $\ZeroJ$.
        \vspace{-0.25ex}
  \item The rule that $\cv$ and $\EphemeralPublic$ \MUST not be \smallOrderAdj has the effect
        of also preventing \nonCanonicalFieldElement encodings of these fields\nufive{, as required by \cite{ZIP-216}}.
        That is, it is necessarily the case that $\reprJ\Of{\abstJ\Of{\cv}\kern0.05em} = \cv$ and
...
\vspace{0.5ex}
\introsection
An \actionDescription comprises $(\cvNet{}\kern-0.2em, \rt{\OrchardLikePoolId}\kern-0.2em, \nf, \AuthSignRandomizedPublic, \spendAuthSig,
\cmX\kern-0.1em, \EphemeralPublic, \TransmitCiphertext{}\kern-0.2em, \OutCiphertext\kern-0.2em, \enableSpends, \enableOutputs,$ $\Proof{})$ where:
\begin{itemize}
  \item $\cvNet{} \typecolon \ValueCommitOutput{Orchard}$ is the \valueCommitment to the value of the
        ...
        \vspace{-0.5ex}
  \item $\rt{\OrchardLikePoolId} \typecolon \MerkleHashOrchard$ is an \anchor, as defined in \crossref{transactions},
        for the output \treestate\nusixthree{ in \OrchardLikePoolId} of a previous \block;
        \vspace{-0.3ex}
...
\begin{nnotes}
        \vspace{-0.25ex}
  \item $\cv$ can be the zero point $\ZeroP$. $\AuthSignRandomizedPublic$ and $\EphemeralPublic$ cannot
        be $\ZeroP$.
        \vspace{-0.25ex}
  \item The restriction $\AuthSignRandomizedPublic \neq \ZeroP$ was enforced as a ``soft fork'', without
        a specific activation height, from \zcashdref{v6.12.1} and \zebraref{v4.3.1} \cite{ZIP-256}.
        It was introduced in response to a panic that $\rkField$ encoding the zero point caused during
        proof verification, in both \zcashd and \zebra. The bug occurred in conversion of the
        $\AuthSignRandomizedPublic$ \pallasCurve point to the corresponding circuit public inputs. The
        underlying circuit implementation did and does support $\AuthSignRandomizedPublic = \ZeroP$,
        represented with coordinates $(0, 0)$, and that case would have been harmless had it been handled
        correctly, but excluding it was the simpler and safer response under the circumstances.
        \vspace{-0.25ex}
...
\vspace{-1ex}
In order to send \SproutPoolAdj funds, the sender constructs a
...
\vspace{-1ex}
In order to send \SaplingPoolAdj \shielded value, the sender constructs a \transaction
...
\crossref{noteptconcept} with $\Pool = \SaplingPoolId$.
...
In order to send \OrchardLikePoolAdj \shielded value, the sender constructs a \transaction
...
\vspace{-0.25ex}
Let $\NotePlaintextLeadByte$ be the \notePlaintextLeadByte, chosen according to
\crossref{noteptconcept} with $\Pool \in \setof{\OrchardLikePoolIdList}$.

\nusixthree{
\vspace{0.5ex}
Define $\mathsf{H}^{\mathsf{rcm,Orchard}}_{\NoteSeedBytes}(\gdStar, \pkdStar, \Value, \NoteUniqueRandBytes, \NoteNullifierRand) = \ToScalar{Orchard}\big(\PRFexpand{\NoteSeedBytes}(\NoteCommitRandPre)\kern-0.1em\big)$, where
$\NoteCommitRandPre = [\hexint{0B}] \bconcat \LEBStoOSPOf{256}{\gdStar} \bconcat \LEBStoOSPOf{256}{\pkdStar} \bconcat \ItoLEOSPOf{64}{\Value} \bconcat \NoteUniqueRandBytes \bconcat \ItoLEOSPOf{256}{\NoteNullifierRand}$.

\vspace{-0.5ex}
Define $\DeriveNoteCommitRand{\OrchardProto}{\NoteSeedBytes}(\NotePlaintextLeadByte, \gdStar, \pkdStar, \Value, \NoteUniqueRandBytes, \NoteNullifierRand) = \begin{cases}
        \ToScalar{Orchard}\big(\PRFexpand{\NoteSeedBytes}([\hexint{05}] \bconcat \NoteUniqueRandBytes)\kern-0.1em\big), &\!\!\!\textsf{if } \NotePlaintextLeadByte = \hexint{02} \\
        \mathsf{H}^{\mathsf{rcm,Orchard}}_{\NoteSeedBytes}(\gdStar, \pkdStar, \Value, \NoteUniqueRandBytes, \NoteNullifierRand), &\!\!\!\textsf{if } \NotePlaintextLeadByte = \hexint{03}
\end{cases}$

\vspace{-0.5ex}
Define $\mathsf{H}^{\mathsf{esk,Orchard}}_{\NoteSeedBytes}(\NoteUniqueRandBytes) = \ToScalar{Orchard}\big(\PRFexpand{\NoteSeedBytes}([\hexint{04}] \bconcat \NoteUniqueRandBytes)\kern-0.1em\big)$.

\vspace{-0.5ex}
Define $\mathsf{H}^{\uppsi,\mathsf{Orchard}}_{\NoteSeedBytes}(\NoteUniqueRandBytes) = \ToBase{Orchard}\big(\PRFexpand{\NoteSeedBytes}([\hexint{09}] \bconcat \NoteUniqueRandBytes)\kern-0.09em\big)$.
} %nusixthree
...
\begin{algorithm}
  ...
  \nusixthree{\item Let $\gdStar = \reprP\Of{\DiversifiedTransmitBase}$ and $\pkdStar = \reprP\Of{\DiversifiedTransmitPublic}$.}
  \item Derive $\EphemeralPrivate = {}$\notnusixthree{$\ToScalar{Orchard}\big(\PRFexpand{\NoteSeedBytes}([4] \bconcat \NoteUniqueRandBytes)\kern-0.1em\big)$}\nusixthree{$\mathsf{H}^{\mathsf{esk,Orchard}}_{\NoteSeedBytes}(\NoteUniqueRandBytes)$}.
  \item If $\EphemeralPrivate = 0 \pmod{\ParamP{r}}$, repeat the above steps using a different $\NoteSeedBytes$.
  \item Derive $\NoteNullifierRand = {}$\notnusixthree{$\ToBase{Orchard}\big(\PRFexpand{\NoteSeedBytes}([9] \bconcat \NoteUniqueRandBytes)\kern-0.09em\big)$}\nusixthree{$\mathsf{H}^{\uppsi,\mathsf{Orchard}}_{\NoteSeedBytes}(\NoteUniqueRandBytes)$}.
  \item Derive $\NoteCommitRand = {}$\notnusixthree{$\ToScalar{Orchard}\big(\PRFexpand{\NoteSeedBytes}([5] \bconcat \NoteUniqueRandBytes)\kern-0.1em\big)$}\nusixthree{$\mathsf{Derive\_rcm}^{\mathsf{Orchard}}_{\NoteSeedBytes}(\NotePlaintextLeadByte, \gdStar, \pkdStar, \Value, \NoteUniqueRandBytes, \NoteNullifierRand)$}.
  ...
  \notnusixthree{\item Let $\cmX = \ExtractPbot\big(\NoteCommit{Orchard}{\NoteCommitRand}(\reprP\Of{\DiversifiedTransmitBase},
                                                                           \reprP\Of{\DiversifiedTransmitPublic},
                                                                           \Value, \NoteUniqueRand, \NoteNullifierRand)\kern-0.1em\big)$.}%
  \nusixthree{\item Let $\cmX = \ExtractPbot\big(\NoteCommit{Orchard}{\NoteCommitRand}(\gdStar, \pkdStar,
                                                                           \Value, \NoteUniqueRand, \NoteNullifierRand)\kern-0.1em\big)$.}
...
\introlist
\vspace{0.75ex}
A \spendDescription for a \dummy \SaplingPoolAdj input \note with \notePlaintextLeadByte
...
As in \Sprout, a \dummy \SaplingPoolAdj output \note is constructed as normal but with
zero value, and sent to a random \shieldedPaymentAddress.
} %sapling
...
\vspace{-0.25ex}
Let $\NotePlaintextLeadByte$ be the \notePlaintextLeadByte, chosen according to
\crossref{noteptconcept} with $\Pool = \OrchardPoolId$.

\nusixthree{\vspace{-0.25ex}
Let $\mathsf{Derive\_rcm}^{\mathsf{Orchard}}$ and $\mathsf{H}^{\uppsi,\mathsf{Orchard}}$ be as defined in \crossref{orchardsend}.\par}

\introlist
\vspace{0.5ex}
The spend-related fields of an \actionDescription for a \dummy \OrchardPoolAdj input \note are
...
\begin{itemize}
  ...
  \nusixthree{\item Let $\gdStar = \reprP\Of{\DiversifiedTransmitBase}$ and $\pkdStar = \reprP\Of{\DiversifiedTransmitPublic}$.}
  \item Derive $\NoteNullifierRand = {}$\notnusixthree{$\ToBase{Orchard}\big(\PRFexpand{\NoteSeedBytes}([9] \bconcat \NoteUniqueRandBytes)\kern-0.1em\big)$}\nusixthree{$\mathsf{H}^{\uppsi,\mathsf{Orchard}}_{\NoteSeedBytes}(\NoteUniqueRandBytes)$}.
  \item Derive $\NoteCommitRand = {}$\notnusixthree{$\ToScalar{Orchard}\big(\PRFexpand{\NoteSeedBytes}([5] \bconcat \NoteUniqueRandBytes)\kern-0.1em\big)$}\nusixthree{$\mathsf{Derive\_rcm}^{\mathsf{Orchard}}_{\NoteSeedBytes}(\NotePlaintextLeadByte, \gdStar, \pkdStar, \Value, \NoteUniqueRandBytes, \NoteNullifierRand)$}.
  \notnusixthree{\item Let $\cm = \NoteCommit{Orchard}{\NoteCommitRand}\big(\reprP\Of{\DiversifiedTransmitBase},
                                                             \reprP\Of{\DiversifiedTransmitPublic},
                                                             \Value, \NoteUniqueRand, \NoteNullifierRand\big)$.}%
  \nusixthree{\item Let $\cm = \NoteCommit{Orchard}{\NoteCommitRand}\big(\gdStar, \pkdStar,
                                                             \Value, \NoteUniqueRand, \NoteNullifierRand\big)$.}
...
As in \Sprout and \Sapling, a \dummy \OrchardPoolAdj output \note is constructed as normal but with
zero value, and sent to a random \shieldedPaymentAddress.
...
Let $\MerkleDepth{}$ be $\MerkleDepth{Sprout}$ for the \SproutPoolAdj \noteCommitmentTree\sapling{,
or $\MerkleDepth{Sapling}$ for the \SaplingPoolAdj \noteCommitmentTree}\nufive{,
or $\MerkleDepth{Orchard}$ for the \OrchardPoolAdj \noteCommitmentTree}.
...
The discussion below applies independently to the \SproutPoolAdj\sapling{ and \SaplingAndOrchardPoolsAdj}
\noteCommitmentTrees.
...
\begin{pnotes}
  \item For \Sapling, Merkle \merkleHashes are specified to be encoded as \bitSequences, but the
        \merkleRoot $\rt{\SaplingPoolId}$ is encoded for the \primaryInput of a \spendProof as an element
...
\overwinter{After \NUOverwinter activation, all \sighashTypes are also extended to cover \transaction fields
introduced in that upgrade\sapling{, and similarly after \NUSapling activation}\nufive{ and
...
as part of the \NUOverwinter upgrade.
} %overwinter
...
\begin{consensusrules}
  ...
  \heartwoodonlyitem{All \transactions \MUST use the \NUHeartwood \consensusBranchID \hexint{F5B9230B}
  ...
  \nusixthreeonlyitem{All \transactions \MUST use the \NUSixThree \consensusBranchID \hexint{37A5165B}
        as defined in \cite{ZIP-258}.}
\end{consensusrules}
...
$\vBalance{\SaplingPoolId}$.

$\vBalance{\SaplingPoolId}$ is encoded in a \transaction as the field \valueBalance{Sapling}.
For a v4 \transaction, $\vBalance{\SaplingPoolId}$ is always explicitly encoded.
\nufive{
For v5, $\vBalance{\SaplingPoolId}$ is implicitly zero if the \transaction has
no \spendDescriptions or \outputDescriptions.
} %nufive
Transaction fields are described in \crossref{txnencoding}.

A positive $\saplingBalancingValue$ takes net value from the \SaplingChainValuePool
and adds it to the \transparentTxValuePool. A negative $\saplingBalancingValue$ does the
reverse. As a result, positive $\vBalance{\SaplingPoolId}$ is treated like an \emph{input} to the
\transparentTxValuePool, whereas negative $\vBalance{\SaplingPoolId}$ is treated like an \emph{output}
from that pool.

\vspace{1ex}
\introlist
Consistency of $\vBalance{\SaplingPoolId}$ with the \valueCommitments in \spendDescriptions
...
\begin{itemize}
  \item To prove that the total value spent by \spendTransfers, minus that produced
        by \outputTransfers, is consistent with the $\vBalance{\SaplingPoolId}$ field
        of the \transaction;
        \vspace{-0.5ex}
...
\begin{itemize}
  ...
  \item \saplingBalancingValue $\vBalance{\SaplingPoolId}$.
\end{itemize}

\vspace{-1ex}
In a correctly constructed \transaction, $\vBalance{\SaplingPoolId} = \ssum{i=1}{n} \vOld{i} - \ssum{j=1}{m} \vNew{j}$,
...
% <https://twitter.com/hdevalence/status/984145085674676224> ¯\_(ツ)_/¯
  \item $\BindingPublic{Sapling} := \Bigg(\!\vcombsum{i=1}{n}\kern 0.2em \cvOld{i}\kern 0.05em\Bigg) \combminus\!
                                    ...
                                    \ValueCommit{Sapling}{0}\big(\vBalance{\SaplingPoolId}\big)$.
\end{formulae}
\vspace{-1ex}
...
\begin{tabular}{@{\hskip 2em}r@{\;}l}
  $\BindingPublic{Sapling}$ &$= \Biggscalarmult{\Bigg(\!\vgrpsum{i=1}{n} \vOld{i}\Bigg) \grpminus\!
                                                \Bigg(\!\vgrpsum{j=1}{m} \vNew{j}\Bigg) \grpminus \vBalance{\SaplingPoolId}}{\ValueCommitValueBase{Sapling}}\, \combplus
                            ...
                            &$= \ValueCommit{Sapling}{\BindingPrivate{Sapling}}\Bigg(\!\vsum{i=1}{n} \vOld{i} - \vsum{j=1}{m} \vNew{j} - \vBalance{\SaplingPoolId}\Bigg)$.
\end{tabular}

\introlist
Let $\vSum = \vsum{i=1}{n} \vOld{i} - \vsum{j=1}{m} \vNew{j} - \vBalance{\SaplingPoolId}$.
...
$\vBalance{\SaplingPoolId}$ is encoded in the \transaction as a signed two's complement $64$-bit integer
...
\SaplingPoolAdj \valueCommitment randomnesses, authorized a \transaction with the given \sighashTxHash
by signing $\SigHash$.

\vspace{1ex}
\pnote{
The spender \MAY reveal any strict subset of the \SaplingPoolAdj \valueCommitment randomnesses
...
and optionally perform an output. As for \Sapling, the net value of \OrchardPool
spends minus outputs in a \transaction is called the \defining{\orchardBalancingValue},
measured in \zatoshi as a signed integer $\vBalance{\OrchardPoolId}$.
\nusixthree{
Similarly, the net value of \IronwoodPool spends minus outputs in a \transaction
is called the \defining{\ironwoodBalancingValue}, measured in \zatoshi as a signed
integer $\vBalance{\IronwoodPoolId}$.

In the rest of this section, let $\Pool$ be $\OrchardPoolId$ or $\IronwoodPoolId$.
} %nusixthree
...
A positive $\orchardLikeBalancingValue$ takes net value from the \OrchardLikeChainValuePool
and adds it to the \transparentTxValuePool. A negative $\orchardLikeBalancingValue$ does the
reverse. As a result, positive $\vBalance{\OrchardLikePoolId}$ is treated like an \emph{input}
to the \transparentTxValuePool, whereas negative $\vBalance{\OrchardLikePoolId}$ is treated
like an \emph{output} from that pool.

\vspace{2ex}
Consistency of $\vBalance{\OrchardLikePoolId}$ with the \valueCommitments in
\actionDescriptions is enforced by the \defining{\orchardLikeBindingSignature}.
The rôle of this signature in the \Orchard protocol is to prove that the net value
spent (i.e.\ the total value spent minus the total value produced) by \actionTransfers
is consistent with the $\vBalance{\OrchardLikePoolId}$ field of the \transaction{}.
...
important in the \OrchardShieldedProtocol because all \actionDescriptions have a
\spendAuthSignature. Still, an \orchardLikeBindingSignature does prove that the signer
knew this commitment randomness; this provides defence in depth and reduces the differences
of the \OrchardShieldedProtocol from the \SaplingShieldedProtocol, which may simplify
security analysis.}

\vspace{1ex}
Instead of generating a key pair at random, we generate it as a function of the
\valueCommitments in the \actionDescriptions of the \transaction, and the
\orchardLikeBalancingValue.
...
Suppose that the \transaction has:
\begin{itemize}
  \item $n$ \actionDescriptions for $\Pool$ with \valueCommitments $\cvNet{\alln}$,
        committing to values $\vNet{\alln}$ with randomness $\ValueCommitRandNet{\alln}$;
  \item \orchardLikeBalancingValue $\vBalance{\OrchardLikePoolId}$.
\end{itemize}

\vspace{-1ex}
In a correctly constructed \transaction, $\vBalance{\OrchardLikePoolId} = \ssum{i=1}{n} \vNet{i}$,
but validators cannot check this directly because the values are hidden by the commitments.
...
\begin{formulae}
  \item $\BindingPublic{Orchard} := \Bigg(\!\vcombsum{i=1}{n}\kern 0.2em \cvNet{i}\kern 0.05em\Bigg) \combminus
                                    \ValueCommit{Orchard}{0}\big(\vBalance{\OrchardLikePoolId}\big)$.
\end{formulae}
\vspace{-1ex}
(This key is not encoded explicitly in the \transaction and must be recalculated.)
...
\vspace{0.5ex}
\begin{tabular}{@{\hskip 2em}r@{\;}l}
  $\BindingPublic{Orchard}$ &$= \Biggscalarmult{\Bigg(\!\vgrpsum{i=1}{n} \vNet{i}\Bigg) \grpminus \vBalance{\OrchardPoolId}}{\ValueCommitValueBase{Orchard}}\, \combplus
                                \Biggscalarmult{\vgrpsum{i=1}{n} \ValueCommitRandNet{i}}{\ValueCommitRandBase{Orchard}}$ \\[3.5ex]
                            &$= \ValueCommit{Orchard}{\BindingPrivate{Orchard}}\Bigg(\!\vsum{i=1}{n} \vNet{i} - \vBalance{\OrchardLikePoolId}\Bigg)$.
\end{tabular}

\introlist
Let $\vSum = \vsum{i=1}{n} \vNet{i} - \vBalance{\OrchardLikePoolId}$.
...
are in $\SignedValueDifferenceType$. $\vBalance{\OrchardLikePoolId}$ is encoded in the \transaction as a
...
\pnote{
The spender \MAY reveal any strict subset of the \OrchardLikePoolAdj \valueCommitment randomnesses to
...
The \defining{\TransparentChainValuePool} balance for a given \blockChain is the sum of
the values of all \utxos in the \utxoSetFull for that chain.
It is denoted by $\ChainValuePoolBalance{\TransparentPoolId}(\BlockHeight)$.

\vspace{-2ex}
\consensusrule{If the \TransparentChainValuePool balance would become negative in the \blockChain
created as a result of accepting a \block, then all nodes \MUST reject the block as invalid.}

\vspace{1.5ex}
As defined in \cite{ZIP-209}, the \defining{\SproutChainValuePool} balance for a given
...
It is denoted by $\ChainValuePoolBalance{\SproutPoolId}(\BlockHeight)$.

\vspace{-2ex}
\consensusrule{If the \SproutChainValuePool balance would become negative in the \blockChain
created as a result of accepting a \block, then all nodes \MUST reject the block as invalid.}

\sapling{
\vspace{1.5ex}
As defined in \cite{ZIP-209}, the \defining{\SaplingChainValuePool} balance for a given
\blockChain is the negation of the sum of all $\valueBalance{Sapling}$ values for
\transactions in the \chain.
It is denoted by $\ChainValuePoolBalance{\SaplingPoolId}(\BlockHeight)$.

\vspace{-2ex}
\consensusrule{If the \SaplingChainValuePool balance would become negative in the \blockChain
created as a result of accepting a \block, then all nodes \MUST reject the block as invalid.}
} %sapling

\nufive{
\vspace{1.5ex}
Similarly to the \SaplingChainValuePool balance defined in \cite{ZIP-209}, the
\defining{\OrchardChainValuePool} balance for a given \blockChain is the negation of the
sum of all $\valueBalance{Orchard}$ field values for \transactions in the \chain.
It is denoted by $\ChainValuePoolBalance{\OrchardPoolId}(\BlockHeight)$.

\vspace{-2ex}
\consensusrule{If the \OrchardChainValuePool balance would become negative in the \blockChain
created as a result of accepting a \block, then all nodes \MUST reject the block as invalid.}
} %nufive
...
Then, consistent with \cite{ZIP-207}, the \defining{\DeferredChainValuePool} balance
for a \blockChain up to and including height $\BlockHeight$ is given by:
$$
\ChainValuePoolBalance{\DeferredPoolId}(\BlockHeight) :=
...
\nusixone{
\vspace{-2ex}
\consensusrule{If the \DeferredChainValuePool balance would become negative in the \blockChain
created as a result of accepting a \block, then all nodes \MUST reject the block as invalid.}
...
\nusixthree{
\vspace{1.5ex}
The \defining{\IronwoodChainValuePool} balance for a given \blockChain is the negation of the
sum of all $\valueBalance{Ironwood}$ field values for \transactions in the \blockChain.
It is denoted by $\ChainValuePoolBalance{\IronwoodPoolId}(\BlockHeight)$.

\vspace{-2ex}
\consensusrule{If the \IronwoodChainValuePool balance would become negative in the \blockChain
created as a result of accepting a \block, then all nodes \MUST reject the block as invalid.}
} %nusixthree
...
$$
\begin{array}{rl}
  \IssuedSupply(\BlockHeight) \; := &\!\!\!             \ChainValuePoolBalance{\TransparentPoolId}(\BlockHeight) \\
                                    & +\,\;             \ChainValuePoolBalance{\SproutPoolId}(\BlockHeight) \\
                                    & \sapling{+\,\;    \ChainValuePoolBalance{\SaplingPoolId}(\BlockHeight)} \\
\notbeforenufive{
                                    & \nufive{+\,\;     \ChainValuePoolBalance{\OrchardPoolId}(\BlockHeight)} \\
}
\notbeforenusix{
                                    & \nusix{+\,\;      \ChainValuePoolBalance{\DeferredPoolId}(\BlockHeight)} \\
}
\notbeforenusixthree{
                                    & \nusixthree{+\,\; \ChainValuePoolBalance{\IronwoodPoolId}(\BlockHeight)}
}
\end{array}
$$

\vspace{-2ex}
\consensusrule{If the \totalIssuedSupply (computed without possibility of overflow) would become
greater than $\MAXMONEY$ \zatoshi in the \blockChain created as a result of accepting a \block, then
...
\vspace{-1ex}
\begin{formulae}
  \item $\oparen\rt{\SaplingPoolId} \typecolon \MerkleHash{Sapling},\\
...
as defined in \crossref{merklepath}, from $\cmU = \ExtractJ(\cmOld{})$ to the \anchor $\rt{\SaplingPoolId}$.
...
\vspace{-1ex}
\begin{formulae}
  \item $\oparen\rt{\OrchardPoolId} \typecolon \MerkleHashOrchard,\\
...
as defined in \crossref{merklepath}, from $\ExtractP(\cmOld{})$ to the \anchor $\rt{\OrchardPoolId}$.
...
\begin{pnotes}
  \item The \primaryInputs are encoded as the following sequence of type $\typeexp{\GF{\ParamP{q}}}{9}$: \\
        $\big[\,\rt{\OrchardPoolId} \pmod{\ParamP{q}}, \Selectx\Of{\cvNet{}}, \Selecty\Of{\cvNet{}},
  ...
  \item The case $\AuthSignRandomizedPublic = \ZeroP$ will not occur, due to the consensus rule in
        \crossref{actiondesc} that $\AuthSignRandomizedPublic$ \MUSTNOT be the identity point.
  ...
  \item The validity of $\DiversifiedTransmitBaseRepr$ and $\DiversifiedTransmitPublicRepr$ are
        \emph{not} checked in this circuit. Also, $\rt{\OrchardPoolId}$ and $\cmX$ are \emph{not} checked
...
\begin{nnotes}
  ...
  \item There is intentionally no equivalent to the \snarkref{Ephemeral public key integrity}{outputepkintegrity}
        check from the \Sapling \outputStatement. It is unnecessary for the sender of an \OrchardPoolAdj \note
        to prove knowledge of $\EphemeralPrivate$, because the potential attack this originally addressed
        for \Sapling is prevented by checks added at \NUCanopy activation in \cite{ZIP-212}. These checks are
...
further security considerations, for example of how to validate a \SproutPoolAdj \note
...
Unlike in \Sprout, each \SaplingLikePoolAdj output \note is encrypted by a fresh \ephemeralPublicKey.
...
\diversifiedTransmissionKey for the intended recipient address of a new \SaplingLikePoolAdj \note,
...
further security considerations, for example of how to validate a \SaplingLikePoolAdj \note
received \outOfBand, which are not addressed in this document.
} %pnote
} %sapling
...
\canopy{
Let \Pool{} and $\allowedLeadBytes{}$ be as defined in \crossref{noteptconcept}.
...
from $\TransmitPlaintext{}$
  \item if $\NotePlaintextLeadByte \not\in \allowedLeadBytes{\Pool}(\BlockHeight, \TxVersion)$, return $\bot$
\notbeforecanopy{
  \item \canopy{for $\Pool = \SaplingPoolId$, let $\NoteCommitRandPre = [4]$ and $\EphemeralPrivatePre = [5]$}
} %notbeforecanopy
\nufive{
  \item for $\Pool = \OrchardPoolId$, let $\NoteUniqueRandBytes = \ItoLEOSP{256}\big(\nfOld{}$ from the same \actionDescription$\kern-0.2em\big)\kern-0.08em$, $\NoteCommitRandPre = [5] \bconcat \NoteUniqueRandBytes$,
  ...
  \item let $\DiversifiedTransmitBase = \DiversifyHash{}(\Diversifier)$.
        if\notbeforenufive{ (for $\Pool = \SaplingPoolId$)} $\DiversifiedTransmitBase = \bot$, return $\bot$
...
\canopy{
  ...
  \item let $\DiversifiedTransmitPublic = \KADerivePublic{}(\InViewingKey, \DiversifiedTransmitBase)$
        \vspace{-0.25ex}
  \item \notbeforenufive{for $\Pool = \SaplingPoolId$,} let $\NoteTuple{} = (\Diversifier, \DiversifiedTransmitPublic, \Value, \NoteCommitRand)$
        \vspace{-0.25ex}
\nufive{
  \item for $\Pool = \OrchardPoolId$, let $\NoteTuple{} = (\Diversifier, \DiversifiedTransmitPublic, \Value, \NoteUniqueRand, \NoteNullifierRand, \NoteCommitRand)$
        where $\NoteNullifierRand = \ToBase{Orchard}\big(\PRFexpand{\NoteSeedBytes}([9] \bconcat \NoteUniqueRandBytes)\kern-0.1em\big)$
        \vspace{-0.2ex}
} %nufive
  \item let $\cmstar' = \NoteCommitment{}\Of{\NoteTuple{}}\,$. \nufive{if (for $\Pool = \OrchardPoolId$) $\cmstar' = \bot$, return $\bot$}
        \vspace{-0.2ex}
...
\begin{pnotes}
  ...
  \item For \Sapling, as explained in the note in \crossref{jubjub}, $\abstJ$ accepts \nonCanonicalPoint
        ...
        (if \NUCanopy is active and $\NotePlaintextLeadByte \neq \hexint{01}$) in the comparison against
...
\nufive{
  ...
  \item if $\NoteCommitRand \geq \ParamG{r}$, return $\bot$
        \vspace{-0.2ex}
  \item let $\DiversifiedTransmitBase = \DiversifyHash{}(\Diversifier)$.
        if\notbeforenufive{ (for $\Pool = \SaplingPoolId$)} $\DiversifiedTransmitBase = \bot$ or $\DiversifiedTransmitPublic \not\in \SubgroupJstar$ (see note below), return $\bot$
  \item \notbeforenufive{for $\Pool = \SaplingPoolId$,} let $\NoteTuple{} = (\Diversifier, \DiversifiedTransmitPublic, \Value, \NoteCommitRand)$
        \vspace{-0.2ex}
\nufive{
  \item for $\Pool = \OrchardPoolId$, let $\NoteTuple{} = (\Diversifier, \DiversifiedTransmitPublic, \Value, \NoteUniqueRand, \NoteNullifierRand, \NoteCommitRand)$
...
\begin{pnotes}
  ...
  \item A previous version of this specification did not have the requirement for the decoded point
        $\DiversifiedTransmitPublic$ of a \Sapling \note to be in the set of \primeOrderAdj points
  ...
  \item For \Sapling \outgoingCiphertexts, $\DiversifiedTransmitPublicRepr$ could also be \nonCanonicalPoint.
        ...
        $\DiversifiedTransmitPublic \not\in \smash{\SubgroupJstar}$, because only \smallOrderAdj \jubjubCurve
...
(\crossref{concretesinsemillacommit}), and for the \OrchardPoolAdj \incrementalMerkleTree
...
\begin{nnotes}
  ...
  \item The above theorem easily extends to the case where additional scalar multiplication terms with
        ...
        defined in \shortcrossref{rhoandnullifiers} against Faerie Gold attacks, as described in
        \crossref{faeriegold}.
        \vspace{-0.25ex}
...
\vspace{-2.5ex}
\nnote{The exclusion\canopy{, before \NUCanopy activation,} of $\PreCanopyExcludedPointEncodings$
...
\canopyonwardnnote{Because the post-\NUCanopy rules for \EdSpecific signatures are a relaxation
of the pre-\NUCanopy rules, a \fullValidator implementation that checkpoints on the \NUCanopy
\activationBlock \MAY validate using the post-\NUCanopy rules for the whole chain (and \zcashd
does so since \zcashdref{v4.2.0}). We retain the pre-\NUCanopy rules in the specification in order
...
\vspace{-1ex}
\lsubsubsubsection{Note Commitments (\SproutText{})}{concretesproutnotecommit}
...
\vspace{-1ex}
\Orchard uses the ``simplified SWU'' algorithm for \randomOracleAdj hashing to elliptic curves
...
\vspace{-1.5ex}
\sapling{Before \NUSapling activation,}
...
could have occurred before activation of the \NUSapling \networkUpgrade, although there
is no evidence of this having happened. Use of the vulnerability to produce false proofs
is believed to have been fully mitigated by activation of \NUSapling. The use of \BCTV
in \Zcash is now limited to verifying proofs that were made prior to the \NUSapling
\networkUpgrade.
...
\saplingonward{An implementation of \Zcash that checkpoints on a \block after \NUSapling
...
Since the most recent \settled \networkUpgrade is after the \NUSapling \networkUpgrade,
...
\vspace{-1ex}
After \NUSapling activation, \Zcash uses \zkSNARKs with the \defining{\Groth} \provingSystem described in
...
Since it is no longer possible (since activation of \cite{ZIP-211} in the \NUCanopy \networkUpgrade)
to send funds into the \SproutPool, this would not be generally useful.
...
\cite{BGG-mpc} and \cite{BGG2017}. \sapling{They are used only before \NUSapling
...
the \Sprout \joinSplitCircuit used after \NUSapling activation, are respectively:
...
\overwinter{A first upgrade, called \defining{\NUOverwinter}, activated on \Mainnet on 26~June, 2018
...
\sapling{A second upgrade, called \defining{\NUSapling}, activated on \Mainnet on 28~October, 2018
at \blockHeight $419200$ \cite{Hamdon2018}. Its specifications are
described in this document, \cite{ZIP-205}, and \cite{ZIP-243}.}

\blossom{A third upgrade, called \defining{\NUBlossom}, activated on \Mainnet on 11~December, 2019
at \blockHeight $653600$ \cite{Zcash-Blossom}. Its specifications are
described in this document, \cite{ZIP-206}, and \cite{ZIP-208}.}

\heartwood{A fourth upgrade, called \defining{\NUHeartwood}, activated on \Mainnet on 16~July, 2020
...
\canopy{A fifth upgrade, called \defining{\NUCanopy}, activated on \Mainnet on 18~November, 2020
...
\nusixthree{This specification describes the set of changes proposed for the \NUSixThree
\networkUpgrade, for which the \Mainnet activation \blockHeight has not yet been set.
Its specifications are described in this document, \cite{ZIP-258}, \cite{ZIP-229}, \cite{ZIP-2005},
and \cite{ZIP-2006}, with updates to \cite{ZIP-209} and \cite{ZIP-317}. Additional information
and rationale is given in \cite{ZODL-Ironwood}. Specifications relating to wallet support and
migration to the \IronwoodPool are in \cite{ZIP-318} and \cite{ZIP-326}.}
...
\NUSixOne, \NUSixTwo, and \NUSixThree), and for future upgrades.
...
minimum. See \cite{ZIP-201} for how this applies to the \NUOverwinter
upgrade, for example.
...
The net value of \SaplingPoolAdj spends minus outputs.\! \\ \hline
...
A \merkleRoot of the \SaplingPoolAdj \noteCommitmentTree at some \blockHeight in the past, $\LEBStoOSP{256}\big(\rt{\SaplingPoolId}\big)$.\! \\ \hline

& \Longunderstack{$192 \mult$ \\$\!\nSpendsSapling\!$} & $\vSpendProofsSapling$ & \type{byte[192]} \type{[$\nSpendsSapling$]} &
Encodings of the \zkSNARKProofs for each \SaplingPoolAdj \spendDescription.\! \\ \hline

& \Longunderstack{$64 \mult$ \\$\!\nSpendsSapling\!$} & $\vSpendAuthSigs{Sapling}$ & \type{byte[64]} \type{[$\nSpendsSapling$]} &
Authorizing signatures for each \SaplingPoolAdj \spendDescription.\! \\ \hline

& \Longunderstack{$192 \mult$ \\$\!\nOutputsSapling\!$} & $\vOutputProofsSapling$ & \type{byte[192]} \type{[$\nOutputsSapling$]} &
Encodings of the \zkSNARKProofs for each \SaplingPoolAdj \outputDescription.\! \\ \hline
...
The net value of \OrchardPoolAdj spends minus outputs.\! \\ \hline
...
is not present, then $\vBalance{\SaplingPoolId}$ is defined to be $0$. \\[-0.8ex]
...
\begin{consensusrules}
  ...
  \overwinteronlyitem{The \transactionVersionNumber \MUST be $3$, and the \versionGroupID \MUST
        be $\hexint{03C48270}$.}
  \saplingprenufiveitem{The \transactionVersionNumber{} \MUST be $4$.}
  \nufiveprenusixthreeitem{The \transactionVersionNumber \MUST be $4$ or $5$.}
  ...
  \saplingonwarditem{If $\effectiveVersion = 4$ and there are no \spendDescriptions or \outputDescriptions,
        then $\valueBalance{Sapling}$ \MUST be $0$.\footnote{\sapling{Prior to \zcashdref{v6.12.1}, \zcashd misimplemented
        the rule that requires $\valueBalance{Sapling} = 0$ for a v4 \transaction with no \spendDescriptions
        and no \outputDescriptions. It incorrectly normalized the value to $0$ when no \spendDescriptions or
  ...
  \nusixthreeonwarditem{If $\effectiveVersion \geq 6$ and $\nActionsIronwood > 0$,
        ...
        \begin{itemize}
          ...
          \item \bindingSig{Ironwood} \MUST represent a valid signature under the \txBindingValidatingKey
                $\BindingPublic{Ironwood}$ of $\SigHash$ ---
                i.e.\ $\BindingSigValidate{Ironwood}{\BindingPublic{Ironwood}}(\SigHash, \bindingSig{Ironwood}) = 1$.
                As specified in \crossref{concretereddsa}, validation of the $\RedDSAReprR{}$ component
                of the signature prohibits \nonCanonicalPoint encodings.
        \end{itemize}}
        \vspace{-1ex}
  \item \nusix{Let $\totalDeferredOutput$\nusixone{ and $\totalDeferredInput$} be as defined in \shortcrossref{subsidies}.}
        ...
        \begin{itemize}
          \item define the \totalOutputValue of its \coinbaseTransaction to be the total value
                in \zatoshi of its \transparentOutputs\heartwood{, minus $\vBalance{\SaplingPoolId}$}\nufive{,
                minus $\vBalance{\OrchardPoolId}$}\nusix{, plus $\totalDeferredOutput(\mathsf{height})$};
  ...
  \saplingonwarditem{\valueBalance{Sapling}, if it exists, \MUST be in the range $\range{-\MAXMONEY}{\MAXMONEY}$.}
  \nufiveonwarditem{\valueBalance{Orchard}, if it exists, \MUST be in the range $\range{-\MAXMONEY}{\MAXMONEY}$.}
  \nusixthreeonwarditem{\valueBalance{Ironwood}, if it exists, \MUST be in the range $\range{-\MAXMONEY}{\MAXMONEY}$.}
  \heartwoodonwarditem{All \SaplingLikePoolsAdj outputs in \coinbaseTransactions \MUST decrypt to a
        \notePlaintext, i.e.\ \crossref{decryptovk} does not return $\bot$, using
        a sequence of $32$ zero bytes as the \outgoingViewingKey. (This implies that\canopy{ before
        \NUCanopy activation,} \SaplingPoolAdj outputs of a \coinbaseTransaction \MUST have \notePlaintextLeadByte
        equal to $\hexint{01}$.)}
  \canopyonwarditem{Any \SaplingOrOrchardPoolAdj output of a \coinbaseTransaction decrypted to a \notePlaintext according
  ...
  \nusixthreeonwarditem{Any \IronwoodPoolAdj output of a \coinbaseTransaction decrypted to a \notePlaintext according
        to that rule \MUST have \notePlaintextLeadByte equal to $\hexint{03}$.}
...
\begin{pnotes}
  \item Previous versions of this specification defined what is now the \headerField{} field
        ...
        rule that the \fOverwintered{} flag \MUSTNOT be set before \NUOverwinter has activated,
        has the same effect.
  \item The semantics of \transactions with \versionNumber not any of
        $\{1, 2\overwinter{, 3}\sapling{, 4}\nufive{, 5}\nusixthree{, 6}\}$ is not
        currently defined.
  \item The exclusion of \transactions with \transactionVersionNumber
        \emph{greater than} $2$ is not a consensus rule before \NUOverwinter activation.
        Such \transactions may exist in the \blockChain and \MUST be treated
        identically to version 2 \transactions.
  \overwinteronwarditem{Once \NUOverwinter has activated, limits on the maximum
  ...
  \saplingonwarditem{Because \coinbaseTransactions have no
        ...
        \heartwood{The negative case can only occur after \NUHeartwood activation, for \transactions
        with \cite{ZIP-213} \shieldedOutputs.}}
\heartwood{
  \item Prior to the \NUHeartwood \networkUpgrade, it was not possible for \coinbaseTransactions
        to have \shieldedOutputs, and therefore the ``coinbase maturity'' rule and the requirement
...
A \merkleRoot $\rt{\SproutPoolId}$ of the \SproutPoolAdj \noteCommitmentTree at some \blockHeight in the past,
...
\saplingandblossom{The \merkleRoot $\LEBStoOSP{256}\big(\rt{\SaplingPoolId}\big)$ of the \SaplingPool
\noteCommitmentTree corresponding to the final \SaplingPoolAdj \treestate of this \block.\\}
...
\notheartwood{
  \saplingonwarditem{$\hashFinalSaplingRoot$ \MUST be $\LEBStoOSPOf{256}{\rt{\SaplingPoolId}}$ where
        $\rt{\SaplingPoolId}$ is the \merkleRoot of the \SaplingPoolAdj \noteCommitmentTree for the final
        \SaplingPoolAdj \treestate of the \block.\!\!}}
\notbeforeheartwood{
  \saplingandblossomitem{$\hashLightClientRoot$ \MUST be $\LEBStoOSP{256}\big(\rt{\SaplingPoolId}\big)$ where
        $\rt{\SaplingPoolId}$ is the \merkleRoot of the \SaplingPoolAdj \noteCommitmentTree for the final
        \SaplingPoolAdj \treestate of the \block.\!\!}
  \heartwoodandcanopyitem{$\hashLightClientRoot$ \MUST be set to the $\hashChainHistoryRoot$
        for the \block, as specified in \cite{ZIP-221}.}
  \nufiveonwarditem{$\hashBlockCommitments$ \MUST be set to the value of $\hashBlockCommitments$
        for the \block, as specified in \cite{ZIP-244}.}
}
...
\begin{pnotes}
  ...
  \item The rule limiting $\nTimeField$ to be no later than $90 \mult 60$ seconds after the
        ...
        \zcashdref{v2.1.1\mhyphen 1}. It had not been violated by any \block from the given \blockHeights
        in the consensus \blockChains of either \Mainnet or \Testnet.
\overwinter{
        \vspace{-0.5ex}
  \item There are no changes to the \blockVersionNumber or format for \NUOverwinter.
} %overwinter
\sapling{
        \vspace{-0.5ex}
  \item Although the \blockVersionNumber does not change for \NUSapling,
...
\blossom{
        \vspace{-0.5ex}
  \item There are no changes to the \blockVersionNumber or format for \NUBlossom.
} %blossom
\heartwood{
        \vspace{-0.5ex}
  \item For \NUHeartwood, the $\hashFinalSaplingRoot$ field is renamed to $\hashLightClientRoot$.
        Once \NUHeartwood activates, the meaning of this field changes according to \cite{ZIP-221}.
} %heartwood
\canopy{
        \vspace{-0.5ex}
  \item There are no changes to the \blockVersionNumber or format for \NUCanopy.
} %canopy
\nufive{
        \vspace{-0.5ex}
...
\nusix{
        \vspace{1ex}
  \item $\totalDeferredOutput(\BlockHeight) := \ssum{\fs\, \in\, \FundingStreams \;:\; \fsRecipient(\BlockHeight) \;=\; \DeferredPoolRecipient}{} \fsValue(\BlockHeight)$
} %nusix
\nusixone{
        \vspace{1ex}
...
\begin{pnotes}
  ...
  \item No \foundersReward is required to be paid for $\BlockHeight > \FoundersRewardLastBlockHeight$
        (i.e.\ after the first \halving), or for $\BlockHeight = 0$ (i.e.\ the \genesisBlock)\canopy{, or after \NUCanopy activation}.
        \vspace{-0.25ex}
...
\begin{nnotes}
  ...
  \blossomonwarditem{$\FoundersRewardLastBlockHeight = 1046399$.}
        \vspace{-0.25ex}
  \item \NUBlossom is not intended to change the total \foundersReward or the effective period over which
        it is paid.
\end{nnotes}
} %blossom
...
\begin{formulae}
  ...
  \item $\fsEndHeight \typecolon \Nat$
        \vspace{-1ex}
  \item $\fsRecipients \typecolon \typeexp{\big(\byteseqs\nusix{ \,\union\, \setof{\DeferredPoolRecipient}}\big)}{\PosInt}$.
\end{formulae}
...
or the identifier $\DeferredPoolRecipient$}.
...
\begin{itemize}
  \item for each \fundingStream $\fs$ active at that \blockHeight with a recipient
        identifier\nusix{ other than $\DeferredPoolRecipient$} given by $\fsRecipient(\BlockHeight)$,
...
\begin{itemize}
  ...
  \item The \defining{\prescribedWay} to pay a \SaplingOrOrchard \paymentAddress is
        defined in \cite{ZIP-213}, using the post-\NUHeartwood consensus rules specified
        for the \SaplingAndOrchardPoolsAdj \shieldedOutputs of \coinbaseTransactions in
...
\begin{pnotes}
  ...
  \item Up to and including \NUSixOne there have been no \fundingStreams or
        ...
        \SaplingAndOrchard \shieldedPaymentAddresses as recipients, as permitted by
        \cite{ZIP-213} and \cite{ZIP-207}.
\end{pnotes}
...
\begin{pnotes}
  \item The \blockHeights of \halvings are different between \Testnet and \Mainnet, as a
        result of different \activationHeights for the \NUBlossom \networkUpgrade (which
        changed the \blockTargetSpacing). The end height of these \fundingStreams
        corresponds to the second \halving on each \network.
  \item On \Testnet, the \activationHeight of \NUCanopy is before the first \halving.
...
dependent on the position of the \noteCommitment in the \SaplingPool
...
Note that it is possible for two distinct \SaplingPoolAdj \positionedNotes (having
...
that \commitmentScheme ensures that \OrchardPoolAdj \nullifiers will be unique. (Specifically,
...
commitment: \shaHash for \SproutPoolAdj \notes\sapling{,\notnufive{ and} $\PedersenHashToPoint$
for \SaplingPoolAdj \notes}\nufive{, and $\SinsemillaHashToPoint$ for \OrchardPoolAdj \notes}.
...
\vspace{-1ex}
\sproutspecificpnote{
\SproutPoolAdj \noteCommitments are not statistically \hiding, so for \SproutPoolAdj \notes,
...
is supported for \SaplingAndOrchardPoolsAdj \notes under the same conditions as in
\Zerocash (by the protocol, not necessarily by \zcashd). Note that
...
\begin{itemize}
    \item The paper defines a \note as $((\AuthPublic, \TransmitPublic), \Value,
          ...
          specification defines a \SproutPoolAdj \note as
...
\xShieldedPool names are set in Raleway Italic, by Matt McInerney, Pablo Impallari,
...
\begin{itemize}
    \item Apply the retrospective changes described in \cite{ZIP-256} in response to
          vulnerabilities found after \NUSixOne:
          \vspace{0.5ex}
          \begin{itemize}
          \nufive{
              \item Add a consensus rule in \crossref{actiondesc} that $\AuthSignRandomizedPublic$
                    \MUSTNOT be the identity point, retrospectively enforced as a soft fork.
              \item Add a non-normative note in \crossref{actionencodingandconsensus} on the invalid
                    $\ephemeralKey$ conformance issue.
          } %nufive
          \sapling{
              \item Add a footnote in \crossref{txnconsensus} clarifying that $\valueBalance{Sapling}$ for
                    a v4 \transaction refers to the value as encoded, not the normalized $\vBalance{\SaplingPoolId}$.
          } %sapling
              \item In \crossref{chainvaluepoolbalances}, add consensus rules that the
                    \TransparentChainValuePool balance \MUSTNOT become negative and that the
                    \totalIssuedSupply \MUSTNOT exceed $\MAXMONEY$ \zatoshi.
          \end{itemize}
          \vspace{-1.5ex}
\nufive{
    \item Correct a typo in the balance argument in \crossref{orchardbalance} for \Orchard:
          it should use the \Pallas group order $\ParamP{r}$, not the \Jubjub order
          $\ParamJ{r}$.
    \item Factor out $\DiversifierKey$ and $\OutViewingKey$ derivation into $\DeriveDkAndOvkOrchard$,
          so that \cite{ZIP-32} can use it without duplication.
} %nufive
    \item Consistently use \discreteLogarithm rather than ``discrete log''.
    \item Expand \crossref{notes}, retitled ``Shielded Pools and Notes'', to describe the
          relation between \shieldedPaymentAddresses, \shieldedPools, and \notes.
    \item Distinguish \networkUpgrades, \shieldedProtocols, and \shieldedPools
          typographically, like this: \upgradetype{Upgrade}, \subprotocoltype{Protocol},
          \pooltype{Pool}.
    \item Drop the blue highlighting for \NUOverwinter and the brown for TODOs, so that
          those colours can be reused.
    \item Update author affiliations.
    \item Update how to report security-relevant issues, suggesting to use a
...
\nufive{
    ...
    \item So that \SaplingAndOrchard are consistent, \crossref{concreteorchardkeyagreement}
          now also defines the type $\KAPublicPrimeOrder{Orchard}$, which is used instead of
...
\heartwood{
    \item In the table of \crossref{blockheader}, clarify that $\hashLightClientRoot$ is used in \NUHeartwood
          and \NUCanopy, but not in \NUFive or later.
} %heartwood
\sapling{
    ...
    \item In the discussion of partitioning oracle attacks on \note encryption in \crossref{inbandrationale},
          ...
          $\DiversifiedTransmitBase$ is not a \smallOrderAdj point, not that it is in the \primeOrderSubgroup.
          It is true that honestly generated addresses have \primeOrderAdj $\DiversifiedTransmitBase$
    ...
    \item Delete a confusing claim in \crossref{spenddesc} that ``The check that $\AuthSignRandomizedPublic$
          ...
          The \smallOrderAdj check excludes the zero point $\ZeroJ$, which the \snarkref{Spend authority}{spendauthority}
...
\canopy{
    \item Document in \crossref{concreteed25519} that a \fullValidator implementation
          that checkpoints on the \NUCanopy \activationBlock \MAY validate \EdSpecific
          signatures using the post-\NUCanopy rules for the whole chain.
} %canopy
...
\begin{itemize}
\nufive{
    \item Set the \NUFive \activationHeight for \Testnet and \Mainnet.
...
\nufive{
    ...
    \item Change the type of $\rt{\OrchardPoolId}$ from $\GroupP_{\!x}$ (i.e.\ a \Pallas $x$-coordinate
          or $0$) to $\range{0}{\ParamP{q}-1}$.
          This reflects the existing \zcashd implementation; also checking
          $\rt{\OrchardPoolId} \in \GroupP_{\!x}$ would require a square root and is unnecessary.
...
\heartwood{
    \item Add a clarification in \crossref{txnconsensus} that after \NUHeartwood and before
          \NUCanopy activation, \SaplingPoolAdj outputs of a \coinbaseTransaction \MUST have
...
\begin{itemize}
\nufive{
    \item Change one of the \sapling{[\NUSapling onward]} consensus rules in \crossref{txnconsensus}
          to have the correct applicability: \sapling{[\NUSapling to \NUCanopy inclusive, \nufive{pre-\NUFive}]}.
} %nufive
...
\nufive{
    ...
    \item Add a step to the algorithm for generating an \OrchardPoolAdj \note in \crossref{orchardsend},
...
\sapling{
    \item Add a note to \crossref{merklepath} clarifying the encoding of $\rt{\SaplingPoolId}$
...
\nufive{
    ...
    \item Clarify that a \dummyNote should be created if no real \OrchardPoolAdj \note is being
    ...
    \item Correct some interim findings of the NCC specification audit:
          \begin{itemize}
            ...
            \item Propagate $\bot$ from the inputs of $\MerkleCRH{Orchard}$ to its output, and
                  add an explicit consensus rule that $\rt{\OrchardPoolId}$ computed from appending a
...
\nufive{
    ...
    \item The $\NoteUniqueRand$ value for an \OrchardPoolAdj output \note was incorrectly described as
...
\sapling{
    ...
    \item The specification of decryption in \shortcrossref{decryptovk} differed from the \zcashd
          ...
          \end{itemize}
    \item Correct the procedure for generating \dummy \SaplingPoolAdj \notes in \crossref{saplingdummynotes}.
    \item Add a note in \crossref{bctv} describing conditions under which an implementation
          that checkpoints on \NUSapling can omit verifying \BCTV proofs.
...
\begin{itemize}
    ...
    \item Clarify the encoding of \blockHeights for the ``height in coinbase'' rule.
          ...
          \crossref{txnconsensus}.
    \item Include the activation dates of \NUHeartwood and \NUCanopy in \crossref{networkupgrades}.
    \item Section links in the \NUHeartwood and \NUCanopy versions of the specification now go to
...
\begin{itemize}
    ...
    \item Define \ChainValuePoolBalance{\SproutPoolId}\sapling{ and \ChainValuePoolBalance{\SaplingPoolId},} and include
          consensus rules from \cite{ZIP-209}.
\sapling{
    \item Correct the \SaplingPoolAdj \note decryption algorithms:
          \begin{itemize}
...
\sapling{
    \item Fix a type error in the output of $\PRFnf{Sapling}{}$; a \SaplingPoolAdj \nullifier is a
...
\blossom{
    ...
    \item Make the \NUCanopy specification the default.
\end{itemize}
...
\begin{itemize}
    ...
    \item Add an explicit lead byte field to \SproutPoolAdj \notePlaintexts, and clearly specify
          the error handling when it is invalid.
\sapling{
    \item Define a \SaplingPoolAdj \notePlaintextLeadByte as having type $\byte$ (so that decoding
          to a \notePlaintext always succeeds, and error handling is more explicit).
...
\canopy{
    \item Incorporate \EdSpecific changes for \NUCanopy from \cite{ZIP-215}.
...
\begin{itemize}
    ...
    \item Network Upgrade 4 has been given the name \NUCanopy.
\canopy{
    \item Reference \cite{ZIP-211}, \cite{ZIP-212}, and \cite{ZIP-215} for the \NUCanopy upgrade.
} %canopy
...
\canopy{
    \item Reference \cite{ZIP-251}, \cite{ZIP-207}, and \cite{ZIP-214} for the \NUCanopy upgrade.
} %canopy
\end{itemize}
...
\heartwood{
    \item Add consensus rules for \NUHeartwood.
} %heartwood
    \item Remove ``pvc'' \Makefile targets.
    \item Make the \NUHeartwood specification the default.
    \item Add macros and \Makefile support for building the \NUCanopy specification.
\end{itemize}
...
\begin{itemize}
    \item Specify a retrospective soft fork implemented in \zcashdref{v2.1.1\mhyphen 1} that
...
\blossom{
    \item Specify the height at which \NUBlossom activated.
    \item Add \NUBlossom to \crossref{networkupgrades}.
    \item Add a non-normative note giving the explicit value of $\FoundersRewardLastBlockHeight$.
    \item Clarify the effect of \NUBlossom on \sighashTxHashes.
} %blossom
    \item \Makefile updates for \NUHeartwood.
\end{itemize}
...
\begin{itemize}
    \item No changes to \Sprout or \Sapling.
\blossom{
    \item Replace dummy \NUBlossom \activationHeight with the \Testnet height, and a reference to \cite{ZIP-206}.
}
\end{itemize}
...
\sapling{
    ...
    \item Make the \NUBlossom spec the default.
\end{itemize}
...
\begin{itemize}
    ...
    \item Correct an inconsistency in difficulty adjustment between the spec and
          ...
          in their \NUBlossom specification audit.
\blossom{
...
\sapling{
    \item Clarify which transaction fields are added by \NUOverwinter and \NUSapling.
    ...
    \item Add macros and \Makefile support for building the \NUBlossom specification.
\end{itemize}
...
\sapling{
    \item Rename \zkSNARK Parameters sections to be named according to the \provingSystem
          (\BCTV or \Groth), not the \shieldedProtocol construction (\Sprout or \Sapling).
    \item In \crossref{networkupgrades}, say when \NUSapling activated.
} %sapling
\end{itemize}
...
\begin{itemize}
    \item Disclose a security vulnerability in \BCTV that affected \Sprout
          before activation of the \NUSapling \networkUpgrade (see \crossref{bctv}).
...
\sapling{
    ...
    \item \Makefile changes: name the PDF file for the \Sprout version of the specification as \texttt{sprout.pdf},
          and make \texttt{protocol.pdf} link to the \NUSapling version.
\end{itemize}
...
\sapling{
    \item Add a missing consensus rule for version 4 \transactions: if there are
          no \SaplingPoolAdj Spends or Outputs, then $\valueBalance{Sapling}$ \MUST be $0$.
} %sapling
\end{itemize}
...
\begin{itemize}
\overwinter{
    \item Update \crossref{networkupgrades} to take account that \NUOverwinter
          has activated.
    \item The recommendation for \transactions without \joinSplitDescriptions to be
          version 1 applies only before \NUOverwinter, not before \NUSapling.
} %overwinter
\sapling{
    ...
    \item Clarify the use of $\cvNew{}$ and $\cmNew{}$, and the selection of
          \outgoingViewingKey, in sending \SaplingPoolAdj \notes.
...
\sapling{
    ...
    \item Add consensus rules that $\cv$ in a \spendDescription, and $\cv$ and $\EphemeralPublic$ in an
          ...
          when encrypting \SaplingPoolAdj \notes.
    ...
    \item Correct the conformance rule for \fOverwintered{} (it must not be set before \NUOverwinter has
          activated, not before \NUSapling has activated).
    ...
    \item Change the light yellow background to white (indicating that this \NUOverwinter and \NUSapling
...
\sapling{
    ...
    \item Allow dummy \SaplingPoolAdj input \notes.
...
\begin{itemize}
\overwinter{
    \item Add references to \NUOverwinter ZIPs and update the section on
          \NUOverwinter/\NUSapling transitions.
}
\sapling{
...
\begin{itemize}
    ...
    \item Add Kexin Hu to acknowledgements\sapling{ (for the idea of explicitly
          encoding the root of the \NUSapling \noteCommitmentTree in \blockHeaders)}.
    \item Move bit/byte/integer conversion primitives into \crossref{endian}.
\sapling{
    \item Refer to \NUOverwinter and \NUSapling just as ``upgrades'' in the abstract, not as
...
\begin{itemize}
    \item Specify the coinbase maturity rule, and the rule that \coinbaseTransactions
          ...
          \outputDescriptions}.
\overwinter{
    \item Delay lifting the 100000-byte \transaction size limit from \NUOverwinter to
          \NUSapling.
}
\sapling{
...
\sapling{
    ...
    \item Add instantiation of a hash extractor (later renamed to \coordinateExtractor)
          for \Jubjub.
    \item Make the background lighter and the \NUSapling green darker, for contrast.
}
\end{itemize}
...
\overwinter{
    \item Refer to Network Upgrade 0 as \NUOverwinter.
}
\end{itemize}
...
on arbitrary \bitSequenceAdj inputs, an attempt by an adversarial prover to use a
...
\begin{formulae}
  \item $\big[1, \Selectu(\AuthSignRandomizedPublic), \Selectv(\AuthSignRandomizedPublic),
              \Selectu(\cvOld{}), \Selectv(\cvOld{}), \LEBStoIP{\MerkleHashLength{Sapling}}\big(\rt{\SaplingPoolId}\big),
...
\hhline{|=|=|=|=|}
  ...
  $\AuthSignPublic$ is not \smallOrderAdj
  ...
  $\DiversifiedTransmitBase$ is on the curve
      ...
      & 4     & \shortcrossref{cctedvalidate} \\ \hline
  $\DiversifiedTransmitBase$ is not \smallOrderAdj
      & \snarkref{Small order checks}{spendnonsmall}
      & 16    & \shortcrossref{cctednonsmallorder} \\ \hline
...
%      = \WindowedPedersenCommit{\NoteCommitRand}(\vOldRepr \bconcat \DiversifiedTransmitBaseRepr \bconcat \DiversifiedTransmitPublicRepr)
  ...
  if $\vOld{} \neq 0$ then ${\rt{}}' = \rt{\SaplingPoolId}$
      &
      & 1     & \shortcrossref{cctcondeq} \\ \cline{1-1}\cline{3-4}
  inputize $\rt{\SaplingPoolId}$
      &
      & ?     & \\ \hline
...
faster validation of a batch of signatures, i.e.\ to determine whether all signatures in a batch are valid.
The correctness of this procedure is dependent on the \EdSpecific validation changes made for the \NUCanopy
