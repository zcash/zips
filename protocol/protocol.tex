...
\sapling{and $\MerkleCRH{Sapling} \typecolon$ $\MerkleLayer{Sapling} \times \MerkleHash{Sapling} \times \MerkleHash{Sapling}
...
\begin{itemize}
  ...
  \saplingonwarditem{one called $\BindingSig{}$. A \saplingBindingSignature is used to
        ...
        $\BindingSig{}$ is instantiated\orchard{ for both \SaplingAndOrchard} in
...
\begin{nnotes}
  ...
  \item $\GroupJHash{}$ is also used to instantiate $\DiversifyHash{Sapling}$ in \crossref{concretediversifyhash}.
        ...
        it is implied by the Decisional Diffie-Hellman assumption on $\SubgroupJ$\orchard{,
  ...
  \item $\URS$ is a \defining{\uniformRandomString}; we chose it verifiably at random
        ...
        been backdoored. \orchard{For \Orchard, we considered a URS to be unnecessary,
...
\begin{itemize}
  ...
  \orchardonwarditem{\HaloTwo (\crossref{halo2}) is used with the
...
\begin{itemize}
  ...
  \orchardonwarditem{$\Action$ for the \Orchard \actionStatement.}
\end{itemize}
...
We\notorchard{ also} omit subscripts on $\SpendProve$,
...
taking them to be the relevant \HaloTwo \provingKeys and
\verifyingKeys defined in \crossref{halo2parameters}.
} %orchard
} %sapling
...
\begin{itemize}
  ...
  \item $\spendAuthSig \typecolon \SpendAuthSigSignature{Sapling}$, specified in \crossref{spendauthsig}.
\end{itemize}
...
\orchard{
Let $\reprPstar$ and $\ParamP{r}$ be as defined in \crossref{pallasandvesta}.
...
\begin{algorithm}
  \item Check that $\DiversifiedTransmitPublic$ is of the correct type. For $\Sapling$ this type
        ...
        \swCurve point other than $\ZeroP$ on the \pallasCurve (as defined in \crossref{pallasandvesta}).}
...
\canopy{
  ...
  \item Calculate
        ...
        \begin{tabular}{@{\hskip 2em}r@{\;}l}
          $\cv$ &$:= \ValueCommit{}{\ValueCommitRand}(\Value)$ \\
...
\orchard{
  ...
  \item Return $(\cv, \cm, \EphemeralPublic, \TransmitCiphertext{}, \OutCiphertext, \ProofOutput\orchard{\text{ or }\ProofAction})$.
\end{algorithm}
