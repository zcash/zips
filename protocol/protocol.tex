...
\pnote{\Orchard is a feature proposal, not a \networkUpgrade in itself.}}
...
\sprout{
It also can include a sequence of zero or more \joinSplitDescriptions.
...
\outputDescriptions}\orchard{ and \actionDescriptions}. Together these describe
...
has its own description.}\orchard{ For \Orchard, each \actionDescription
...
\begin{formulae}
  \item \begin{flushleft}
    ...
    $\InViewingKeyLength{Sapling}$, $\OutViewingKeyLength$, $\ScalarLength{Sapling}$,\orchard{ $\ScalarLength{Orchard}$,}}
...
\setorchard $\MerkleCRH{Orchard}$ &\setorchard $\typecolon\, \MerkleLayer{Orchard}$ &\setorchard $\times\; \MerkleHash{Orchard}$ &\setorchard $\times\; \MerkleHash{Orchard}$ &\setorchard $\rightarrow \MerkleHash{Orchard}$.
\end{tabular}

$\MerkleCRH{Sprout}$ is \collisionResistant except on its first argument.
\sapling{$\MerkleCRH{Sapling}$\notorchard{ is}\orchard{ and $\MerkleCRH{Orchard}$ are}
\collisionResistant on all\notorchard{ its}\orchard{ their} arguments.}
...
\begin{tabular}{@{\hskip 2em}l@{\;}l@{\;}l@{\;}l@{\,}l}
$\PRFock{Orchard}{} $&$\typecolon\; \OutViewingKeyType $&$\times\; \ReprJBytes \times \ReprJBytes \times \ReprJBytes $& &$\rightarrow \Keyspace$ \\
$\PRFnf{Orchard}{}  $&$\typecolon\; \NullifierKeyType  $&$\times\; \NoteUniqueRandTypeOrchard $& &$\rightarrow \PRFOutputNfOrchard$
\end{tabular}
%} %orchard
...
\begin{itemize}
  ...
  \orchardonwarditem{in \crossref{orchardkeycomponents}, with inputs $[6]$, $[7]$, and $[8]$;}
  \item in the processes of sending and receiving \SaplingOrOrchard \notes
        (see \crossref{saplingandorchardsend} and \crossref{saplingandorchardinband}), with inputs $[4]$ and $[5]$;
...
$\PRFock{Sapling}{}$\notorchard{ is}\orchard{ and $\PRFock{Orchard}{}$ are} used in \crossref{inband}.
...
\begin{securityrequirements}
  ...
  \item In addition to being \pseudoRandomFunctions, it is required that
        $\PRFaddr{x}$,\changed{ $\PRFrho{x}$,\sprout{ and} $\PRFnf{Sprout}{x}$}\sapling{,\notorchard{ and}
        $\PRFnf{Sapling}{x}$}\orchard{ and $\PRFnf{Orchard}{x}$} be \collisionResistant across all $x$ ---
        ...
        $\PRFnf{Sapling}{}$}\orchard{ and $\PRFnf{Orchard}{}$}}.
...
\begin{tabular}{@{\hskip 1.5em}r@{\;}l}
  $\NoteCommit{Sprout}{}  $&$\typecolon\; \NoteCommitTrapdoor{Sprout} \times \PRFOutputSprout
     ...
     \notsprout{\\[-1ex]\hphantom{$\NoteCommitAlg{Sapling}$} &\hspace{26.9em}}$\rightarrow \NoteCommitOutput{Sprout}$,
\end{tabular}
...
\begin{tabular}{@{\hskip 1.5em}r@{\;}l@{\;}l}
  ...
  $\CommitIvkAlg            $&$\typecolon\; \CommitIvkTrapdoor \times \GF{\ParamP{q}} \times \NullifierKeyType $&$\rightarrow \CommitIvkOutput$
\end{tabular}
...
\vspace{-3ex}
\nnote{Ideally, we would also have that for all $S$ not in the image of $\reprG{}$, $\abstG{}(S) = \bot$.
...
\begin{nnotes}
  ...
  \item $\GroupJHash{}$ is also used in \crossref{concretediversifyhash} to instantiate $\DiversifyHash{Sapling}$.
...
Let $\CommitIvk{}$, instantiated in \crossref{concretecommitivk}, be a \commitmentScheme.
...
Let $\reprP$, $\GroupP$, $\GroupPstar$, and $\ReprP$ be as defined in \crossref{pallasandvesta}.
and let $\GroupPHash$ be as defined in \crossref{concretegrouphashpallasandvesta}.
...
\nullifierRandomness $\CommitIvkRand \typecolon \CommitIvkRandType$ are derived as follows:

\vspace{-0.5ex}
\begin{tabular}{@{\hskip 1.7em}r@{\;}l}
  $\AuthSignPrivate$ &$:= \truncate{\ScalarLength{Orchard}}(\PRFexpand{\SpendingKey}([6]))$ \\
  $\NullifierKey$ &$:= \truncate{\ScalarLength{Orchard}}(\PRFexpand{\SpendingKey}([7]))$ \\
  $\OutViewingKey$ &$:= \truncate{(\OutViewingKeyLength/8)}(\PRFexpand{\SpendingKey}([8]))$
\end{tabular}

If $\AuthSignPrivate = 0$, discard this key and repeat with a new $\SpendingKey$.

\vspace{1ex}
$\AuthSignPublic \typecolon \GroupP$, the \incomingViewingKey
$\InViewingKey \typecolon \InViewingKeyType{Orchard}$, and the \outgoingViewingKey
$\OutViewingKey \typecolon \OutViewingKeyType{Orchard}$ are then derived as:

\vspace{-0.5ex}
\begin{tabular}{@{\hskip 1.7em}r@{\;}l}
  $\AuthSignPublic$ &$:= \SpendAuthSigDerivePublic{Orchard}(\AuthSignPrivate)$ \\
  $\InViewingKey$ &$:= \CommitIvk{\CommitIvkRand}\big(\reprP\Of{\ExtractP{\AuthSignPublic}}, \reprP\Of{\NullifierKey}\kern-0.08em\big)$ \\
  $\OutViewingKey$ &$:= \CRHovk\big(\CommitIvkRand, \reprP\Of{\ExtractP{\AuthSignPublic}}, \reprP\Of{\NullifierKey}\kern-0.08em\big)$.
\end{tabular}
...
\begin{nnotes}
  \item Assume that $\PRFexpand{}$ is a \xPRF with output range $\PRFOutputExpand$, where
        ...
        is computationally indistinguishable from that of $\SpendAuthSigGenPrivate{Sapling}()$ defined
        in \crossref{concretespendauthsig}.
  \item Since todo{}, the distribution of $\NullifierKey$ will be computationally
        indistinguishable from uniform on $\GF{\ParamP{q}}$ (which is the keyspace of $\PRFnf{Orchard}{}$).
  \item The \zcashd wallet picks \diversifiers as in \cite{ZIP-32}, rather than using the default
        \diversifier specified above.
\end{nnotes}
} %orchard
...
\nnote{$\cv$ and $\AuthSignRandomizedPublic$ can be the zero point $\ZeroP$.}
} %orchard
...
\sapling{
\introlist
\extralabel{saplingsend}{\lsubsubsection{Sending Notes (\SaplingAndOrchardText)}{saplingandorchardsend}}
...
scope of this specification. The encoded \transaction is submitted to the peer-to-peer network.
} %sapling
...
\orchardonward{The \sighashAlgorithm used after activation of the \networkUpgrade that
...
\vspace{2ex}
Let $\PRFnf{Sprout}{}$\sapling{ and $\PRFnf{Sapling}{}$}\orchard{ and $\PRFnf{Orchard}{}$}
...
\orchard{
For an \Orchard \note, the \nullifier is derived as
$\PRFnf{Orchard}{\NullifierKeyRepr}(\NoteUniqueRandRepr)$, where $\NullifierKeyRepr$
is a representation of the \nullifierDerivingKey associated with the \note and
$\NoteUniqueRandRepr = \reprP(\NoteUniqueRand)$.
} %orchard
...
$\cmX = \ExtractP\big(\NoteCommit{Orchard}{\NoteCommitRandNew{}}(\DiversifiedTransmitBaseNewRepr,
                                                                 ...
                                                                 \vNew{})
                                                                 \NoteUniqueRand,
                                                                 \NoteNullifierRand)\kern-0.12em\big)$,
...
\begin{itemize}
  ...
  \item let $\ExtractJ$ be as defined in \crossref{concreteextractorjubjub};
        \vspace{-0.5ex}
  \item let $\PRFock{Sapling}{}$ and $\PRFock{Orchard}{}$ be as instantiated in \crossref{concreteprfs}.
\end{itemize}
} %sapling
...
\vspace{-0.5ex}
  ...
  \item \tab let $\cmuField = \LEBStoOSP{256}\big(\ExtractJ(\cm)\kern-0.12em\big)$
        \vspace{-0.5ex}
  \item \tab let $\OutCipherKey = \PRFock{Sapling}{\OutViewingKey}(\cvField, \cmuField, \ephemeralKey)$
        \vspace{0.5ex}
...
in \crossref{saplingkeycomponents}\orchard{ or \crossref{orchardkeycomponents}}, that is to
be used for decryption.
(If $\OutViewingKey = \bot$ was used for encryption, the payment is not decryptable by
this method.)

Let $\GroupG{}$ be $\GroupJ$ for a \Sapling \noteCiphertextSapling, or $\GroupP$ for an
\Orchard \noteCiphertextOrchard; and similarly for $\ReprG{}$.
...
Let $\PRFock{}{}$ be $\PRFock{Sapling}{}$ or $\PRFock{Orchard}{}$.
...
\vspace{-0.25ex}
  ...
  \orchardonwarditem{if $\reprP\big(\DiversifiedTransmitPublic\big) \neq \DiversifiedTransmitPublicRepr$, return $\bot$}
  \item let $\DHSecret{} = \KAAgree{Sapling}(\EphemeralPrivate, \DiversifiedTransmitPublic)$
  \item let $\TransmitKey{} = \KDF{Sapling}(\DHSecret{}, \ephemeralKey)$
  \item let $\TransmitPlaintext{} = \SymDecrypt{\TransmitKey{}}(\TransmitCiphertext{})$
\vspace{-0.25ex}
...
\vspace{-0.25ex}
  ...
  \item let $\cmU' = \ExtractG\big(\NoteCommit{Sapling}{\NoteCommitRand}(\reprG{}\Of{\DiversifiedTransmitBase},
                                                                         \reprG{}\Of{\DiversifiedTransmitPublic},
                                                                         \Value)\kern-0.12em\big)$
  \item if $\LEBStoOSPOf{256}{\cmU'} \neq \cmuField$, return $\bot$
  \item if $\reprG{}\big(\KADerivePublic{Sapling}(\EphemeralPrivate, \DiversifiedTransmitBase)\kern-0.12em\big) \neq \ephemeralKey$,
        return $\bot$
  \item return $\NotePlaintext{}$.
\end{algorithm}
...
\begin{pnotes}
  \item As explained in the note in \crossref{jubjub}, $\abstJ$ accepts \nonCanonicalPoint compressed encodings
        ...
        $\reprJ\big(\KADerivePublic{Sapling}(\EphemeralPrivate, \DiversifiedTransmitBase)\kern-0.12em\big)$.
        \vspace{-0.5ex}
  \preorcharditem{$\DiversifiedTransmitPublicRepr$ can also be \nonCanonicalPoint. Since $\bot$ is returned
  ...
  \orchardonwarditem{This procedure returns $\bot$ if $\DiversifiedTransmitPublicRepr$ is \nonCanonicalPoint,
  ...
  \item A previous version of this specification did not have the requirement for the decoded point
        $\DiversifiedTransmitPublic$ to be in the subgroup $\SubgroupJ$. That did not match the implementation
        in \zcashd, which does require $\DiversifiedTransmitPublic$ to be in the subgroup. The specification
        has been changed to match \zcashd.
...
$\tilde{x} = x \bmod 2$.\notsprout{\footnotewithlabel{coordinatenames}{\changed{Here we use the $(x, y)$ naming of coordinates in
...
\vspace{-1ex}
Let $\ItoLEBSP{}$ be as defined in \crossref{endian}; let $\MerkleHashLength{Orchard}$ be as defined
in \crossref{constants}, and let $\GroupP$ be as defined in \crossref{pallasandvesta}.

\vspace{1ex}
Let $\Selectx\big(\kern-0.1em(x, y)\kern-0.1em\big) = x$ and let $\Selectx\big(\ZeroP\big) = 0$.

Define $\ExtractP \typecolon \GroupP \rightarrow \MerkleHash{Orchard}$ by
\begin{formulae}
  \item $\ExtractP(P) := \ItoLEBSP{\MerkleHashLength{Orchard}}\big(\Selectx(P)\kern-0.12em\big)$.
\end{formulae}
} %orchard
...
$\PRFnf{Sapling}{\NullifierKey}(\NoteUniqueRandRepr)$}\orchard{ or
... $\PRFnf{Orchard}{\NullifierKey}(\NoteUniqueRandRepr)$ ...},
this is only possible if the adversary finds a collision across both
inputs on $\PRFnf{Sprout}{}$\sapling{ or $\PRFnf{Sapling}{}$}\orchard{ or
$\PRFnf{Orchard}{}$}, which is assumed to be infeasible --- see
...
\canopy{
    ...
    \item The definition of an abstraction function in \crossref{abstractgroup} incorrectly
          ...
          of $\reprG{}$. \notsprout{While this was originally intended, it is not true of $\abstJ$.
