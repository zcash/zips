...
% begin paste from <https://tex.stackexchange.com/a/60212/78411>
...
\newcommand{\RepresentedPairings}{\titleterm{Represented Pairings}}
...
\newcommand{\SproutJoinSplitParameters}{\titleterm{\Sprout JoinSplit Parameters}}
\newcommand{\SaplingJoinSplitParameters}{\titleterm{\Sapling JoinSplit Parameters}}
...
\newcommand{\ZKJoinSplit}[1]{\mathsf{ZK}^{#1}_{\JoinSplit}}
\newcommand{\ZKJoinSplitVerify}[1]{\ZKJoinSplit{#1}\mathsf{.Verify}}
\newcommand{\ZKJoinSplitProve}[1]{\ZKJoinSplit{#1}\mathsf{.Prove}}
\newcommand{\ZKJoinSplitProof}[1]{\ZKJoinSplit{#1}\mathsf{.Proof}}
\newcommand{\Proof}[1]{\pi_{\!{#1}}}
\newcommand{\JoinSplitProof}[1]{\Proof{\JoinSplit}^{#1}}
...
\newcommand{\AtInfP}[1]{\AtInf_{\mathbb{P}_{#1}}}
...
\newcommand{\AtInfG}[1]{\AtInf_{\mathbb{G}_{#1}}}
...
\newcommand{\AtInfS}[1]{\AtInf_{\mathbb{S}_{#1}}}
...
\begin{itemize}
  ...
  \item \Sapling Transition | the strategy for upgrading from \Sprout to \Sapling;
...
part of the normative protocol specification. \sapling{This overview applies
...
\introlist
\nsubsubsection{Represented Pairing}

A \representedPairing $\GroupP{}$ consists of:

\begin{itemize}
  \item a group order parameter $\ParamP{r} \typecolon \Nat$ which must be prime;
  \item for each $i \typecolon \setof{1,2}$,

        \begin{itemize}
          \item a group $\GroupP{i}$ of order $\ParamP{r}$, written additively
                with operation $+ \typecolon \GroupP{i} \times \GroupP{i} \rightarrow \GroupP{i}$,
                additive identity $\AtInfP{i}$, and generator $\GenP{i}$;
          \item a bit-length parameter $\ellP{i} \typecolon \Nat$;
          \item a representation function $\repr_{\GroupP{i}} \typecolon
                  \GroupP{i} \rightarrow \bitseq{\ellP{i}}$;
          \item an abstraction function $\abst_{\GroupP{i}} \typecolon
                  \bitseq{\ellP{i}} \rightarrow \GroupP{i}$;
        \end{itemize}
        \vspace{-2ex}
        such that $\abst_{\GroupP{i}}$ is the left inverse of $\repr_{\GroupP{i}}$, i.e.
        for all $P \in \GroupP{i}$, $\abst_{\GroupP{i}}(\repr_{\GroupP{i}}(P)) = P$.
  ...
  \item a pairing function
        $\ParamP{\hat{e}} \typecolon \GroupP{1} \times \GroupP{2} \rightarrow \GroupP{T}$
        satisfying:

        \begin{itemize}
          \item (Bilinearity)\; for all $a, b \typecolon \GFstar{r}$,
                $P \typecolon \GroupP{1}$, and $Q \typecolon \GroupP{2}$,
                $\ParamP{\hat{e}}(a \mult P, b \mult Q) = \ParamP{\hat{e}}(P, Q)^{a \mult b}$, and
          \item (Nondegeneracy)\; there does not exist $P \typecolon \GroupP{1} \setminus \AtInfP{1}$
                such that for all $Q \typecolon \GroupP{2},
                \ParamP{\hat{e}}(P, Q) = \ParamP{\mathbf{1}}$;
        \end{itemize}
\end{itemize}
...
$\ZKJoinSplit{}$ refers to this \provingSystem with the $\BNCurve$ pairing,
specialized to the \joinSplitStatement given in \crossref{sproutstatement}.
In this case we omit the key subscripts on $\ZKJoinSplitProve{}$ and $\ZKJoinSplitVerify{}$,
...
\begin{itemize}
  ...
  \item $\Groth$ (\crossref{groth}) is used with the
        ...
        to prove and verify the \Sapling \joinSplitStatement
        (\crossref{saplingstatement}).
\end{itemize}

These specializations are referred to as
$\ZKJoinSplit{\Sprout}$ and $\ZKJoinSplit{\Sapling}$ respectively.

We omit the key subscripts on $\ZKJoinSplitProve{\Sprout}$ and
$\ZKJoinSplitVerify{\Sprout}$, taking them to be the \Sprout \provingKey
and \verifyingKey defined in \crossref{sproutparameters}.

Similarly, we omit the key subscripts on $\ZKJoinSplitProve{\Sapling}$
and $\ZKJoinSplitVerify{\Sapling}$, taking them to be the \Sapling \provingKey
...
\h{\allOld}, \JoinSplitProof{\Sprout}, \TransmitCiphertext{\allNew})$
...
\begin{itemize}
  ...
  \item $\JoinSplitProof{\Sprout} \typecolon \ZKJoinSplitProof{\Sprout}$ is
...
A valid instance of $\JoinSplitProof{\Sprout}$ assures that given a \term{primary input}:
...
A valid instance of $\JoinSplitProof{\Sapling}$ assures that given a \term{primary input}:
...
\nsubsubsection{\RepresentedPairings} \label{concretepairings}
...
Let $\ParamG{\hat{e}}$ be the optimized ate pairing of type
$\GroupG{1} \times \GroupG{2} \rightarrow \GroupG{T}$.

For $i \typecolon \range{1}{2}$, let $\AtInfG{i}$ be the point at infinity in $\GroupG{i}$,
and let $\GroupGstar{i} = \GroupG{i} \setminus \setof{\AtInfG{i}}$.
...
\begin{itemize}
  \item The use of big-endian byte order is different from the encoding
        ...
        $\GroupGstar{2}$.
  \item The points at infinity $\AtInfG{1, 2}$ never occur in proofs and
  ...
  \item A point $P \typecolon (\GF{\ParamGexp{q}{2}})^2 = (\xP, \yP)$ known to satisfy
        the $\CurveG{2}$ curve equation $\yP^2$ = $\xP^3 + \frac{\ParamG{b}}{\xi}$ can be
        verified to be of order $\ParamG{r}$, and therefore in $\GroupGstar{2}$,
        by checking that $\hfrac{\#\CurveG{2}}{r} \mult P \neq \AtInfG{2}$.
\end{itemize}
...
$t^2 + 1$; in this representation, $i$ is given by $?$.

Let $\GroupS{T}$ be the subgroup of $\ParamSexp{r}{\mathrm{th}}$ roots of unity in
$\GFstar{\ParamSexp{q}{12}}$.

Let $\ParamS{\hat{e}}$ be the optimized ate pairing of type
$\GroupS{1} \times \GroupS{2} \rightarrow \GroupS{T}$.

For $i \typecolon \range{1}{2}$, let $\AtInfS{i}$ be the point at infinity in $\GroupS{i}$,
and let $\GroupSstar{i} = \GroupS{i} \setminus \setof{\AtInfS{i}}$.
...
\begin{itemize}
  ...
  \item The points at infinity $\AtInfS{1, 2}$ never occur in proofs and
        have no defined encodings in this protocol.
  \item Algorithms for decompressing points from the encodings of
        ...
        for $\GroupGstar{1}$.
  \item A point $P \typecolon (\GF{\ParamSexp{q}{2}})^2 = (\xP, \yP)$ known to
        satisfy the $\CurveS{2}$ curve equation can be verified to be
        of order $\ParamS{r}$, and therefore in $\GroupSstar{2}$, by checking that
        $\hfrac{\#\CurveS{2}}{\ParamS{r}} \mult P \neq \AtInfS{2}$.
\end{itemize}
...
\sapling{and \crossref{saplingparameters}}), and a \provingSystem implementation
that is interoperable with
\sprout{the \Zcash fork of \libsnark}
\sapling{the \bellman library used by \Zcash},
...
described in \cite{groth}.

A proof consists of a tuple
$(\Proof{A} \typecolon \GroupSstar{1},\;
  \Proof{B} \typecolon \GroupSstar{2},\;
  \Proof{C} \typecolon \GroupSstar{1})$.
It is computed using the parameters above as described in \cite{groth}.
...
\vspace{0.8ex}
\introlist
In addition to the steps to verify a proof given in \cite{groth}, the
verifier \MUST check, for the encoding of each element, that:
...
\introlist
\nsubsection{\SproutJoinSplitParameters} \label{sproutparameters}
...
\sapling{
\introsection
\nsubsection{\SaplingJoinSplitParameters} \label{saplingparameters}

The $\FullHashName$ hashes of the \provingKey and \verifyingKey for the \Sapling
\joinSplitStatement, encoded in \bellman format, are:
}
\begin{verbatim}
    xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  sapling-proving.key
    xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  sapling-verifying.key
\end{verbatim}
\sapling{
...
Thus, there are two independent mechanisms preventing \transaction
replay:
\begin{itemize}
  \item ...
  \item ...
...
Similarly, there are two independent mechanisms preventing a
``wipe-out'' situation:
\begin{itemize}
  \item ...
  \item ...
...
\Sapling-supporting nodes \MUST support this.
...
$\JoinSplitProof{\Sprout}$ (see \crossref{phgr}). \\ \hline
