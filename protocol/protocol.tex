...
are the \defining{\TransparentPool, \SproutPool, \sapling{\SaplingPool{}}\nufive{, \OrchardPool{}}\nusix{, and
\DeferredPool}}.
...
\spendingKey\sapling{ (or the \nullifierDerivingKey for \SaplingOrOrchardPoolAdj \notes)}.
...
are the \SproutPool\sapling{ and \SaplingAndOrchardPools}.
...
\introlist
Let $\NoteType{Orchard}$ be the type of an \OrchardPoolAdj \note, i.e.
\begin{formulae}
...
\introlist
Each \defining{\SaplingOrOrchardPoolAdj \notePlaintext} (denoted $\NotePlaintext{}$) consists of
...
Unlike in \Sapling, the definition of an \OrchardPoolAdj \note includes the
...
for the \SproutPool\sapling{ and \SaplingAndOrchardPools}. \sapling{Each} \treestate consists of:
...
\introsection
In a given \blockChain, for \sapling{each of} the \SproutPool\sapling{ and \SaplingAndOrchardPools,}
...
in a \validBlockChain. \sapling{\SproutPoolAdj and \SaplingAndOrchardPoolsAdj \nullifiers are
...
Unlike in \Sprout, each \SaplingOrOrchardPoolAdj output \note is encrypted by a fresh \ephemeralPublicKey.
...
\diversifiedTransmissionKey for the intended recipient address of a new \SaplingOrOrchardPoolAdj \note,
...
further security considerations, for example of how to validate a \SaplingOrOrchardPoolAdj \note
received \outOfBand, which are not addressed in this document.
} %pnote
} %sapling
...
\begin{pnotes}
  ...
  \item Normally only \noteCiphertextsSapling of \transactions in \blocks need to be decrypted. In that case,
        ...
        To test whether a \SaplingOrOrchardPoolAdj \note is unspent in a particular \blockChain also requires
...
\begin{consensusrules}
  ...
  \nufiveonwarditem{\valueBalance{Orchard} \MUST be in the range $\range{-\MAXMONEY}{\MAXMONEY}$ for version
        5 \transactions.}
  \heartwoodonwarditem{All \SaplingAndOrchardPoolsAdj outputs in \coinbaseTransactions \MUST decrypt to a
