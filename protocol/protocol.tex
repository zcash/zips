...
\newcommand{\PRFaddrSapling}[1]{\PRF{#1}{addrSapling}}
...
them to be the appropriate $\Groth$ \provingKeys and
...
\begin{consensusrules}
  ...
  \item If $\nJoinSplit > 0$, then \joinSplitPubKey{} \MUST represent a point of order $\ell$
        on the Ed25519 curve, in the encoding specified by \cite{BDL+2012}. Note that it is
...
\introlist
For reference, the incomplete affine-Montgomery addition formulae given in
...
Affine-Edwards addition formulae are given in \cite{BBJLP2008}.
