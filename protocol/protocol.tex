...
\begin{nnotes}
  ...
  \item There is intentionally no equivalent to the \snarkref{Ephemeral public key integrity}{outputepkintegrity}
        ...
        required after the end of the ZIP 212 grace period, which precedes \NUFive activation.
  \item If $\NoteCommitAlg{Orchard}$ returns $\bot$ for the old or new \note, then the corresponding
        ...
        finding a nontrivial discrete logarithm relation. If the Discrete Logarithm Problem is hard on the
...
$\InViewingKeyTypeOrchard$ (in \Orchard)}} be the recipient's \incomingViewingKey, as in
...
The \commitmentScheme $\NoteCommitAlg{Sapling}$ specified in \crossref{abstractcommit} is
...
\cite{ZIP-252}, \cite{ZIP-216}, \cite{ZIP-221}, \cite{ZIP-224}, \cite{ZIP-225}, \cite{ZIP-244},
and \cite{ZIP-316}, with updates to \cite{ZIP-32} and \cite{ZIP-221}.
...
\diversifiedPaymentAddresses can be linked if the Decisional Diffie--Hellman Problem
...
\nufive{
    ...
    \item Caveat how the result of \cite{GG2015} applies to analysis of $\PRFnf{Orchard}{}$ in
          \crossref{concreteprfs}.
    \item Unlinkability of \diversifiedAddresses depends on the Decisional Diffie--Hellman Problem,
          not the Discrete Logarithm Problem.
