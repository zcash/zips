...
\begin{bytefield}[bitwidth=0.045em]{320}
    \bitbox{256}{256 bit $\SharedKey{}$} &
    \bitbox{120}{64 bit $\vOld{i}$}
\end{bytefield}
\end{lrbox}
...
where $\DisclosePlaintext{i} = \Justthebox{\disclosebox}{-1.3ex}$.
}
...
implies that for the $\TransmitCiphertext{}$ and $\SharedCiphertext$
...
encryption of plaintext $\Plaintext$ with empty ``associated data", all-zero nonce,
and key $\Key$.

Similarly, let $\SymDecrypt{\Key}(\Ciphertext)$ be the $\SymSpecific$
decryption of ciphertext $\Ciphertext$ with empty ``associated data", all-zero
...
\SymEncrypt{\TransmitKey{i}}(\TransmitPlaintext{i})$.
  ...
  \item For $i$ in $\{1..\NOld\}$,
    \begin{itemize}
      \item Let $\DisclosePlaintext{i} := \Justthebox{\disclosebox}{-1.3ex}$.
...
\begin{enumerate}
  \item For $i$ in $\{1..\NOld\}$,
    \begin{itemize}
      ...
      \item If $\DisclosePlaintext{i} = \bot$ then set $\SharedPlaintext{i} := \bot$
and $\vOld{i} := \bot$, and continue with the next $i$.
...
decrypts correctly is the one that should be used in step 3 onward.
