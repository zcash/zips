...
in the \blockChain increments by $1$. Implementations \MUST support \blockHeights in the
range $\binaryrange{31}$. \nufive{As of \NUFive, there is a consensus rule that all
\coinbaseTransactions MUST have the $\nExpiryHeight$ field set to the \blockHeight, and
this limits the maximum \blockHeight to $2^{32} - 1$, absent future consensus changes.}
...
A \transaction has a \defining{\transactionID} that can be used to refer to it.
\xTransactionIDs are not part of the consensus protocol but are used by other parts of the
ecosystem, for example they are shown in block explorers and can be used in higher-level protocols.
...
Version 1.1 of the reference implementation.
