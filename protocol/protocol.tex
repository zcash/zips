...
\begin{itemize}
    ...
    \item $\NoteCommitRand$ is a 24-byte \COMMtrapdoor.
\end{itemize}
...
\begin{bytefield}[bitwidth=0.039em]{776}
    ...
    \bitbox{256}{256 bit $\NoteAddressRand$}
    \bitbox{192}{192 bit $\NoteCommitRand$} &
\end{bytefield}
\end{lrbox}
...
The \notePlaintexts associated with a \pourDescription are encrypted to the
...
\changed{$\Memo$ is a 64-byte \memo associated with this \note.

The usage of the $\memo$ is by agreement between the sender and recipient of the
\note. It should be encoded as a UTF-8 human-readable string \cite{Unicode}, padded
...
\begin{bytefield}[bitwidth=0.035em]{1288}
    ...
    \bitbox{192}{$\NoteCommitRand$ (\changed{24} bytes)} &
...
\begin{itemize}
    ...
    \item \changed{24} bytes specifying $\NoteCommitRand$.
\changed{
...
store \noteCommitments that \pourTransfers produce. Just as the \term{unspent
...
after all of its constituent \pourDescriptions' \noteCommitments have been
...
to $\pourPubKey$ to sign this \transaction.


\subsection{Balance}

A \pourTransfer can be seen, from the perspective of the \transaction, as
