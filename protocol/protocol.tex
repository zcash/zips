...
\newcommand{\heartwoodonly}[1]{\heartwood{[\Heartwood only, \canopy{pre-\Canopy}]\, {#1}}}
...
\newcommand{\blossomonly}[1]{\blossom{[\Blossom only, \heartwood{pre-\Heartwood}]\, {#1}}}
\newcommand{\blossomonward}[1]{\blossom{[\Blossom onward]\, {#1}}}
\newcommand{\presapling}[1]{\sapling{[Pre-\Sapling\!]\,} {#1}}
\newcommand{\saplingonly}[1]{\sapling{[\Sapling only, \blossom{pre-\Blossom}]\, {#1}}}
...
\overwinteronly{All \transactions use the \Sapling \consensusBranchID \hexint{5BA81B19}
...
\saplingonly{All \transactions use the \Sapling \consensusBranchID \hexint{76B809BB}
as defined in \cite{ZIP-205}.}

\blossomonly{All \transactions use the \Blossom \consensusBranchID \hexint{2BB40E60}
as defined in \cite{ZIP-206}.}

\heartwoodonly{All \transactions use the \Heartwood \consensusBranchID \hexint{F5B9230B}
as defined in \cite{ZIP-250}.}

\canopyonly{All \transactions use the \Canopy \consensusBranchID \hexint{E9FF75A6}
...
\nufiveonly{All \transactions use the \NUFive \consensusBranchID \hexint{F919A198}
as defined in \cite{ZIP-252}.}

\nusixonly{All \transactions use the \NUSix \consensusBranchID \hexint{C8E71055}
...
\begin{nnotes}
  ...
  \item Currently there is no way to withdraw from the \deferredChainValuePool, and so
        there is no possibility of it going negative. Therefore, no consensus rule for
        the \DeferredChainValuePoolBalance corresponding to the above rules for other
        \chainValuePool balances is needed at this time.
\end{nnotes}
} %nusix
...
(for which a \Mainnet activation height has not yet been set). Its specifications are described
in this document, \cite{ZIP-253}, \cite{ZIP-236}, and \cite{ZIP-2001}, with updates to
...
\begin{consensusrules}
  ...
  \nufiveonwarditem{If $\effectiveVersion \geq 5$ and $\nActionsOrchard > 0$,
        ...
        \end{itemize}}
        \vspace{-1ex}
  \item Let $\totalDeferredOutput$ be as defined in \crossref{subsidies}.
        ...
        \begin{itemize}
          \item define the \totalOutputValue of its \coinbaseTransaction to be the total value
                ...
                plus $\totalDeferredOutput(\mathsf{height})$;
...
\hline
Stream                   & Numerator & Denominator & Start height & End height \\\hline\raisedstrut
\texttt{FS\_DEFERRED}    &   $12$    &    $100$    &  $2726400$   &  $3146400$ \\
\texttt{FS\_FPF\_ZCG}    &    $8$    &    $100$    &  $2726400$   &  $3146400$ \\\hline
\end{tabular}
\end{formulae}
...
\hline
Stream                   & Numerator & Denominator & Start height & End height \\\hline\raisedstrut
\texttt{FS\_DEFERRED}    &   $12$    &    $100$    &  $2976000$   &  $3396000$ \\
\texttt{FS\_FPF\_ZCG}    &    $8$    &    $100$    &  $2976000$   &  $3396000$ \\\hline
\end{tabular}
\end{formulae}
...
Conrado Gouvea, and no doubt others.
...
\historyentry{2024.5.1}{}

\begin{itemize}
\nusix{
    \item Apply \NUSix consensus changes from \cite{ZIP-2001} and \cite{ZIP-236}.
} %nusix
    \item Moved the descriptions of \chainValuePool balances for each pool into
          a common section \crossref{chainvaluepoolbalances}.
\end{itemize}
