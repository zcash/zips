...
\setof{\mathtt{0x01}},               &\caseif \BlockHeight < \CanopyActivationHeight \\
\setof{\mathtt{0x01}, \mathtt{0x02}},&\caseif \CanopyActivationHeight \leq \BlockHeight < \CanopyActivationHeight + \ZIPTwoOneTwoGracePeriod \\
\setof{\mathtt{0x02}},               &\caseotherwise.
\end{cases}$

The $\NotePlaintextLeadByte$ of a \Sapling or \Orchard \note MUST satisfy $\NotePlaintextLeadByte \in \AllowedLeadBytes^{\Protocol}(\BlockHeight, \TxVersion)$.
Senders \SHOULD choose the highest \notePlaintextLeadByte allowed under this condition.

\notbeforenufive{
\nnote{Since \Orchard was introduced after the end of the \cite{ZIP-212} grace period, \notePlaintexts
for \Orchard \notes \MUST have $\NotePlaintextLeadByte \geq \mathtt{0x02}$.}
} %notbeforenufive
...
\nufive{
    ...
    \item Fix an error in the statement of Merkle Path Validity for \Orchard: the  
} %nufive
\sapling{
