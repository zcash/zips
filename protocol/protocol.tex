...
private keys are defined as byte strings, which are converted from integers
...
string $\bytes{q}$ by the Curve25519 secret key represented by the
byte string $\bytes{n}$, as defined in section 2 of \cite{Curve25519};
  \item $\CurveBase$ is the public string representing the Curve25519
base point;
  \item $\Clamp(\bytes{x})$ takes a 32-byte string $\bytes{x}$ as input
and returns a byte string representing a Curve25519 private key, with
