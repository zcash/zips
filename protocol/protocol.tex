...
\makeatletter
\renewcommand*{\@fnsymbol}[1]{\ensuremath{\ifcase#1\or \dagger\or \ddagger\or \mathsection\or \parallel\or \mathparagraph\else\@ctrerr\fi}}
\makeatother
...
% There is no math italic font available. \textit\textsf has noticeably different metrics and glyphs
% (wider, lower weight, and the g glyph is open, which occurs frequently in crossrefs containing "Sapling").
% Instead we use a slanted variant of real mathsf, which is fine for a sans-serif font in the specific
% contexts we're using it.
% <https://tex.stackexchange.com/a/63188/78411> Bruno Le Floch; retrieved 2026-06-15; CC BY-SA 4.0
\newsavebox{\argumentbox}
\newcommand{\slantbox}[2][0.22]{\mbox{\sbox{\argumentbox}{#2}\hskip\wd\argumentbox\pdfsave\pdfsetmatrix{1 0 #1 1}\llap{\usebox{\argumentbox}}\pdfrestore}}
...
\newcommand{\pooltype}{\slantedtype}
...
\newcommand{\termpool}[1]{\termandindexx{\pooltype{#1}}{#1}}
...
\newcommand{\llSprout}{\termandindexx{\SproutProto}{Sprout shielded protocol}}  % don't use directly
\newcommand{\Sprout}{\readas{Sprout protocol}{\llSprout{}}\xspace}
\newcommand{\SproutText}{\texorpdfstring{\SproutProto}{Sprout}}

% The Sapling shielded protocol
\newcommand{\llSapling}{\termandindexx{\SaplingProto}{Sapling shielded protocol}}  % don't use directly
\newcommand{\Sapling}{\readas{Sapling protocol}{\llSapling{}}\xspace}
...
\newcommand{\llOrchard}{\termandindexx{\OrchardProto}{Orchard shielded protocol}}  % don't use directly
\newcommand{\Orchard}{\readas{Orchard protocol}{\llOrchard{}}\xspace}
...
% The Sprout pool
\newcommand{\SproutChainValuePool}{\termandindex{\pooltype{Sprout chain value pool}}{chain value pool (Sprout)}}
\newcommand{\SproutPool}{\termandindex{\pooltype{Sprout pool}}{chain value pool (Sprout)}}
\newcommand{\SproutPoolId}{\pooltype{SproutPool}}
\newcommand{\SproutPoolText}{\texorpdfstring{\pooltype{Sprout Pool}}{Sprout Pool}}
% There is no Sprout transaction value pool, since JoinSplits are balanced individually.

% The Sapling pool
\newcommand{\SaplingChainValuePool}{\termandindex{\pooltype{Sapling chain value pool}}{chain value pool (Sapling)}}
\newcommand{\SaplingPool}{\termandindex{\pooltype{Sapling pool}}{chain value pool (Sapling)}}
\newcommand{\SaplingPoolId}{\pooltype{SaplingPool}}
\newcommand{\SaplingPoolText}{\texorpdfstring{\pooltype{Sapling Pool}}{Sapling Pool}}

% The Orchard pool
\newcommand{\OrchardChainValuePool}{\termandindex{\pooltype{Orchard chain value pool}}{chain value pool (Orchard)}}
\newcommand{\OrchardPool}{\termandindex{\pooltype{Orchard pool}}{chain value pool (Orchard)}}
\newcommand{\OrchardPoolId}{\pooltype{OrchardPool}}
\newcommand{\OrchardPoolText}{\texorpdfstring{\pooltype{Orchard Pool}}{Orchard Pool}}

% The Ironwood pool
\newcommand{\IronwoodChainValuePool}{\termandindex{\pooltype{Ironwood chain value pool}}{chain value pool (Ironwood)}}
\newcommand{\IronwoodPool}{\termandindex{\pooltype{Ironwood pool}}{chain value pool (Ironwood)}}
\newcommand{\IronwoodPoolId}{\pooltype{IronwoodPool}}
\newcommand{\IronwoodPoolText}{\texorpdfstring{\pooltype{Ironwood Pool}}{Ironwood Pool}}

% Sapling or Orchard pool, excluding Ironwood.
\newcommand{\SaplingOrOrchardPool}{\sapling{\SaplingPool{}}\nufive{ or \OrchardPool{}}\xspace}
\newcommand{\SaplingAndOrchardPool}{\sapling{\SaplingPool{}}\nufive{ and \OrchardPool{}}\xspace}
...
\newcommand{\SaplingLikeChainValuePool}{\notnufive{\SaplingChainValuePool}%
  \notbeforenufive{\termandindex{\pooltype{Sapling or Orchard chain value pool}}{chain value pool (Sapling or Orchard)}}%
  \notbeforenusixthree{\termandindex{\pooltype{Sapling, Orchard, or Ironwood chain value pool}}{chain value pool (Sapling or Orchard or Ironwood)}}\xspace}
...
\newcommand{\SaplingLikeChainValuePools}{\notnufive{\SaplingChainValuePool}%
  \notbeforenufive{\termandindex{\pooltype{Sapling and Orchard chain value pools}}{chain value pool (Sapling or Orchard)}\xspace}%
  \notbeforenusixthree{\termandindex{\pooltype{Sapling, Orchard, and Ironwood chain value pools}}{chain value pool (Sapling or Orchard or Ironwood)}\xspace}}
\newcommand{\SaplingLikePools}{\SaplingPool{}\nufive{ and \OrchardPool{}}\nusixthree{ and \IronwoodPool{}}}
...
\newcommand{\OrchardLikeChainValuePool}{\notnusixthree{\OrchardChainValuePool}\notbeforenusixthree{\termandindex{\pooltype{Orchard \nusixthree{or Ironwood} chain value pool}}{chain value pool (Orchard or Ironwood)}\xspace}}
\newcommand{\OrchardLikePool}{\OrchardPool\nusixthree{ or \IronwoodPool}}

\newcommand{\OrchardLikeChainValuePools}{\notnusixthree{\OrchardChainValuePool}\notbeforenusixthree{\termandindex{\pooltype{Orchard \nusixthree{and Ironwood} chain value pool}}{chain value pool (Orchard or Ironwood)}\xspace}}
\newcommand{\OrchardLikePools}{\notnufive{\OrchardPool}\notbeforenusixthree{\termandindex{\pooltype{Orchard \nusixthree{and Ironwood} chain value pools}}{chain value pool (Orchard or Ironwood)}}\xspace}

% Transparent pool
\newcommand{\TransparentChainValuePool}{\termandindex{\pooltype{transparent chain value pool}}{chain value pool (transparent)}}
\newcommand{\TransparentPool}{\termandindex{\pooltype{transparent pool}}{chain value pool (transparent)}}

% Deferred development fund pool
\newcommand{\DeferredChainValuePool}{\termandindex{\pooltype{deferred development fund chain value pool}}{chain value pool (deferred development fund)}}
\newcommand{\DeferredPool}{\termandindex{\pooltype{deferred development fund pool}}{chain value pool (deferred development fund)}}
...
\newcommand{\BitcoinCore}{\termsoftware{Bitcoin Core}}
...
\newcommand{\allowedLeadBytes}[1]{\mathsf{allowedLeadBytes}^{#1}}
...
\newcommand{\vBalance}[1]{\mathsf{v^{#1\pooltype{Balance}}}}
...
The name ``Sprout'' is used for the \Zcash protocol prior to \NUSapling
(both before and after \NUOverwinter), and in particular its \Sprout \shieldedProtocol.
The name ``Sapling'' is used for the \NUSapling \networkUpgrade, the \Sapling
\shieldedProtocol, and the \SaplingPool.
\nufive{The name ``Orchard'' is used for the \Orchard \shieldedProtocol and the
\OrchardPool.}
...
\spendingKey\sapling{ (or the \nullifierDerivingKey for \SaplingAndOrchardPool \notes)}.
...
A \defining{\note} (denoted $\NoteTuple{}$) can be a \SproutPool\sapling{ or \SaplingLikePool}
\note. In each case it represents that a value $\Value$ is spendable by the recipient who
...
\introlist
Let $\NoteType{Sprout}$ be the type of a \SproutPool \note, i.e.
\begin{formulae}
...
\introlist
Let $\NoteType{Sapling}$ be the type of a \SaplingPool \note, i.e.
\begin{formulae}
...
\introlist
Let $\NoteType{Orchard}$ be the type of an \OrchardLikePool \note, i.e.
\begin{formulae}
...
\nusixthree{
Let the constant $\NUSixThreeActivationHeight$ be as defined in \cite{ZIP-257}.
} %nusixthree
...
\begin{formulae}
  ...
  \item $\begin{cases}
           ...
           \setof{\hexint{01}, \hexint{02}},  &\caseif \CanopyActivationHeight \leq \BlockHeight < \CanopyActivationHeight \\
...
The $\NotePlaintextLeadByte$ of a \SaplingLikePool \notePlaintext \MUST satisfy\notbeforenusixthree{ the condition}
...
Unlike in \Sapling, the definition of an \OrchardPool \note includes the
...
\introsection
In a given \blockChain, for \sapling{each of} the \Sprout\sapling{ and \SaplingLikePools,}
...
\sapling{There is no equivalent of interstitial \treestates for the \SaplingLikePools.}
...
\vspace{-1ex}
\joinSplitTransfers are not used for \SaplingPool \notes. \defining{Instead, there is a
...
A \spendDescription specifies an \anchor, which refers to the output \SaplingPool \treestate
...
both the \OrchardPool and the \IronwoodPool.}
...
\orchardBindingSignature\nusixthree{ (for each \shieldedPool)} that the result commits
to a value consistent with the net \transparent value change (as described in
\crossref{orchardbalance}). This approach allows all of the \zkSNARK \statements to be
...
\notnusixthree{
  ...
  \item The $\anchorField{Orchard}$ field of the \transaction, whenever it exists (i.e.\
        ...
        final \OrchardPool \treestate.
} %notnusixthree
\notbeforenusixthree{
  \item For $\Pool \in \setof{\OrchardLikePoolIds}$,
        \begin{itemize}
          \item The \actionTransfers of a \transaction for $\Pool$ \MUST be consistent
                with its $\vBalance{\Pool}$ value as specified in \crossref{orchardbalance}.
          \item The $\anchorField{\Pool}$ field of the \transaction, whenever it exists (i.e.\
                when there are any \actionDescriptions for $\Pool$), \MUST refer to some earlier
                \block's final $\Pool$ \treestate.
        \end{itemize}
} %notbeforenusixthree
\end{consensusrules}
} %nufive
...
in a \validBlockChain. \sapling{\SproutPool and \SaplingLikePool \nullifiers are
...
to derive the unique $\NoteUniqueRand$ value for a \SaplingPool \note. It is also used
...
\notnufive{
  \item \sapling{sending (\crossref{saplingsend}) and receiving (\shortcrossref{saplingandorchardinband}) \SaplingPool \notes,
...
\vspace{-1ex}
In order to send \SproutPool funds, the sender constructs a
...
\vspace{-1ex}
In order to send \SaplingPool \shielded value, the sender constructs a \transaction
...
In order to send \OrchardPool \shielded value, the sender constructs a \transaction
...
\introlist
\vspace{0.75ex}
A \spendDescription for a \dummy \SaplingPool input \note with \notePlaintextLeadByte
...
As in \Sprout, a \dummy \SaplingPool output \note is constructed as normal but with
zero value, and sent to a random \shieldedPaymentAddress.
} %sapling
...
\introlist
\vspace{0.5ex}
The spend-related fields of an \actionDescription for a \dummy \OrchardPool input \note are
...
As in \Sprout and \Sapling, a \dummy \OrchardPool output \note is constructed as normal but with
zero value, and sent to a random \shieldedPaymentAddress.
...
Let $\MerkleDepth{}$ be $\MerkleDepth{Sprout}$ for the \SproutPool \noteCommitmentTree\sapling{,
or $\MerkleDepth{Sapling}$ for the \SaplingPool \noteCommitmentTree}\nufive{,
or $\MerkleDepth{Orchard}$ for the \OrchardPool \noteCommitmentTree}.
...
The following discussion applies independently to the \SproutPool\sapling{ and \SaplingPool}\nufive{ and \OrchardPool}
\noteCommitmentTrees.
...
\begin{consensusrules}
  ...
  \nusixthreeonlyitem{All \transactions \MUST use the \NUSixThree \consensusBranchID \hexint{TODO}
        as defined in TODO.}
\end{consensusrules}
...
\nufive{
For a v5 \transaction, $\vBalance{\SaplingPoolId}$ is implicitly zero if the \transaction has
...
\SaplingPool \valueCommitment randomnesses, authorized a \transaction with the given \sighashTxHash
by signing $\SigHash$.

\vspace{1ex}
\pnote{
The spender \MAY reveal any strict subset of the \SaplingPool \valueCommitment randomnesses
...
\pnote{
The spender \MAY reveal any strict subset of the \OrchardPool \valueCommitment randomnesses to
...
It is denoted by $\ChainValuePoolBalance{\TransparentPool}(\BlockHeight)$.
...
$$
\ChainValuePoolBalance{\DeferredPool}(\BlockHeight) :=
...
$$
\begin{array}{rl}
  \IssuedSupply(\BlockHeight) \; := &\!\!\!          \ChainValuePoolBalance{\TransparentPool}(\BlockHeight) \\
...
\notbeforenusix{
                                    & \nusix{+\,\;   \ChainValuePoolBalance{\DeferredPool}(\BlockHeight)}
}
\end{array}
$$
...
\begin{nnotes}
  ...
  \item There is intentionally no equivalent to the \snarkref{Ephemeral public key integrity}{outputepkintegrity}
        check from the \Sapling \outputStatement. It is unnecessary for the sender of an \OrchardPool \note
...
further security considerations, for example of how to validate a \SproutPool \note
...
\diversifiedTransmissionKey for the intended recipient address of a new \SaplingLikePool \note,
...
further security considerations, for example of how to validate a \SaplingLikePool \note
received \outOfBand, which are not addressed in this document.
} %pnote
} %sapling
...
(\crossref{concretesinsemillacommit}), and for the \OrchardPool \incrementalMerkleTree
...
part of deriving the \nullifier for an \OrchardLikePool \note.
...
The net value of \SaplingPool spends minus outputs.\! \\ \hline
...
Authorizing signatures for each spend of an \OrchardPool \actionDescription.\! \\ \hline
...
\begin{consensusrules}
  ...
  \nufiveonwarditem{\valueBalance{Orchard} \MUST be in the range $\range{-\MAXMONEY}{\MAXMONEY}$ for version
        5 \transactions.}
  \heartwoodonwarditem{All \SaplingLikePool outputs in \coinbaseTransactions \MUST decrypt to a
        ...
        \NUCanopy activation,} \SaplingPool outputs of a \coinbaseTransaction \MUST have \notePlaintextLeadByte
        equal to $\hexint{01}$.)}
  \canopyonwarditem{Any \SaplingOrOrchardPool output of a \coinbaseTransaction decrypted to a \notePlaintext according
  ...
  \nusixthreeonwarditem{Any \IronwoodPool output of a \coinbaseTransaction decrypted to a \notePlaintext according
...
\heartwood{
  ...
  \canopyonwarditem{The rule that \SaplingPool outputs in \coinbaseTransactions \MUST decrypt to a \notePlaintext
  ...
  \nufiveonwarditem{The flags in \flagsOrchard{} allow a version 5 \transaction to declare that no funds are
        spent from \OrchardPool \notes (by setting \enableSpendsOrchard{} to $0$), or that no new \OrchardPool \notes
        ...
        in \coinbaseTransactions for \TransparentPool, \SproutPool, or \SaplingPool funds, which would not otherwise
  ...
  \nufiveonwarditem{Because \enableSpendsOrchard{} is set to $0$ in version 5 \coinbaseTransactions\ --which
        disables non-zero-valued \OrchardPool spends-- the $\valueBalance{Orchard}$ field of a \coinbaseTransaction
...
\noteCommitmentTree corresponding to the final \SaplingPool \treestate of this \block.\\}
...
\notheartwood{
  \saplingonwarditem{$\hashFinalSaplingRoot$ \MUST be $\LEBStoOSPOf{256}{\rt{\SaplingPoolId}}$ where
        $\rt{\SaplingPoolId}$ is the \merkleRoot of the \SaplingPool \noteCommitmentTree for the final
        \SaplingPool \treestate of this \block.}
}
\notbeforeheartwood{
...
\begin{itemize}
  ...
  \item The \defining{\prescribedWay} to pay a \SaplingOrOrchard \paymentAddress is
        ...
        for \SaplingAndOrchardPool \shieldedOutputs of \coinbaseTransactions in
...
Note that it is possible for two distinct \SaplingPool \positionedNotes (having
...
that \commitmentScheme ensures that \OrchardPool \nullifiers will be unique. (Specifically,
...
commitment: \shaHash for \SproutPool \notes\sapling{,\notnufive{ and} $\PedersenHashToPoint$
for \SaplingPool \notes}\nufive{, and $\SinsemillaHashToPoint$ for \OrchardPool \notes}.
...
\vspace{-1ex}
\sproutspecificpnote{
\SproutPool \noteCommitments are not statistically \hiding, so for \SproutPool \notes,
...
is supported for \SaplingAndOrchardPool \notes under the same conditions as in
...
\begin{itemize}
    \item The paper defines a \note as $((\AuthPublic, \TransmitPublic), \Value,
          ...
          specification defines a \SproutPool \note as
...
\begin{itemize}
\nufive{
    \item Set $\mathsf{NU5ActivationHeight}$ for \Testnet and \Mainnet.
...
\nufive{
    ...
    \item Add a step to the algorithm for generating an \OrchardPool \note in \crossref{orchardsend},
...
\nufive{
    ...
    \item Clarify that a \dummyNote should be created if no real \OrchardPool \note is being
...
\nufive{
    ...
    \item The $\NoteUniqueRand$ value for an \OrchardPool output \note was incorrectly described as
...
\sapling{
    ...
    \item The specification of decryption in \shortcrossref{decryptovk} differed from the \zcashd
          ...
          \end{itemize}
    \item Correct the procedure for generating \dummy \SaplingPool \notes in \crossref{saplingdummynotes}.
...
\sapling{
    \item Fix a type error in the output of $\PRFnf{Sapling}{}$; a \SaplingPool \nullifier is a
...
\sapling{
    \item Add a missing consensus rule for version 4 \transactions: if there are
          no \SaplingPool Spends or Outputs, then $\valueBalance{Sapling}$ \MUST be $0$.
} %sapling
\end{itemize}
...
\sapling{
    ...
    \item Clarify the use of $\cvNew{}$ and $\cmNew{}$, and the selection of
          \outgoingViewingKey, in sending \SaplingPool notes.
...
\sapling{
    ...
    \item Add consensus rules that $\cv$ in a \spendDescription, and $\cv$ and $\EphemeralPublic$ in an
          ...
          when encrypting \SaplingPool notes.
...
\sapling{
    ...
    \item Allow dummy \SaplingPool input \notes.
