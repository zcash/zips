...
\begin{pnotes}
  \item This property implies (and is stronger than) collision-resistance,
        since a collision $(m_1, m_2)$ for $\GroupGHash{U}$ trivially gives a
        discrete logarithm relation with $x_1 = 1$ and $x_2 = -1$.
  \item An alternative approach is to model $\GroupGHash{U}$ as a random
...
\begin{formulae}
  ...
  \item $T := \BlakeTwob{(\mathnormal{n \mult m})}(\powtag,\, S \,||\, \powcount(\floor{\frac{i-1}{m}}))$.
\end{formulae}
...
Let $\KDFSapling(\OutputUnique, \OutputIndex, \DHSecret{}, \EphemeralPublic) :=
       \BlakeTwob{256}(\ascii{Zcash\_SaplingKDF},
                       \OutputUnique || \OutputIndex || \reprJ(\DHSecret{}) || \reprJ(\EphemeralPublic))$.
}
...
\sapling{
\nsubsubsubsection{\GroupHash{} into \Jubjub} \label{grouphashjubjub}
...
splits the input into segments of up to $249$ bits, and then splits each segment
