...
The first \transaction in a block must be a \coinbaseTransaction,
\question{what's a coinbase transaction? The term hasn't been defined
before, not clear if its defined later as well.} which should
...
this, so perhaps must-> should} be chosen independently at random for each
  ...
  \item $\h{\allOld} \typecolon (\PRFOutput)^{\NOld}$ is
        a sequence of tags \question{what's a tag?} that bind $\hSig$ to each
...
\begin{itemize}
  \item[] $\MerkleDepth = \changed{29}$ \question{What about the maximal number of JS in a tx?
