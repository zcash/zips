...
- `zcashd` v4.7.0 will implement what is expected to be the final revision
  of the NU5 consensus rules, causing a long rollback to an alternate chain.
  It will be necessary to use the ``-reindex`` option to `zcashd` in order
  to follow this chain as intended.
