...
authority that may be exercised without user intervention. This is not the case
for e.g. wallets where key material is stored in a secure enclave with
access that is mediated by direct user interaction, or wallets used in
conjunction with hardware devices.
