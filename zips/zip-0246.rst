...
We omit the descriptions of the sections that do not change for the OrchardZSA protocol::
    ...
    ├── transparent_sighash_infos_digest
    ├── sapling_auth_digest
    ├── orchard_auth_digest
...
In the case that the transaction contains transparent inputs, this is a BLAKE2b-256 hash of the transparent sighash information 
and the transparent scripts for all transparent inputs belonging to the transaction.
For each transparent input, the field encoding of the ``TransparentSighashInfo`` value associated with that transparent input, 
followed by the field encoding of the concatenated values of the Bitcoin script values associated with that transparent input 
...
The field encoding of ``TransparentSighashInfo`` is specified in ZIP 230 [#zip-0230-transaction-field-encodings]_.
