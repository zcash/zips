...
A BLAKE2b-256 hash of the following values ::
   ...
   A.2: transparent_sighash_infos_digest (32-byte hash output)
   A.3: sapling_auth_digest              (32-byte hash output)
   A.4: orchard_auth_digest              (32-byte hash output)
   A.5: issuance_auth_digest             (32-byte hash output)
...
A.2: transparent_sighash_infos_digest
`````````````````````````````````````

A BLAKE2b-256 hash of the field encodings of all ``TransparentSighashInfo`` values belonging to the transaction.

The personalization field of this hash is set to::

    "ZTxAuthTrSHIHash"

In the case that the transaction has no transparent inputs, ``transparent_sighash_infos_digest`` is::

    BLAKE2b-256("ZTxAuthTrSHIHash", [])
