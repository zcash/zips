...
payment address, amount, asset id (if applicable), and optional memo.
...
.. code-block:: EBNF
  ...
  assetidparam    = "req-assetid"[ paramindex ] "=" *base64url
...
The value of an ``req-assetid`` parameter MUST be a `base64url` string without
padding. This is the canonical ``EncodeAssetId`` defined in ZIP 227 [#zip-0227]_.
Implementations MUST reject any ``req-assetid`` value that does not match
`*base64url`.
...
by ``EncodeAssetId`` as specified in ZIP 227 [#zip-0227]_.
