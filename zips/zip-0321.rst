...
amount encoded as an unsigned integer in minimal units.
...
::
  ...
  req-asset = base64url( 0x00 || EncodeAssetId(66 bytes) || uint64_be(8 bytes) )

The leading version byte (``0x00``) indicates the encoding format.
For version ``0x00``, the next 66 bytes are the ``EncodeAssetId`` value
defined in ZIP 227 [#zip-0227]_, followed by 8 bytes representing the amount
as an unsigned 64-bit integer in big-endian (network) byte order,
corresponding to the number of minimal units of the asset to be transferred.
...
be for the default asset (ZEC), and the transfer amount is specified using
...
::
  ...
  = 0x0000000000002747
  = 00 00 00 00 00 00 27 47 (8 bytes, big-endian)

Concatenating the version byte, asset id, and amount gives (hex):
::
...
If an amount is provided without an asset parameter, the amount is considered to be
...
Amounts for other assets are specified in the Assets section above.
...
encodes a version byte, an asset identifier and asset amount.
 ...
 * A Sapling address is used with a ``req-asset`` parameter. Sapling receivers do not
   support asset payments (only Unified Addresses with an Orchard receiver do).
