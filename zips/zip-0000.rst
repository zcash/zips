...
* if it concerns Consensus Process, assign a number in the range 0..9
  or 1000..1199;
* if it describes an update to existing ZIPs and/or the Zcash Protocol
  Specification, assign a number above 2000.
...
* If the proposed change affects consensus and has already been deployed,
  it SHOULD be written as an "Update ZIP" describing the changes to be
  made, rather than directly as a pull request modifying the existing
  ZIP or the Zcash Protocol Specification. At the discretion of the ZIP
  Editors, this might not be needed in some cases for compatible changes
  or clarifications.
