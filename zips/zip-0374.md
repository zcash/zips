...
- v2
  ...
  - Adds an `IronwoodBundle` carrying the *Ironwood-pool* component of a v6
    transaction.
  - Adds a `note_version` field to the Orchard-protocol bundles, so that an
    `IronwoodBundle` can carry notes using the ZIP 2005 [^zip-2005]
    quantum-recoverable note plaintext format. (An `OrchardBundle` is not
    permitted to use that format [^zip-0229]; the field makes the shared
    bundle structure self-describing.)
...
This section canonically defines how the data types used in the "Structure"
part of this specification are encoded. The encodings are a profile of the
Postcard Wire Format [^postcard], and are byte-for-byte identical to the
serialization implemented in the `pczt` Rust crate [^pczt-rust] (as of
release `pczt 0.7.0` [^pczt-0.7.0] for the v1 encoding). That crate is a
reference implementation; this ZIP is the canonical definition of the PCZT
encodings, and implementations in other languages can be written from this
section alone, without reference to the Postcard specification or to the Rust
serialization machinery.

Define `varint(n)` to be the unsigned little-endian base-128 encoding of the
integer `n`: the value is encoded 7 bits at a time, least significant bits
first, one byte per group, where every byte except the last has bit 7 set and
the last byte has bit 7 clear. Encoders MUST emit the shortest encoding of the
value (equivalently: the final byte is nonzero, unless the entire encoding is
the single byte `0x00`). The encoding of a `k`-bit integer occupies at most
`ceiling(k/7)` bytes, and in a maximum-length encoding the final byte
contributes only the high `k mod 7` bits (for example, the fifth byte of an
encoded `u32` is at most `0x0f`); decoders MUST reject encodings that exceed
either limit, and MAY either accept or reject non-minimal encodings within
them. (The reference implementation accepts them; encoders MUST NOT rely on
this.)

The data types are encoded as follows:

- `u8`: a single byte containing the value. (Not a varint.)

- `u32`, `u64`: `varint(n)`; at most 5 and 10 bytes respectively.

- `i128` (used only for `SaplingBundle.value_sum`): `varint(zigzag(n))`,
  at most 19 bytes, where `zigzag(n)` reinterprets
  `(n << 1) XOR (n >> 127)` (with `>>` an arithmetic shift) as an unsigned
  128-bit integer. That is, a non-negative value `n` encodes as `2n`, and a
  negative value `n` encodes as `2|n| - 1`.

- `bool`: a single byte; `0x00` is false and `0x01` is true. Decoders MUST
  reject other values.

- `[T; N]`: the concatenation of the encodings of the N elements, with no
  length prefix. In particular `[u8; N]` is exactly N raw bytes, and
  `[[u8; 32]; 32]` (the type of `witness` Merkle paths) is exactly 1024 raw
  bytes.

- `(A, B)`: the encoding of `A` followed by the encoding of `B`, with no
  framing.

- `Option<T>`: the single byte `0x00` for `None`; the single byte `0x01`
  followed by the encoding of the value for `Some`. Decoders MUST reject
  other tag bytes.

- `List<T>`: `varint(count)`, where count is the number of elements, followed
  by the encodings of the elements in order. (For `List<u8>`, the count is
  the byte length.)

- `UTF8String`: `varint(len)`, where len is the length in bytes, followed by
  the UTF-8 encoding of the string. Decoders MUST reject byte sequences that
  are not valid UTF-8.

- `Map<K, V>`: `varint(count)`, where count is the number of entries,
  followed by each entry as the encoding of its key followed by the encoding
  of its value. Encoders MUST emit entries in strictly ascending
  lexicographic byte order of the (unencoded) keys, with no duplicate keys.
  Decoders MAY either accept or reject out-of-order or duplicated entries.
  (The reference implementation accepts them, retaining the last value for a
  duplicated key; encoders MUST NOT rely on this.)

- `Enum { A, B, ... }`: `varint(d)`, where `d` is the variant's 0-based
  discriminant, followed by the encoding of the variant's payload. (Every
  enumeration in this ZIP consists of payload-free variants, so the
  discriminant stands alone.)

- Structures (`Global`, the bundles, and their substructures): the
  concatenation of the encodings of their fields, in the order the fields are
  listed in this ZIP, with no framing, field tags, or padding. A field that
  is annotated as not present in the PCZT version being encoded is omitted
  entirely (it is *not* encoded as an absent `Option`).

The version-specific encoding ends at the last byte of the final field.
Encoders MUST NOT emit trailing data after it; decoders MAY either ignore or
reject trailing data. (The reference implementation ignores it.)

### v1 Encoding

The v1 version-specific encoding is the concatenation of the following
structures, encoded according to the "Data type encodings" section above,
using the struct names from the "Structure" part of the specification; fields
within each struct are encoded in the order they appear in the ZIP.
```
PCZT
- Global
  - fields...
...
The v2 version-specific encoding likewise uses the encodings of the "Data
type encodings" section above, and differs from the v1 encoding as follows:
...
The `pczt` Rust crate and the protocol crates listed below are the reference
implementation of this ZIP; the ZIP itself is the canonical definition of the
format. The v1 encoding corresponds to release `pczt 0.7.0`. [^pczt-0.7.0]
...
[^pczt-0.7.0]: [Release `pczt 0.7.0` of the `pczt` Rust crate](https://github.com/zcash/librustzcash/releases/tag/pczt-0.7.0)
