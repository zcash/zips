...
7. Specify which fields of the effecting data are compact (intended for the
   light wallet protocol) and which are noncompact. The digest algorithms
   for the bundle's contributions to the transaction identifier and
   authorizing data commitment are determined by the `Opaque Digest
   Personalizations`_ and do not need to be specified per bundle type.
...
Opaque Digest Personalizations
``````````````````````````````

The digest of each bundle's data is computed as a flat BLAKE2b-256 hash
using one of three fixed personalization strings. The hash input is always
prefixed with ``compactSize(bundleType) || compactSize(bundleVariant)``,
which provides domain separation between bundle types without needing to
embed the bundle identifier in the personalization. This allows any wallet
to compute the transaction identifier and authorizing data commitment for
a v6 transaction, even one containing bundle types the wallet does not
understand.

The personalizations are:

* ``"ZTxIdCEffectHash"`` — compact effects digest
* ``"ZTxIdNEffectHash"`` — noncompact effects digest
* ``"ZTxAutBundleHash"`` — authorizing data digest

For each of these, the hash input is::

  compactSize(bundleType) || compactSize(bundleVariant) || <data bytes>

where ``<data bytes>`` is ``vCompactData``, ``vNoncompactData``, or
``vAuthData`` respectively.

V6 Transaction Bundle Type Registry
```````````````````````````````````

The following ``(bundleType, bundleVariant)`` pairs are registered for the V6
...
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``mEffectBundles``            |``EffectBundleData[nEffectBundles]``            |A map from bundle identifier to the effecting data of a bundle.      |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``nAuthBundles``              |``compactSize``                                 |Number of bundles in the transaction that have per-bundle data.      |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``mAuthBundles``              |``AuthBundleData[nAuthBundles]``                |A map from bundle identifier to the authorizing data of a bundle.    |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
...
Each entry in ``mEffectBundles`` separates the bundle's effecting data into
a **compact** portion (the subset intended for the light wallet protocol, such
as nullifiers, commitments, ephemeral keys, and the leading bytes of encrypted
ciphertexts) and a **noncompact** portion (everything else). This split enables
trustless partial verification: a light client can verify the compact data it
receives against the transaction identifier without needing the noncompact data.
...
| varies                      |``bundleVariant``             |``compactSize``                                 |An encoding of the bundle variant identifier.                        |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``nCompactDataLen``           |``compactSize``                                 |The length of the ``vCompactData`` byte array.                       |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``vCompactData``              |``byte[nCompactDataLen]``                       |The compact effecting data for the bundle, as defined by the         |
|                             |                              |                                                |bundle type's defining ZIP.                                          |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``nNoncompactDataLen``        |``compactSize``                                 |The length of the ``vNoncompactData`` byte array.                    |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``vNoncompactData``           |``byte[nNoncompactDataLen]``                    |The noncompact effecting data for the bundle, as defined by the      |
|                             |                              |                                                |bundle type's defining ZIP.                                          |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+

The compact/noncompact split for each bundle type defined in this ZIP is
specified in the corresponding bundle section below. When a ZIP introduces a
new bundle type, it MUST specify which fields of its effecting data belong to
the compact portion and which to the noncompact portion.

AuthBundleData
--------------

Each entry in ``mAuthBundles`` carries the bundle's authorizing data as a
single byte array.
...
| varies                      |``bundleVariant``             |``compactSize``                                 |An encoding of the bundle variant identifier.                        |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``nAuthDataLen``              |``compactSize``                                 |The length of the ``vAuthData`` byte array.                          |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
| varies                      |``vAuthData``                 |``byte[nAuthDataLen]``                          |The authorizing data for the bundle, as defined by the bundle        |
|                             |                              |                                                |type's defining ZIP.                                                 |
+-----------------------------+------------------------------+------------------------------------------------+---------------------------------------------------------------------+
...
The rules in this section are requirements on the byte encoding of a V6
transaction. A byte stream that violates any of these rules is not a
well-formed V6 transaction and MUST be rejected by any parser, regardless
...
* A bundle type MUST NOT appear in any map for which the corresponding column
  of the `V6 Transaction Bundle Type Registry`_ is marked ❌, and MUST NOT
...
* Within the encoding of any bundle's effecting and authorizing data, every
  ...
  * ``vSpendAuthSigsOrchard`` has ``nActionsOrchard`` elements, and
    ``proofsOrchard`` aggregates exactly ``nActionsOrchard`` per-action
    proofs.

A wallet that successfully parses a V6 transaction under these rules is
guaranteed to be able to compute the transaction identifier using the
`Opaque Digest Personalizations`_, and to enumerate the transparent value
flows of the transaction at the granularity of bundle types.
...
For a non-coinbase transaction, a full validator MUST verify that the ZEC
value pool delta for ``bundleType = 0`` equals the total value of the
transparent inputs minus the total value of the transparent outputs. (The
input values are not encoded in the transaction itself; they are obtained
from the UTXOs being spent.)

For a coinbase transaction, the total transparent input value is implicitly
the block subsidy plus the total transaction fees paid by other transactions
in the block. A full validator MUST verify that the ZEC value pool delta for
``bundleType = 0`` equals this implicit input value minus the total value of
the coinbase transparent outputs.
...
The 1:1 correspondences between Orchard authorizing-data elements and the
elements of the corresponding effecting data are required by the parsing
rules in `Parsing Rules`_.
...
The following rules relate value pool deltas across bundles within a
transaction, or across transactions within a block. They MUST be enforced by
full validators.
...
* For the coinbase transaction, the sum of value pool deltas in the ZEC asset
  MUST be equal to the negative of the block subsidy for that block; the block
...
signature in the authorizing data of a V6 transaction.
...
follows::
    ...
    └── effects_bundles_digest
        ├─ (bundle_type_id || bundle_variant || bundle_effects_digest)
        │   ├── compact_effects_digest
        │   └── noncompact_effects_digest
        ├─ ...
        └─ (bundle_type_id || bundle_variant || bundle_effects_digest) ...
...
``bundle_effects_digest`` is a BLAKE2b-256 hash of the following values::

   T.3c.i:  compact_effects_digest    (32-byte hash output)
   T.3c.ii: noncompact_effects_digest (32-byte hash output)

The personalization field of ``bundle_effects_digest`` is set to::

  "ZTxBndEffectHash"

``compact_effects_digest`` is::

  BLAKE2b-256("ZTxIdCEffectHash",
              compactSize(bundleType) || compactSize(bundleVariant) || vCompactData)

``noncompact_effects_digest`` is::

  BLAKE2b-256("ZTxIdNEffectHash",
              compactSize(bundleType) || compactSize(bundleVariant) || vNoncompactData)

In the case that a bundle has empty ``vCompactData`` or ``vNoncompactData``,
the corresponding digest is still computed over the
``compactSize(bundleType) || compactSize(bundleVariant)`` prefix (i.e. it is
not the hash of the empty string).

The personalization field of ``effects_bundles_digest`` is set to::
...
currently defined for V6 transactions), the digest algorithm is as specified
...
.. note:: The auth digest approach (flat vs structured, and whether to use
   the `Opaque Digest Personalizations`_ or per-bundle algorithms) is still
   under discussion. The per-bundle auth digest sections below are retained
   from the structured approach pending this decision.
...
transactions regardless of whether they use new v6 features.

Support for receiving funds in v6 transactions
``````````````````````````````````````````````

Zcash wallets MUST support parsing v6 transactions by the time they are allowed
on the network.

Because the v6 transaction format uses a type-length-value encoding for
...
For bundle types not understood by a wallet, the wallet computes
``compact_effects_digest`` and ``noncompact_effects_digest`` as flat
BLAKE2b-256 hashes of the raw ``vCompactData`` and ``vNoncompactData`` bytes
using the `Opaque Digest Personalizations`_. This enables any wallet to
compute the transaction identifier for any well-formed v6 transaction, even
if it contains bundle types the wallet does not understand.
...
Rationale for flat effect digests with compact/noncompact split
---------------------------------------------------------------

The transaction identifier commits to the effecting data of each bundle via
flat BLAKE2b-256 hashes using the `Opaque Digest Personalizations`_, rather
than per-bundle structured sub-tree digests as used in ZIP 244 [#zip-0244]_.
This has two advantages:

* Any wallet can compute the transaction identifier for any well-formed v6
  transaction, even one containing bundle types the wallet does not understand.
  With structured digests, a wallet that encounters an unknown bundle type
  would need the ``bundle_effects_digest`` to be supplied externally by a
  source that implements the bundle type's digest algorithm.

* New bundle types do not need to specify a digest algorithm in their defining
  ZIP — only the compact/noncompact split of their effecting data. This
  reduces the per-bundle-type specification cost.

The effecting data is split into compact and noncompact portions in the wire
format (``EffectBundleData``) and hashed separately, so that a light client
can verify the compact data it receives against the transaction identifier
without needing the noncompact data. This supports trustless partial
verification in the light wallet protocol, reducing trust in the light wallet
server.

Rationale for structured auth digests
-------------------------------------

The authorizing data commitment retains per-bundle structured digests
(defined by each bundle type's defining ZIP) rather than using flat hashes.
This asymmetry with the effects side is motivated by two considerations:

* It is less important for wallets to be able to compute the authorizing data
  commitment without understanding the corresponding bundle types. Full
  validators check auth digests as part of transaction validation, so a
  wallet that cannot compute the auth commitment for an unknown bundle type
  can still rely on the network's full validators for that assurance.

* Structured auth digests support *selective malleability*: the ability to
  modify specific components of the authorizing data (such as sighash types
  or sighash version information) without invalidating the entire auth
  commitment. This is used by the existing sighash versioning mechanism and
  is motivated by the possible future addition of *action groups* to support
  ZSA asset swaps, where different signers may need to commit to different
  subsets of the transaction's effects.
