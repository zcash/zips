...
$(\mathsf{BundleType}, \mathsf{AssetUuid})$; this logical map MUST NOT contain
more than a single entry for a given key. The `ValuePoolDelta`_ records that
represent the entries this map MUST be encoded in increasing order of
`(bundleType, assetClass, assetUuid)` Lookups in this map are denoted with the
...
``bundleType``. The records that represent each map entry map MUST be encoded
in increasing order of ``bundleType``. Each map MUST NOT contain more than a
single entry for a given ``bundleType``. For each ``bundleType`` that exists in
``mAuthBundles``, a corresponding entry with the same ``bundleVariant`` MUST
exist in ``mEffectBundles``.
...
All entries in ``mValuePoolDeltas`` having the same ``bundleType`` MUST have
the same ``bundleVariant``.
...
* Each of ``mValuePoolDeltas``, ``mEffectBundles``, and ``mAuthBundles`` MUST
  ...
  strictly increasing key order as defined in `Transaction Format`_.
