...
``mValuePoolDeltas`` MUST NOT contain more than a single entry for a given
``(bundleType, assetClass, assetId)`` tuple, which is treated as the "key" of
the map.
...
An example of this was [ZIP 212](https://zips.z.cash/zip-0212). In that case
...
* Can wallets still compute the txid and wtxid of an arbitrary transaction?
    * Yes, provided that effecting and authorizing data is separated. Then they
      ...
      239]](https://zips.z.cash/zip-0239).
