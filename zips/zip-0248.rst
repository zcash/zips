...
Per-Signature Structures
````````````````````````

The following structures carry sighash version information alongside each
signature in the authorizing data of a V7 transaction.
...
|                             |                          |                                        |as defined in `Per-Signature Structures`_.                           |
+-----------------------------+--------------------------+----------------------------------------+---------------------------------------------------------------------+
...
The ``TransparentSighashInfo`` type is defined in `Per-Signature Structures`_.
It consists of a ``compactSize``-prefixed byte array containing the sighash
version and any associated data required by that version.
...
+-----------------------------+--------------------------+----------------------------------------+---------------------------------------------------------------------+
|``8``                        |``blockSubsidy``          |``int64``                               |The block subsidy for that block, in zatoshis.                       |
+-----------------------------+--------------------------+----------------------------------------+---------------------------------------------------------------------+
|``8``                        |``lockboxValue``          |``int64``                               |The part of that block subsidy deposited into the lockbox, in        |
|                             |                          |                                        |zatoshis.                                                            |
+-----------------------------+--------------------------+----------------------------------------+---------------------------------------------------------------------+
...
* ``blockSubsidy`` MUST be nonnegative.
...
A BLAKE2b-256 hash of the following values::
   ...
   T.3.1b: blockSubsidy  (8-byte signed little-endian)
   T.3.1c: lockboxValue  (8-byte signed little-endian)
...
in `Signature Digest`_, computed with ``hash_type = SIGHASH_ALL`` (0x01).
...
The ``SaplingSignature`` field encoding is defined in `Per-Signature Structures`_
and includes ``sighashInfo``.
...
the digest defined in `Signature Digest`_, computed with
...
The ``OrchardSignature`` field encoding is defined in `Per-Signature Structures`_
and includes ``sighashInfo``.
