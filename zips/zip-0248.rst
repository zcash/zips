...
|                             |                          |                                        |[#zip-0230-transparent-sighash-info]_ [#zip-0246-sighash-versioning]_|
+-----------------------------+--------------------------+----------------------------------------+---------------------------------------------------------------------+
...
The ``TransparentSighashInfo`` type is defined in ZIP 230
[#zip-0230-transparent-sighash-info]_. It consists of a ``compactSize``-prefixed
byte array containing the sighash version and any associated data required by
that version, as specified in ZIP 246 [#zip-0246-sighash-versioning]_.
...
* For each ``bundleType`` that appears in any of ``mValuePoolDeltas``,
  ...
  across all three maps MUST have the same ``bundleVariant``. (This, combined
  with the constraint that each map is keyed by ``bundleType``, ensures that a
  transaction uses at most one variant of any given bundle type.)
...
This ZIP adopts the sighash algorithm versioning defined in ZIP 246
[#zip-0246-sighash-versioning]_. Where previously each transaction version had a
single associated sighash algorithm, going forward it is possible for signers to
use any sighash algorithm within the closed set specified for a given transaction
version (and made available in consensus via network upgrades).
...
authorizing data of the transaction, via the per-signature structures defined in
ZIP 230 [#zip-0230]_ (``TransparentSighashInfo``, ``SaplingSignature``,
``OrchardSignature``). See ZIP 246 [#zip-0246]_ for the full specification of
sighash versioning.
...
below. Future sighash versions, defined via updates to ZIP 246
[#zip-0246]_, may specify different digest algorithms.
...
The field encoding of ``TransparentSighashInfo`` is specified in ZIP 230
[#zip-0230-transparent-sighash-info]_.
...
The ``SaplingSignature`` field encoding includes ``sighashInfo``, as specified in
ZIP 230 [#zip-0230-sapling-signature]_.
...
The ``OrchardSignature`` field encoding includes ``sighashInfo``, as specified in
ZIP 230 [#zip-0230-orchard-signature]_.
...
.. [#zip-0230-transparent-sighash-info] `ZIP 230: Version 6 Transaction Format — Transparent Sighash Information <zip-0230.rst#transparent-sighash-information-transparentsighashinfo>`_
.. [#zip-0230-sapling-signature] `ZIP 230: Version 6 Transaction Format — Sapling Signature <zip-0230.rst#sapling-signature-saplingsignature>`_
.. [#zip-0230-orchard-signature] `ZIP 230: Version 6 Transaction Format — Orchard Signature <zip-0230.rst#orchard-signature-orchardsignature>`_
...
.. [#zip-0246-sighash-versioning] `ZIP 246: Digests for the Version 6 Transaction Format — Sighash Versioning <zip-0246.rst#sighash-versioning>`_
