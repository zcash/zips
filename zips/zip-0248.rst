...
Effecting Data and Authorizing Data
```````````````````````````````````
...
1. Request allocation of a bundle type identifier from the registry defined
   below. The identifier is a non-negative integer encoded as a ``compactSize``
   value.
