...
7. Define the digest algorithm for the bundle's contribution to the transaction
   identifier, if effecting data is present. The algorithm SHOULD use a
   structured sub-tree of personalized BLAKE2b-256 hashes, separating compact
   data (intended for the light wallet protocol) from non-compact data, to
   enable trustless partial verification against the transaction identifier.
...
tree, as defined by the bundle type's defining ZIP. For the bundle types defined
in this ZIP, the digest algorithms are specified below.
...
These values may be obtained from any source that implements the digest
algorithm defined by the bundle type's defining ZIP, such as a light wallet
