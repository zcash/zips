...
on the relevant network, calculated in [NSM reissuance height](#nsm-reissuance-height)
below. It is named $\mathsf{NSM\_REISSUANCE\_HEIGHT}$ in the NU7 deployment
ZIP [^zip-0259].

## NSM reissuance height

The reissuance height is determined from the scheduled issuance curve, without
assuming that any funds are removed from circulation. This makes it independent
of transaction activity and the actual NSM Value Balance.

For each network, let $A = \mathsf{NU7ActivationHeight}$ and let $B_A(h)$ be
$\mathsf{ScheduledBlockSubsidy}(h)$ after applying ZIP 218 [^zip-0218] at
$A$. Let $S_A(h) = \sum_{i=0}^{h} B_A(i)$ be cumulative scheduled issuance
through height $h$, excluding additional NSM subsidies and amounts removed
from circulation. Let $H_3$ and $H_4$ be the first heights at which the
ZIP 218 $\mathsf{Halving}$ function equals 3 and 4, respectively.

$\mathsf{DEPLOYMENT\_BLOCK\_HEIGHT}$ SHALL be the first height $h$ with
$\max(A, H_3 + 1) \leq h < H_4$ for which

$$\left\lceil\frac{1375\,(\mathsf{MAX\_MONEY} - S_A(h - 1))}{10^{10}}\right\rceil < B_A(h).$$

The strict inequality selects the first block where the rounded reference
NSM subsidy is below the scheduled block subsidy. The ceiling is the same
rounding rule used for $\mathsf{AdditionalBlockSubsidy}$; the reference
balance $\mathsf{MAX\_MONEY} - S_A(h - 1)$ is used only to select the
deployment height. NU7 activation heights SHALL be chosen so that this height
exists before $H_4$. The NU7 deployment ZIP [^zip-0259] specifies the
resulting height for each network.

The value can be calculated without testing every block. Set
$h_0 = \max(A, H_3 + 1)$, $b = B_A(h_0)$,
$R_0 = \mathsf{MAX\_MONEY} - S_A(h_0 - 1)$, and
$R_{\max} = \lfloor (b - 1)10^{10}/1375 \rfloor$. Since $b$ is constant
before $H_4$, the first qualifying height is

$$h_0 + \left\lceil\frac{\max(0, R_0 - R_{\max})}{b}\right\rceil.$$

For the expected case in which $A$ lies between the second and third
pre-NU7 halvings, the ZIP 218 third halving occurs at
$H_3 = A + 3(H_3^{(75)} - A)$, where $H_3^{(75)}$ is 4,406,400 on Mainnet
and 4,476,000 on Testnet. Let $H_2^{(75)}$ be the corresponding second
halving height in the table below. In this range, $b = 26{,}041{,}666$,
$R_{\max} = 189{,}393{,}927{,}272{,}727$, and
$R_0 = 262{,}499{,}975{,}638{,}334 - (A - H_2^{(75)})$ zatoshi.
Consequently $\lceil (R_0 - R_{\max})/b \rceil = 2{,}807{,}273$
throughout the range. Including the one-block offset from $H_3$ to
$h_0$ gives
$\mathsf{DEPLOYMENT\_BLOCK\_HEIGHT} = H_3 + 2{,}807{,}274$:
...
For example, an illustrative Mainnet activation height of $A = 3{,}543{,}000$
gives a third halving at 6,133,200 and a reissuance height of 8,940,474.
At height 8,940,473 the rounded reference subsidy is 26,041,667 zatoshi,
above the scheduled 26,041,666; at 8,940,474 it is 26,041,663, below it.
That example does not assign an NU7 activation height.
...
![A graph showing a comparison of the current halving-based step function vs the halving-preserving NSM issuance](../rendered/assets/images/zip-0237-block_subsidy.png)

The graph below shows the balance of the Money Reserve assuming this ZIP is
implemented, under the same removal schedule.

![A graph showing the balance of the Money Reserve assuming halving-preserving issuance is implemented](../rendered/assets/images/zip-0237-balance.png)
...
This ZIP is proposed to activate with Network Upgrade 7 [^zip-0259], with
reissuance beginning at
