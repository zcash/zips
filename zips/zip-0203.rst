...
Maximum: 499999999. The wall-clock time corresponding to this absolute height
depends on the block target spacing in effect before it is reached.
...
At NU7 activation, ZIP 218 [#zip-0218]_ recommends that implementations
creating transactions change the default expiry delta from 40 to 120 blocks,
which remains approximately 50 minutes at the 25-second target spacing.
An explicitly configured ``-txexpirydelta`` value continues to apply before
and after activation. This changes neither the expiry consensus rule nor
the maximum expiry height above.
