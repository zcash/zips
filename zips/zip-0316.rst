...
* A `Revision 1`_ Unified Address encoded with the ``zu`` prefix MUST NOT
  ...
  the `Typecodes permitted in zu Unified Addresses`_ registry.
...
* Within a single UA or UVK, all HD-derived Receivers, FVKs, and IVKs SHOULD
  ...
  generate independent addresses corresponding to that spending authority).
...
However, as of `Revision 1`_ there are uses for transparent-only UVKs that are
...
.. _`Typecodes permitted in zu Unified Addresses`:

Typecodes permitted in ``zu`` Unified Addresses
''''''''''''''''''''''''''''''''''''''''''''''''

The following table lists the Item Typecodes that are permitted in a
`Revision 1`_ Unified Address encoded with the ``zu`` prefix. Any Item
with a Typecode not listed here MUST NOT be included in a ``zu`` Unified
Address by Producers, and Consumers MUST reject ``zu`` addresses
containing such Items. Experimental Typecodes (see `Metadata Items`_)
are permitted at the discretion of wallets that have opted into the
corresponding experiment; such wallets are responsible for ensuring that
the experiment does not compromise the privacy properties of ``zu``
addresses.
...
.. list-table::
   ...
   * - $\mathtt{0xE0}$
     - Address Expiry Height
     - `Address Expiration Metadata`_
   * - $\mathtt{0xE1}$
     - Address Expiry Time
     - `Address Expiration Metadata`_

The guiding principle for this registry is that a ``zu`` address MUST NOT
contain any Metadata Items whose interpretation in transaction
...
transactions sent to addresses that do not.
...
- Let $I_\mathsf{ovk} = \mathsf{BLAKE2b\text{-}512}(\texttt{"ZIP316\_P2SH\_OVK\_"},\, \mathsf{fvk\_ser}).$
- Let $\mathsf{ovk_{external}}$ be the first $32$ bytes of
  $I_\mathsf{ovk}$ and let $\mathsf{ovk_{internal}}$ be the
  remaining $32$ bytes of $I_\mathsf{ovk}$.
