::
  ...
  Credits: Taylor Hornby
           Stephen Smith
  Status: [Revision 0] Active, [Revision 0.1] Proposed, [Revision 1] Proposed
...
- Provide a “bridging mechanism” to allow shielded wallets to interact
  with wallets that have limited shielded protocol support.
...
* Revision 0.1: This version adds support for `P2SH Viewing Key Items`_
  in Unified Full Viewing Keys and Unified Incoming Viewing Keys, using
  BIP 388 wallet policies [#bip-0388]_. Revision 0.1 uses the same
  Human-Readable Parts as `Revision 0`_. Consumers that do not support
  Revision 0.1 will treat P2SH Items as unrecognised and ignore them.

.. _`Revision 1`:

* Revision 1: This version adds support for `MUST-understand Typecodes`_
  and `Address Expiration Metadata`_. It removes Transparent Receivers from
  Unified Addresses, while still permitting Transparent Items in Unified
  Viewing Keys. For Unified Viewing Keys, it also drops the restriction
  that a UVK must contain at least one shielded Item. It also incorporates
  the `P2SH Viewing Key Items`_ support introduced in `Revision 0.1`_.
...
use are met (i.e. there are no MUST-understand Metadata Items, and
the UA/UVK contains at least one shielded item). At some point when
Revision 1 UA/UVKs are widely supported, this will be unnecessary and
it will be sufficient to always produce Revision 1 UA/UVKs.
...
As of `Revision 1`_, Transparent Receiver Typecodes ($\mathtt{0x00}$
and $\mathtt{0x01}$) MUST NOT be included in a Unified Address by
Producers, and MUST be treated as unrecognised by Consumers. (Prior to
`Revision 1`_, the Priority List also included Typecode $\mathtt{0x01}$
— a Transparent P2SH address, *or* Typecode $\mathtt{0x00}$ — a
...
(Prior to `Revision 1`_, a Receiver Encoding could also be the
...
* As of `Revision 0.1`_, a P2SH Viewing Key Item with Typecode
  $\mathtt{0x01}$ MAY be included in a UFVK or UIVK. Prior to
  `Revision 0.1`_, the Typecode $\mathtt{0x01}$ MUST NOT be included
  in a UFVK or UIVK by Producers, and MUST be treated as unrecognised
  by Consumers.

  A P2SH Viewing Key Item encodes a BIP 388 wallet policy
  [#bip-0388]_ as follows:
  ...
  * $\mathtt{template} : \mathtt{byte[template\_len]}$ — a BIP 388
    descriptor template [#bip-0388]_, encoded as UTF-8. Key placeholders
    in the template use the ``@N`` notation defined by BIP 388. In a
    UFVK, the template MUST use the ``/**`` multipath notation for each
    key placeholder (indicating derivation of both external and internal
    addresses). In a UIVK, the template MUST use the ``/*`` notation
    instead (indicating derivation of external addresses only).
  ...
  * For each of the $\mathtt{n\_keys}$ entries:
    ...
    * $\mathtt{fingerprint} : \mathtt{byte[4]}$ — the BIP 32 master
      key fingerprint [#bip-0032-key-identifiers]_.

    * $\mathtt{depth} : \mathtt{byte}$ — the number of derivation
      path components.

    * $\mathtt{path} : \mathtt{uint32[depth]}$ — the derivation path
      components, each encoded as a 4-byte unsigned little-endian integer.
      Bit 31 is set for hardened derivation.
  ...
  The number of ``@N`` key placeholders in the descriptor template MUST
  equal $\mathtt{n\_keys}.$ Consumers MUST reject P2SH Viewing Key
  Items for which this does not hold, or for which the descriptor
  template is not a valid BIP 388 descriptor template.
...
(Prior to `Revision 1`_, this was only partially achieved if the UA
contained a Transparent P2PKH Address, since the on-chain transaction
graph can potentially be used to link transparent addresses. As of
`Revision 1`_, Transparent Receivers are no longer included in UAs.)
...
* A Unified Address MUST contain at least one shielded Item (Typecodes
  $\mathtt{0x02}$ and $\mathtt{0x03}$).
...
* A `Revision 0`_ Unified Viewing Key MUST contain at least one shielded
  ...
  is dropped for `Revision 1`_ UVKs; however, a `Revision 1`_ UVK MUST
  contain at least one non-Metadata Item.
...
* Prior to `Revision 1`_, for Transparent Addresses, the Receiver
  Encoding does not include the first two bytes of a raw encoding.
...
* Consumers MUST reject Unified Addresses/Viewing Keys in which the
  same Typecode appears more than once, or that contain only Metadata Items.

* Consumers MUST reject Unified Addresses/Viewing Keys that include both
  P2SH and P2PKH Transparent Items.

* As of `Revision 1`_, Consumers MUST reject Unified Addresses that include
  any Transparent Receiver (Typecodes $\mathtt{0x00}$ and $\mathtt{0x01}$).
...
However, as of `Revision 1`_ there are uses for transparent-only UVKs
that are not covered by the existing formats. In particular, they can
use Metadata Items to represent expiration heights/dates as described in
`Address Expiration Metadata`_.

Note that this restriction is *not* dropped for Unified Addresses; as of
`Revision 1`_, Transparent Receivers are no longer permitted in Unified
Addresses at all (see `Rationale for removing Transparent Receivers from Unified Addresses`_).

.. raw:: html

   </details>

.. _`Rationale for removing Transparent Receivers from Unified Addresses`:

Rationale for removing Transparent Receivers from Unified Addresses
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Removing Transparent Receivers from Unified Addresses simplifies the
security model for UA recipients. Transparent addresses do not provide
any privacy protection, and including them in UAs created a risk that
Senders would fall back to transparent transfers even when the Recipient
supports shielded protocols. By ensuring that Unified Addresses contain
only shielded Receivers, all transfers to a Unified Address will benefit
from the privacy properties of shielded protocols.
...
As of `Revision 0.1`_, to satisfy the above properties for transparent
P2SH keys, we derive the external and internal $\mathsf{ovk}$ components
from the P2SH FVK (described in `P2SH Viewing Key Items`_) as follows:

- Let $\mathsf{raw\_keys}$ be the concatenation of
  $\mathsf{c}_i \,||\, \mathsf{ser_P}(\mathsf{pk}_i)$ for each key
  $i$ in the key information vector, in the order they appear.
- Let $\mathsf{ovk\_seed} = \mathsf{BLAKE2b\text{-}256}(\texttt{"Zcash\_P2SH\_OVK\_"},\, \mathsf{raw\_keys}).$
- Let $I_\mathsf{ovk} = \mathsf{PRF^{expand}}_{\mathsf{ovk\_seed}}\big([\mathtt{0xd1}] \,||\, \mathsf{template}\big)$
  where $\mathsf{template}$ is the descriptor template encoded as UTF-8.
- Let $\mathsf{ovk_{external}}$ be the first $32$ bytes of
  $I_\mathsf{ovk}$ and let $\mathsf{ovk_{internal}}$ be the
  remaining $32$ bytes of $I_\mathsf{ovk}$.
...
UIVK that will be included in the derived Unified Address. That is,
...
* A Transparent diversifier index MUST be in the range $0$ to
  $2^{31} - 1$ inclusive. (As of `Revision 1`_, Transparent Receivers
  are not permitted in Unified Addresses, so this constraint no longer
  applies to UA derivation.)
...
As of `Revision 1`_, Transparent IVK Items (including both P2PKH and
P2SH) MUST NOT be included in the derived Unified Address.

Prior to `Revision 1`_, for a Transparent P2PKH IVK, the diversifier
index was used as a BIP 44 child key index at the Index level
[#bip-0044-path-index]_ to derive the corresponding Transparent P2PKH
Receiver.

Prior to `Revision 1`_, for a P2SH IVK (as of `Revision 0.1`_), the
corresponding P2SH Receiver is obtained by:
...
* In this variant, part b) above is replaced by the meaning of the new
  ...
  or to insert a Transparent Address into a UA, that would be a significant
  malleability attack.
...
adversary (e.g. it would lead to the Sender using a less-preferred Receiver).
...
This allows for a UVK containing only a Transparent P2PKH Viewing Key:

* Transparent P2PKH Viewing Key Item:
...
Allowing only a Transparent P2PKH Viewing Key is consistent with dropping
the requirement to have at least one shielded Item in Revision 1 UVKs
(`see rationale <#rationale-for-dropping-the-at-least-one-shielded-item-restriction-for-uvks>`_).
...
As of `Revision 1`_, a UA containing a Sapling Address and an Orchard
