...
For transparent P2SH multisig keys as specified in ZIP 48 [#zip-0048]_,
a Full Viewing Key consists of a threshold $t$ and $N$ Account-level
extended public keys $(\mathsf{c}_i, \mathsf{pk}_i)$ at path
``m/48'/coin_type'/account'/133000'``. $t$ and $N$ MUST satisfy
$1 \le t \le N \le 255$. We define the canonical serialization of this
FVK as
$\mathsf{fvk\_ser} = [t] \,||\, [N] \,||\, \mathsf{c}_{\pi(0)} \,||\, \mathsf{ser_P}(\mathsf{pk}_{\pi(0)}) \,||\, \cdots \,||\, \mathsf{c}_{\pi(N-1)} \,||\, \mathsf{ser_P}(\mathsf{pk}_{\pi(N-1)})$
where $\pi$ is the permutation that sorts keys in ascending lexicographic
order of $\mathsf{ser_P}(\mathsf{pk}_i)$ (consistent with the
``sortedmulti()`` ordering specified in BIP 383 [#bip-0383]_),
$[t]$ and $[N]$ are single bytes, and each
$(\mathsf{c}_i, \mathsf{pk}_i)$ pair is $65$ bytes ($32$-byte chain code
concatenated with $33$-byte compressed public key).

The external and internal $\mathsf{ovk}$ components are then derived as
follows:

- Let $K = \mathsf{BLAKE2b‐256}(\texttt{"Zcash\_ZIP48MsOVK"},\, \mathsf{fvk\_ser})$.
- Let $I_\mathsf{ovk} = \mathsf{PRF^{expand}}_{\mathsf{LEOS2BSP}_{256}(K)}\big([\mathtt{0xd1}]\big)$.
- Let $\mathsf{ovk_{external}}$ be the first $32$ bytes of
  $I_\mathsf{ovk}$ and let $\mathsf{ovk_{internal}}$ be the
  remaining $32$ bytes of $I_\mathsf{ovk}$.

The two-step derivation is necessary because $\mathsf{PRF^{expand}}$ requires a
$256$-bit key, while the FVK serialization for a multisig wallet is
variable-length. The BLAKE2b-256 step derives a fixed-size PRF key from the
variable-length multi-party key material.
...
.. [#bip-0383] `BIP 383: Multisig Output Script Descriptors <https://github.com/bitcoin/bips/blob/master/bip-0383.mediawiki>`_
