...
``m/48'/coin_type'/account'/133000'``, sorted in ascending
lexicographic order of $\mathsf{ser_P}(\mathsf{pk}_i)$ (consistent
with the ``sortedmulti()`` ordering specified in BIP 383 [#bip-0383]_).
$t$ and $N$ MUST satisfy $1 \le t \le N \le 255$.

We define the canonical serialization of this FVK as

.. math::

  \mathsf{fvk\_ser} = [\mathtt{0x00}, t, N] \,||\, \mathsf{c}_0 \,||\, \mathsf{ser_P}(\mathsf{pk}_0) \,||\, \ldots \,||\, \mathsf{c}_{N-1} \,||\, \mathsf{ser_P}(\mathsf{pk}_{N-1})
...
- Let $I_\mathsf{ovk} = \textsf{BLAKE2b‐512}(\texttt{“ZcashIP316TmsOVK”}, \mathsf{fvk\_ser})$.
- Let $\mathsf{ovk_{external}}$ be the first $32$ bytes of
  $I_\mathsf{ovk}$ and let $\mathsf{ovk_{internal}}$ be the
  remaining $32$ bytes of $I_\mathsf{ovk}$.
