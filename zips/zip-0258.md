...
* ZIP 209: ... [^zip-0209] (updated)
* ZIP 213: ... [^zip-0213] (updated)
...
* ZIP 317: Proportional Fee Mechanism [^zip-0317] (updated)
...
> If the chain value pool balance for any of the *Sprout*, *Sapling*, *Orchard*,
> *Ironwood*, *transparent*, or *deferred development fund pools* would become
> negative in the block chain created as a result of accepting a block, then all
...
Many changes are required throughout the specification to disambiguate pools from protocols
(some of these were done previously in version 2026.6.4), and to add the *Ironwood pool* as
another use of the Orchard protocol. Again these are not spelled out here.

The following consensus rules are added, each an *Ironwood-pool* analog of an existing
*Orchard-pool* (or *Sapling-pool*) rule:

* The *Ironwood-pool* Action transfers of a transaction MUST be consistent with its
  `valueBalanceIronwood` value (as for `valueBalanceOrchard`).
* The `anchorIronwood` field, whenever it exists (i.e. when there are any *Ironwood-pool*
  Action descriptions), MUST refer to some earlier block's final *Ironwood-pool* treestate
  (as for `anchorOrchard`).
* A block MUST NOT add *Ironwood-pool* note commitments that would cause the *Ironwood-pool*
  note commitment tree to exceed its capacity of $2^{\mathsf{MerkleDepth}^{\mathsf{Orchard}}}$
  leaf nodes (as for the *Orchard pool*).
* Any *Ironwood-pool* output of a coinbase transaction decrypted to a note plaintext MUST
  have note plaintext lead byte `0x03` (the corresponding rule for the *Sapling* and
  *Orchard pools* requires `0x02`).
...
ZIP 2005 [^zip-2005] activates at NU6.3 on each network. The *Ironwood pool* is created at
NU6.3 activation; from that point onward, every *Ironwood-pool* output note uses the
quantum-recoverable note plaintext format (lead byte `0x03`) defined in ZIP 2005.
...
[^protocol-chainvalue]: [Zcash Protocol Specification, Version 2026.8.0 [NU6.3] or later. Section 4.17: Chain Value Pool Balances](protocol/protocol.pdf#chainvaluepoolbalances)
