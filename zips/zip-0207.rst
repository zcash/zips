::
  ...
  Status: [Revision 0: Canopy, Revision 1: NU6] Final, [Revision 2: NU7] Draft
  Category: Consensus
  Created: 2019-01-04
...
Revision 2 preserves the approximate duration and value of each address
period when NU7 reduces the target block spacing to 25 seconds.
...
* Revision 2: Extends the remaining address period at NU7 activation and
  subsequent address periods in proportion to the change in target block
  spacing specified in ZIP 218 [#zip-0218]_.
...
As of `Revision 2`_, the definition of $\mathsf{AddressPeriod}$ above is
replaced on each network by the following piecewise definition. Let $A$ be
the NU7 activation height on that network, let
$R = \mathsf{NU7PoWTargetSpacingRatio} = 3$ as defined in ZIP 218
[#zip-0218]_, let $C = \mathsf{AddressChangeInterval}$, let
$I = \mathsf{PostBlossomHalvingInterval}$, and let
$H_1 = \mathsf{HeightForHalving}(1)$.
...
.. math::
    ...
    \mathsf{AddressPeriod}(\mathsf{height}) =
        \begin{cases}
            \mathsf{floor}\left(
                \dfrac{\mathsf{height} + I - H_1}{C}
            \right), &\textsf{if } \mathsf{height} < A \\
            \mathsf{floor}\left(
                \dfrac{R(A + I - H_1) + \mathsf{height} - A}{RC}
            \right), &\textsf{otherwise}
        \end{cases}

The $\mathsf{FundingStream[FUND].AddressIndex}$ and
$\mathsf{FundingStream[FUND].Address}$ calculations above MUST use this
definition for funding streams active at or after NU7 activation, including
when calculating the period at a stream's start height. This preserves the
address index at activation. If NU7 activates partway through an address
period, its remaining block count is tripled; each subsequent period lasts
105000 blocks. At the 25-second target spacing, each period therefore
retains approximately its previous wall-clock duration and funding amount,
...
Revision 2 is intended to activate with NU7 as specified in ZIP 259
[#zip-0259]_.
...
- Drafted Revision 2 for the NU7 address-period schedule.

2025-08-19
----------

- Defined Revision 0 and Revision 1 explicitly.
...
- Applied ZIP 2001 [#zip-2001]_ to this ZIP (implicitly defining Revision 1).

2020-05-30
----------

- Released Revision 0 (status changed from Draft to Proposed).
