...
Full node implementations MUST track an additional :math:`\mathsf{PoolValue}_{Deferred}`
chain value pool balance, in addition to the Sprout, Sapling, and Orchard
...
funding streams with a recipient of :math:`\mathsf{DEFERRED}\_\mathsf{POOL}`
in the block at height :math:`\mathsf{height}`.

The :math:`\mathsf{PoolValue}_{Deferred}` chain value pool balance for a given
block chain is the sum of the values of payments to
...
is that payments to the :math:`\mathsf{DEFERRED}\_\mathsf{POOL}` cause
