...
$\mathsf{DEFERRED}\_\mathsf{POOL}$. The latter, added in the NU6 network
...
funding streams with recipient identifier $\mathsf{DEFERRED}\_\mathsf{POOL}$
...
$\mathsf{DEFERRED}\_\mathsf{POOL}$ for transactions in the block chain.
...
- In each block with coinbase transaction $\mathsf{cb}$ at block height
  ...
  $\mathsf{DEFERRED}\_\mathsf{POOL}$ given by
...
above is that payments to the $\mathsf{DEFERRED}\_\mathsf{POOL}$ cause
