...
- $\mathsf{Issuance.\!MKGDomain} := \texttt{"ZcashSA_Issue_V1"}$
...
It is possible for the $\textit{PubKey}$ algorithm to fail with very low probability, which means that $\mathsf{IssueAuthSig}.\!\mathsf{DerivePublic}$ could return $\bot$ with very low probability.
...
Note that $\mathsf{IssueAuthSig}.\!\mathsf{Sign}$ could return $\bot$ with very low probability.
