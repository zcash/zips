...
* All *Orchard-pool* funds, except a residual smaller than the dust floor, are
  eventually transferred to the *Ironwood pool*.
...
* **Provably leak-free migration.** The procedure does not have to provably leak
  ...
  and a residual amount below the dust floor is left unmigrated.
...
1. **Note preparation** (see [Phase 1, Note preparation](#phase1notepreparation)):
   one or more wallet-internal send-to-self transactions that restructure the
...
The purpose of note preparation is to ensure that a note of exactly the right
value exists on-chain for every part of the schedule, so that every part can be
funded independently of the others and the entire schedule can be pre-signed at
once: a part whose funding note does not yet exist cannot be pre-signed, because
the note has no witness. Whether preparation divides larger notes into smaller
ones, combines smaller notes into larger ones, or both, is purely a function of
the wallet's existing note distribution, and the distinction is not visible
on-chain (see
[Rationale for the canonical transaction shape](#rationaleforthecanonicaltransactionshape)).
...
* The output notes of preparation are determined by the quantization of
  [Amount selection, canonical quantization](#amountselectioncanonicalquantization):
...
* Where the number of notes involved exceeds the per-transaction action bound
  (an open point; see
  [Open questions for Phase 1](#openquestionsforphase1)), preparation SHOULD be
  partitioned into multiple transactions with disjoint input notes, so that the
...
* If notes are not prepared before the *Ironwood pool* activates, the wallet
  ...
  transactions funded by change from earlier ones cannot be pre-signed in
  advance and incur longer waits while that change confirms. (Change-bearing
  parts carry no privacy penalty; see
  [Rationale for the canonical transaction shape](#rationaleforthecanonicaltransactionshape).)

* Exact-value funding notes bake the canonical fee into note values at
  preparation time; a change to the canonical fee before broadcast requires the
  affected parts to be rebuilt or the remaining balance to be re-prepared (see
  [Error handling](#errorhandling)).

* The wallet MUST wait for a note-preparation transaction to confirm before
  treating its output notes as available for migration, consistent with the
  confirmation requirements for trusted TXOs in ZIP 315. [^zip-0315]

### Open questions for Phase 1

* The maximum number of actions in a single note-preparation transaction — and
  hence when preparation must be partitioned across multiple transactions, for
  example for balances on the order of $10^6$ ZEC or for wallets holding very
  many small notes — is an open point.

* The fee accounting for note preparation itself, whose ZIP 317 fee [^zip-0317]
  scales with its action count and therefore with the shape of the wallet's
  existing note distribution, and its interaction with the deterministic
  quantization of
  [Amount selection, canonical quantization](#amountselectioncanonicalquantization),
  is an open point for the Zcash core team.
...
* The quantization leaves a deterministic residual of less than `DUST_FLOOR`
  (the balance modulo the smallest denomination) unmigrated; this residual
  remains in the *Orchard pool* at the end of the migration. Notes individually
  smaller than `DUST_FLOOR` are nonetheless migratable, because note preparation
  may combine them; whether consuming a given small note is economic is a
  ZIP 317 marginal-fee consideration. [^zip-0317]
...
equivalently the *Ironwood* output equals the denomination). In the recommended
flow each part is funded by a single note of exactly this value, produced by
note preparation (see [Phase 1, Note preparation](#phase1notepreparation)).
...
* **Amount-selection / note-preparation heuristic**: canonical power-of-ten
  quantization is the recommended baseline; the exact decomposition, `DENOM_CAP`,
  `DUST_FLOOR`, and fee accounting (the canonical fee per part, and the
  action-count-dependent fee of note preparation itself) remain to be ratified.
...
* **Note-preparation transaction bounds**: the maximum number of actions in a
  single note-preparation transaction, and the partitioning of preparation across
  multiple parallel transactions for very large balances or note counts.
