...
upgrade [^zip-0258]. It uses the same Orchard shielded protocol as the *Orchard
pool* [^zip-0229], and exists for two reasons. First, its notes use a
...
counterfeiting has taken place. Because ordinary transfers within the old
*Orchard pool* become restricted once NU6.3 activates, migrating to the
*Ironwood pool* is effectively necessary rather than optional, and wallets
...
wallet MAY instead use any amount-selection strategy that achieves the same broad
...
The migration runs as a two-phase process: a note-preparation phase followed by a
...
- Because the old pool can no longer carry ordinary payments, post-upgrade it
  contains little other than *Orchard*→*Ironwood* pool-crossing spends (whether
  generated by this flow or made manually) and wallet-internal restructuring
  transactions. The canonical transaction shape in this document is chosen so
  that automated migration transactions are indistinguishable from manual
  cross-pool spends; this strengthens the ZIP 308 stance that migration
  transactions need only be hidden **among themselves** into hiding among the
  whole population of conforming cross-pool spends (see
  [Non-requirements](#non-requirements) and
...
* **Provably leak-free migration.** The procedure does not have to provably leak
  ...
  of canonical-shaped *Orchard*→*Ironwood* spends
  (see [Consensus context](#consensuscontext)), and a residual amount below the
  dust floor is left unmigrated.
