...
* Buckets are delimited by **boundaries**: blocks whose height is
  ...
  $144 \times 75\,\text{s} = 3$ hours. A longer interval increases privacy; a
  shorter interval speeds up the migration. `EXPIRY_MODULUS` (34560 blocks) is
  an exact multiple of $M$ (240 buckets), so expiry buckets align with anchor
  buckets.
...
* All randomness used in scheduling — the shuffle and the delay draws — and in
  the randomized decomposition of
  [Amount selection, canonical quantization](#amountselectioncanonicalquantization)
...
* Each migration transaction takes the canonical anchor determined by the
  bucket from which it is actually broadcast (see
  ...
  data rather than in the signature hash [^zip-0229], the wallet can update a
  pre-signed transaction's anchor and proof without re-signing.

* Whenever the application is granted background processing time, the wallet
  SHOULD synchronize and update the anchors and proofs of upcoming scheduled
  transactions to the most recent boundary anchor, but MUST NOT broadcast a
  migration transaction within the same background processing window in which
  a synchronization is performed (see
  [Decoupling synchronization from broadcast](#decouplingsynchronizationfrombroadcast)).

* Spreading a wallet's transfers across several buckets, with memoryless
  inter-transfer delays, is the primary means by which the wallet's own
  transfers are kept from being linked to one another; distinct per-wallet
  anchor heights are **not** required, because the shared boundary anchor
  already carries no per-wallet information.
...
* Under the version 6 transaction format, the anchor is committed as
  ...
  to be signed in a single session while each transaction still carries the
  fresh shared boundary anchor of the bucket from which it is broadcast (see
...
* Where a synchronization is needed, it MUST be separated in time from any
  ...
  *Orchard-pool* change, so the required synchronizations are the one that
  ...
  send, and the routine background synchronizations that keep the anchors and
  proofs of upcoming transactions current (see
...
minutes) would compress a wallet's entire migration into one or two buckets:
the transfers would share a tight, predictable window, and the burst itself
would be a clustering signal. With `MEAN_DELAY` equal to the bucket modulus
($M$ = 144), a wallet contributes on average about one transfer to each
bucket it participates in, and a typical migration spreads across several
...
anchor is pinned to the previous boundary, a value identical for every wallet
sending at that boundary, making anchor age constant and non-identifying.
...
* **Anchor bucket modulus $M$**: provisionally 144 blocks (3 hours, equal to
  `MEAN_DELAY`). Tightening $M$ by a factor $c$ speeds migration ~$c\times$ but
