...
* The migration process, by default, preserves the following privacy
  ...
  * **Holdings-concentration privacy.** The migration does not reveal the 
    number of users that hold funds in the the *Orchard pool* or how funds are
...
* The anchor height of each migration transaction MUST be a shared,
  ...
  own chain tip, so that the anchor does not reveal when this wallet last
  synchronized.
...
* Where a synchronization is needed, it MUST be separated in time from any
  ...
  *Orchard-pool* change, so the required synchronizations are the one that
  ...
  does output change back to the *Orchard pool* (for example, one rebuilt
  during error recovery) requires the wallet to synchronize before that
  change can be spent; the wallet MUST schedule that synchronization
  decoupled in time from the broadcast of the next migration transaction.
...
* When a window is missed and the remaining transfers are rescheduled, the wallet
  SHOULD shift the remaining schedule by the offset, or redraw the delays of
...
* **Anchor bucket modulus $M$**: provisionally 144 blocks (3 hours, equal to
  `MEAN_DELAY`). Because broadcast pacing is set by the schedule and anchors
  are drawn from past boundaries, $M$ does not affect migration speed;
  tightening it by a factor $c$ divides the per-anchor cohort size by ~$c$
  unless concurrent migrator volume rises by ~$c$. $M$ SHOULD be derived from
  a target anonymity-set floor based on expected *concurrent* migrator volume,
  not chosen arbitrarily.
* **Anchor age distribution**: provisionally geometric with parameter 1/2
  over ages $a \geq 1$ (excluding the most recent boundary), capped at
  `ANCHOR_AGE_CAP` = 16 boundaries; the parameter, the cap, and the exclusion
  of the most recent boundary require the same analysis and simulation as $M$
  and `MEAN_DELAY`.
* **Transfer-scheduling delays**: `MEAN_DELAY` (provisionally 144 blocks) and
  `MAX_DELAY` (provisionally 576 blocks) require analysis against the bucket
  modulus $M$ and expected cohort sizes.
* **Target signing sessions and `K_MAX`**: provisionally ~6 sessions; the
  ...
  size. The meaning of a "typical balance" also needs definition. Under the
  Poisson schedule, per-cohort multiplicity emerges from the delay draws
  rather than being chosen directly, so the relationship between the schedule
  and the `K_MAX` bound needs to be specified.
* **Expiry bucketing and dedup/rebroadcast semantics**: `EXPIRY_MODULUS`
  (provisionally 34560 blocks, a 30-day bucket giving a one-to-two-month
  rolling validity window) requires ratification; dedup/rebroadcast semantics
