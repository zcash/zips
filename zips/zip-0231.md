...
This weighting ensures that a memo recipient learns only (a) the number of
chunks in their own memo and (b) the total number of chunks they cannot decrypt,
but gains no information about how the undecryptable chunks are distributed
among other memos.
...
fashion will be indistinguishable how from real encrypted memo chunks appear to
an observer who does not hold the memo key for those chunks.
...
- The fee algorithm could be modified to be a superlinear function of the
  number of memo blocks, in order to economically discourage the use of
