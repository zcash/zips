...
* For V7-onward note ciphertexts, the KDF used to derive the note encryption key
  ...
  the transaction version group id ($\mathsf{nVersionGroupId}$):
    ...
    to a wallet as though it were a partial v5 or v6 ciphertext, which could otherwise cause
...
v5 or v6 note ciphertext. Without domain separation, a wallet that supports the
v5/v6 partial-ciphertext optimization (downloading note ciphertexts without the
MAC to save bandwidth) could decrypt a V7 ciphertext without verifying its
