...
Recipients should not treat unsigned memos as trustworthy unless they can
reliably verify its origin through other means. In a peer-to-peer transaction
Bob and Alice can split a lunch check. Bob can send a shielded memo to Alice
that says "Nice Lunch! It was fun! this is my half + tips. Hugs, Bob.". Bob
would show Alice his transaction detail in his wallet and Alice can visibly
...
obtain Alice's address could attempt to impersonate Bob and send Alice the
message, _"Actually, can you bail me out on this one, I came a bit short on
ZEC? Reply-to u1....."_. Previous to this specification the inability to
accomodate authentication data in memos facilitates a social-engineering attack
like this. If Alice's wallet could have authenticated Bob's original message it
would have make it possible for the wallet's UI to clearly help Alice to
distinguish Bob's message from Mallory's. 

This becomes more evident for the case of software using memo data as input for
business logic. The most widely used mobile wallets use remote configuration
extensively to display different features dynamically based on parameters
defined remotely by their product team, such as promotional events, A/B testing
or even disabling a feature bacause of a technical outages or regional
regulations. Moreover, remote configuration makes features like
"forced-upgrade" possible by allowing applications to force users to update to
a newer version before being able to continue without having to hardcode such
End-of-Support parameters in the distributed software. Remote configuration
pulling has privacy downsides since servers learn which IPs have launched the
application and other information. A privacy-centric alternative can be
implemented using authenticated payloads with memo bundles. Application
developers hardcode a viewing key into their application bundle with which the
application can receive trusted payloads. Developers sign them with their
spending key making the payloads contained in the memo bundles _trusted_ and
_actionable_ pieces of information. Nothing stops attackers from being able to
reverse engineer the application and extract the viewing key to attempt to
spoof a remote configuration payload. Given that they can't sign the malicious
payload with a verifiable signature, the application can just ignore such
messages. Moreover the "asynchronous push" nature of the configuration payload
delivery through the Zcash blockchain lets the application sync the remote
configuration data over Tor or NYM infrastructure in a decentralized manner
...
v6-onward note ciphertext to 100 bytes. This makes it practical for the light
client protocol to transmit complete note ciphertexts, including the AEAD tag,
in exchange for a relatively minor bandwidth increase compared to the current
...
To mitigate this, a transaction with any shielded outputs is required to include
at least 2 memo chunks in its memo bundle (see [Memo bundle padding]). This
ensures that the common use case of sending a transaction without including
explicit memo data is indistinguishable from sending a transaction with a short
memo, as is currently supported by all wallets in common use.
...
be possible to require transactions be padded to 4 chunks to ameliorate this
potential distinguisher, this introduces an unreasonable amount of waste in the
case of ordinary transactions.
...
be used to encrypt more than one memo within a single transaction. To share a
memo among multiple recipients, the same $\mathsf{K^{memo}}$ value is placed in
each recipient's note plaintext; this does not violate the above constraint
because all recipients decrypt the same memo. If an output has no memo data, it
is assigned the memo key consisting of 32 $\mathtt{0xFF}$ bytes.
...
If a randomly generated $\mathsf{K^{memo}}$ is 32 $\mathtt{0xFF}$ bytes, the
transaction constructor MAY generate a new random key, in order to avoid the
recipient misinterpreting the output as having no memo data. Since that has
negligible probability, it alternatively MAY omit this check.
...
In pre-v6 transactions, trial decryption of each shielded output performs
constant work regardless of whether the output belongs to the wallet: the AEAD
decryption of the full 580-byte note ciphertext costs the same whether it
succeeds or fails.
...
trial decryptions that could be exploited by a network adversary to determine
whether a node's wallet recognized a particular output.

To mitigate this, wallet implementations SHOULD defer memo bundle decryption to
an asynchronous process that is independent of the trial decryption of note
ciphertexts. This decorrelates the observable timing of block processing from
whether any notes were successfully decrypted, making it difficult for an
adversary to attribute memo decryption work to a specific transaction.

Alternatively, an implementation MAY perform memo chunk decryption with a
dummy key for every output that fails trial decryption, ensuring constant work
per output at the cost of additional computation.
...
- The 64-chunk limit could be modified to be either smaller or larger. However,
  as Orchard authenticated reply-to address proofs are approximately 4
  kilobytes in size, the limit needs to be sufficient to permit this use case.
