...
Each Orchard note plaintext consists of

.. math:: (\mathsf{leadByte} : \mathbb{B}^{\mathbb{Y}}, \mathsf{d} : \mathbb{B}^{[\ell_{\mathsf{d}}]}, \mathsf{v} : \{0 .. 2^{\ell_{\mathsf{value}}} - 1\}, \mathsf{rseed} : \mathbb{B}^{\mathbb{Y}[32]}, \mathsf{asset\_base} : \mathbb{B}^{[\ell_{\mathbb{P}}]}, \mathsf{K^{memo}} : \mathbb{B}^{\mathbb{Y}[32]})

The encoding of an Orchard note plaintext [#protocol-noteptencoding]_ is updated and is now defined as:

$[\mathsf{leadByte}] \,||\, \mathsf{LEBS2OSP}_{88}(\mathsf{d}) \,||\, \mathsf{I2LEOSP}_{64}(\mathsf{v}) \,||\, \mathsf{rseed} \,||\, \mathsf{LEBS2OSP}_{256}(\mathsf{repr}_{\mathbb{P}}(\mathsf{AssetBase})) \,||\, \mathsf{K^{memo}}$

* The $\mathsf{leadByte}$ MUST be $\mathtt{0x03}$ for Orchard note plaintexts in V6 transactions. 
* The $\mathsf{AssetBase}$ of the note, as defined in ZIP 226 [#zip-0226]_, is added to the Orchard note plaintext.
* The $\mathsf{K^{memo}}$ field replaces the 512-byte $\mathsf{memo}$ field in the Orchard note plaintext, containing the memo key for the note, as defined in ZIP 231 [#zip-0231-memo-encryption]_.
