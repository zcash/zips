...
The encoding of an Orchard note plaintext [#protocol-noteptencoding]_ is updated and now consists of the following fields:
...
* The $\mathsf{leadByte}$ MUST be encoded to be $\mathsf{0x03}$ for Orchard note plaintexts in V6 transactions. 
* The $\mathsf{AssetBase}$ field is added to the Orchard note plaintext, containing the encoding of the Asset Base of the note, as defined in ZIP 226 [#zip-0226]_.
* The $\mathsf{K^{memo}}$ field replaces the 512 byte $\mathsf{memo}$ field in the Orchard note plaintext, containing the encoding of the memo key for the note, as defined in ZIP 231 [#zip-0231-memo-encryption]_.
...
.. [#protocol-noteptencoding] `Zcash Protocol Specification, Version 2024.5.1 [NU6]. Section 5.5: Encodings of Note Plaintexts and Memo Fields <protocol/protocol.pdf#noteptencoding>`_
