...
and motivated in [#draft-ecc-quantum-recoverability]_.
...
ZIP 226 [#zip-0226-orchardzsa-transaction-structure]_, and draft-ecc-quantum-recoverability
[#draft-ecc-quantum-recoverability]_ (which defines the necessary changes to the
Zcash protocol specification). Note that the changes arising from [#zip-0231]_
apply also to Sapling outputs.
...
as explained in [#draft-ecc-quantum-recoverability-security-analysis]_. They
also have a new $\mathsf{AssetBase}$ field used to support ZSAs.
Therefore, they cannot be validated or spent without making *all* of the
specified changes. A wallet attempting to non-conformantly use the note
decryption algorithm for lead byte $\mathtt{0x02}$ would compute a different
note commitment $\mathsf{cm}_x$ and would reject the note.
...
.. [#draft-ecc-quantum-recoverability] `draft-ecc-quantum-recoverability: Quantum Recoverability <draft-ecc-quantum-recoverability.md>`_
.. [#draft-ecc-quantum-recoverability-security-analysis] `draft-ecc-quantum-recoverability: Quantum Recoverability — Security Analysis <draft-ecc-quantum-recoverability#security-analysis>`_
