...
The terms "V7 transaction format" and "V7 transaction" are to be interpreted as
defined in ZIP 248. [#zip-0248]_


Abstract
========

This proposal makes the transaction fee explicit in the V7 transaction format,
...
This ZIP proposes to register bundle type 5 variant 0 ("Transaction fee") in
the V7 transaction bundle type registry defined in ZIP 248 [#zip-0248]_.

+------------+---------------+----------------------+--------------------+------------------+
| BundleType | BundleVariant | ``mValuePoolDeltas`` | ``mEffectBundles`` | ``mAuthBundles`` |
+============+===============+======================+====================+==================+
| 5          | 0             |✅                    |❌                  |❌                |
+------------+---------------+----------------------+--------------------+------------------+
...
``bundleType = 5`` and ``assetClass = 0`` (ZEC).
...
ZIP 236 [#zip-0236]_), add the following consensus rules:
  ...
  * [NU7 onward] For non-coinbase V7 transactions, the value of
  ...
  * [NU7 onward] For coinbase V7 transactions, the value of
    $\mathsf{mValuePoolDeltas}[(\mathsf{FeeBundleId}, \mathsf{Zec})]$ MUST be
...
Since the fee bundle (bundle type 5) has no effecting data and no authorizing
...
which is assumed to be V7. [#zip-0248]_
