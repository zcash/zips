...
TODO: Specify how P2MS scripts are constructed for Zcash addresses and transactions,
referencing the following BIPs:

- BIP 379 [^bip-0379]: Miniscript
- BIP 380 [^bip-0380]: Output Script Descriptors General Operation
- BIP 381 [^bip-0381]: Non-Segwit Output Script Descriptors
- BIP 383 [^bip-0383]: Multisig Output Script Descriptors
  - Implies BIP 67 [^bip-0067]
- BIP 388 [^bip-0388]: Wallet Policies for Descriptor Wallets

Below here is proposed specification text:
...
The following BIP 388 [^bip-0388] descriptor template produces a 2-of-3 P2SH multisig
wallet compatible with this ZIP:
...
multisig policy, wallets need to produce a sequence of P2SH addresses that can be given
out for receiving funds, and scanned to detect incoming payments. This specification
adopts BIP 48 [^bip-0048] for this purpose, with the following modification:

- The only defined $\mathit{script\_type}$ is $\mathtt{0x133000}$ for “Zcash P2SH” as
  defined in this specification. The examples for Nested Segwit and Native Segwit are
  therefore not applicable.

## Transaction construction

TODO: Anything we need to specify here?


# Rationale

We specify a likely-non-colliding $\mathit{script\_type}$ instead of using either `0` (the gap
left in BIP 48) or `2` (the next unassigned contant in BIP 48), because both of those values
have in the past been proposed and rejected as referring to P2SH. This means that there
is no compatibility benefit to trying to use one or the other, and the likelihood of a
collision with some other use case is higher. Instead we pick a value that is prefixed
(in decimal) with the SLIP 44 [^slip-0044] $\mathit{coin\_type}$ for Zcash mainnet, which is
highly unlikely to organically occur in another related specification.
...
[^bip-0379]: [BIP 379: Miniscript](https://github.com/bitcoin/bips/blob/master/bip-0379.md)
