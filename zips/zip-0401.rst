...
than 40 full blocks’ worth of transactions in the worst case. At the
75-second target spacing after Blossom [#zip-0208]_, this corresponds to
approximately 50 minutes of block production, matching the default
transaction expiry delta in ZIP 203 [#zip-0203]_. At the 25-second target
spacing specified for NU7 in ZIP 218 [#zip-0218]_, it corresponds to
approximately 17 minutes, while the recommended default expiry delta is
120 blocks (approximately 50 minutes). The cost limit bounds node memory;
it does not guarantee that a transaction remains in the mempool until expiry.
...
The recommended default expiry is 40 blocks after Blossom activation
and 120 blocks after NU7 activation [#zip-0203]_ [#zip-0218]_. Both
represent an expected time of 50 minutes. Even if some blocks are slow,
most legitimate transactions are expected to expire within 60 minutes.
An attacker’s transactions cannot be relied on to expire.
