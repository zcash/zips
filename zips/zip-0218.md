...
Note: during the second halving period, the post-Blossom block subsidy is
1.5625 ZEC. At 25-second spacing it becomes
$\mathsf{floor}(156250000 / 3) = 52083333$ zatoshi (0.52083333 ZEC),
so three post-NU7 blocks issue one zatoshi less than one pre-NU7 block.
After the third halving, the subsidy is
$\mathsf{floor}(78125000 / 3) = 26041666$ zatoshi (0.26041666 ZEC).

Let $A$ be the NU7 activation height, and let $H_3$ be the third-halving
height under the 75-second schedule on the relevant network. If NU7
activates between the second and third halvings, the additional shortfall
in scheduled issuance relative to keeping 75-second blocks is
$0.4872 + (H_3 - A)/10^8$ ZEC. The first term is the combined rounding
shortfall from the third and later halving periods; the second is the
one-zatoshi shortfall per remaining 75-second block in the second period.
The existing 75-second schedule falls short of $\mathsf{MAX\_MONEY}$ by
0.1848 ZEC, so the post-NU7 scheduled shortfall is
$0.6720 + (H_3 - A)/10^8$ ZEC. These figures exclude any NSM reissuance.
