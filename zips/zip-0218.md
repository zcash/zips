...
- Introduces action limits. These provide limits on the number of actions that
  can go into a block. There is a global limit shared by all shielded pools,
  and per-pool limits. It is configured to more than double
  Orchard-protocol TPS (2.9 → 6.6 TPS), while lowering the impact
...
- **Greater throughput.** With 3× as many blocks per day and the same
  ...
  double Ironwood TPS for 2-action transactions. Longer term, when we get a
...
- A maximum of 0 Sprout JoinSplits per block.
...
Furthermore, Sprout is being deprecated in NU7 when this proposal goes live,
hence its limit is set to zero.
...
proofs and spend authorization signatures for those ~617 actions.
