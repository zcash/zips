...
The term "TPS" refers to the maximum number of transactions per second
supported by a Zcash subprotocol, assuming the target block spacing.
...
the Sapling, Orchard, and Ironwood shielded pools.

This solves three problems.
- Significantly improves the UX for actors who need 1 or 2 confirmations,
  such as Near Intents, and for small payments. The user-visible latency
  goes down 3x.
- Increases consensus bandwidth, which would amplify the scaling impact
  of any future shielded pools that do not require shielded sync.
- Introduces action limits. These provide limits on the number of actions that
  can go into a block. There is a *block shielded action limit* across all
  shielded pools, as well as per-pool limits. These limits are configured to more
  than double Orchard-protocol TPS (2.9 → 6.6 TPS), while lowering the impact
  a DoS attacker can impose on wallets; for example, maximum shielded sync
  bandwidth for light clients is reduced by 37% (271 → 169 MB/day).
...
- **Greater throughput.** With 3× as many blocks per day and the same
  ...
  capacity. In the short term, when paired with the action limits, we will more than
  double the Ironwood TPS for 2-action transactions. In the longer term, when
  we get a shielded pool with no shielded sync burden, we will have 3x higher
  throughput.
...
same factor, provided that block validation and propagation times remain small
...
while the mean stays roughly the same. Recall that proof-of-work block times
have high variance, so variance reduction is a substantial benefit.
...
yielding far higher Ironwood TPS. We benchmark
[block verification](#block-processing-time), and the uncle-rate post-proposal
...
clients per day, and 4.8M trial decryptions per day [^syncsimulator]. This is
achieved by fully utilizing block space with Sapling transactions that have
few inputs and many outputs. The worst-case DoS for node verification time
uses many Sapling inputs and a single transparent output, totaling 5,600
Sapling inputs and zero-knowledge proofs. This takes 3.2 seconds on a modern
AMD laptop CPU with 4 pinned threads. Regular user flows impose far lower
costs on the network than this DoS model.

We propose introducing block shielded action limits and per-pool action limits as follows:

- A block shielded action limit of 330 across all pools

- A maximum of 330 Orchard actions per block

- A maximum of 330 Ironwood actions per block

- A maximum of 300 Sapling inputs + outputs. (Total inputs + outputs <= 300)

- No Sprout JoinSplits (achieved by deploying this ZIP in conjunction with
  ZIP 2003 [^zip-2003] which disables v4 transactions).
...
bandwidth despite 3× more blocks. This yields a 2.3× increase in Ironwood TPS,
and keeps Sapling TPS at a higher level than today's Ironwood TPS.
...
The reduced Sapling per-block limit and disabling of Sprout is justified by
the current distribution of shielded funds across pools (other motivations for
disabling Sprout are discussed in ZIP 2003). As of May 2026:
...
trend is expected to continue. The Sapling limit is set generously relative
to current usage while substantially reducing the potential for DoS.
Furthermore, this ZIP is proposed to activate together with disabling v4
transactions in NU7, which would prevent any Sprout transactions from being
included in blocks.
...
propagation delay may imply due to hash-power centralization in mining pools.
...
Orchard or Ironwood actions, or ~2,090 Sapling outputs, with no per-pool
limits. A fully packed Orchard-protocol block requires verifying all action
proofs, spend authorization, and binding signatures, and checking validity
of curve point encodings, for those ~617 actions.

**Proposed worst case:** With the action limits, a block contains at
most 330 actions across pools. There are also separate per-pool limits:

- 330 Orchard actions
- 330 Ironwood actions
- 300 Sapling inputs+outputs.

This means the new worst case block processing time, if Orchard- or
Ironwood-dominant, would be 14% of today's worst case, and Sapling's would be
less than one tenth. The worst case per-block verification work is therefore
substantially reduced.

**Batch verification.** Ironwood transaction verification benefits from
batch validation, where proof and signature verification is amortized across
multiple transactions. Ironwood uses the same Action encoding and proof system
as Orchard. [^zip-0229] In current node software, Orchard transactions are
batch-verified in groups of up to 64 transactions (each worst case being a
single action). Zebra has performed batch verification during live network
syncing since version 3.0.0. With the action limits, the Orchard-protocol
bundles in a worst-case block can be fully batch-verified in a small number of
batches.
...
- **Full Sapling limit (proposed)** — verification worst case is all-spends; 
  the bandwidth worst case (all-outputs) is in [Rationale](#rationale).
- **Today's Orchard worst case** — dense multi-action (single-action would
  understate the byte bound).
...
When transactions have already been pre-verified upon entering the mempool,
block validation reduces to checking signatures and state updates, which is even
cheaper. This is the typical case for a node that has been online under normal
usage of the network, but under DoS attack it cannot be assumed, because the
adversary can send blocks with transactions that were never in the mempool.
...
$$\mathsf{OrchardProtocolBlockActionLimit} := 330$$
$$\mathsf{SaplingBlockIOLimit} := 300$$
$$\mathsf{SproutBlockJoinSplitLimit} := 0$$
...
- The total number of Orchard actions across all transactions in the
  block MUST NOT exceed $\mathsf{OrchardProtocolBlockActionLimit}$. That is,
  $\sum_{\mathit{tx} \in \mathit{block}} \mathit{nActionsOrchard}(\mathit{tx}) \leq \mathsf{OrchardProtocolBlockActionLimit}$.

- The total number of Ironwood actions across all transactions in the
  block MUST NOT exceed $\mathsf{OrchardProtocolBlockActionLimit}$. That is,
  $\sum_{\mathit{tx} \in \mathit{block}} \mathit{nActionsIronwood}(\mathit{tx}) \leq \mathsf{OrchardProtocolBlockActionLimit}$.
...
- The total number of Sprout JoinSplits across all transactions in
  ...
  is,
  $\sum_{\mathit{tx} \in \mathit{block}} \mathit{nJoinSplit}(\mathit{tx}) \leq 0$.

**Block shielded budget:**

In addition to the per-pool limits, the total shielded cost across all
pools in a block MUST NOT exceed $\mathsf{BlockShieldedBudget}$. The
shielded cost of a block is defined as:

$$\sum_{\mathit{tx}} \left(\mathit{nActionsOrchard}(\mathit{tx}) + \mathit{nActionsIronwood}(\mathit{tx})\right) \;+\; \sum_{\mathit{tx}} (\mathit{nSpendsSapling}(\mathit{tx}) + \mathit{nOutputsSapling}(\mathit{tx})) \;+\; 2 \times \sum_{\mathit{tx}} \mathit{nJoinSplit}(\mathit{tx}) \;\leq\; \mathsf{GlobalShieldedBudget}$$

Thus, Orchard and Ironwood do not receive separate 330-unit global
budgets: every action in either pool consumes one unit of the same global
budget.

The factor of 2 for Sprout JoinSplits reflects that each
...
lightweight wallets must download for shielded sync. The Light Wallet
Protocol represents both Orchard and Ironwood actions using
`CompactOrchardAction`. [^lightwallet-protocol] The per-action field payload
has the following costs:

**Orchard and Ironwood:** 148 bytes per action, consisting of:
...
- **Orchard or Ironwood:** $330 \times 148 = 48{,}840$ bytes
- **Sapling:** at most $300 \times 116 = 34{,}800$ bytes (all-output
  pathological case)

Due to the global shielded budget, these cannot stack: a block that
uses 330 Orchard and/or Ironwood actions has zero budget remaining for
Sapling. The worst-case compact sync bandwidth per block is
therefore always bounded by the Orchard-protocol case at 48,840 bytes.
...
**Orchard and Ironwood pools:**

| Parameter | Value |
|-----------|-------|
| $\mathsf{OrchardProtocolBlockActionLimit}$ | 330 actions per pool |
...
| Max Orchard + Ironwood actions/block | ~617 (block-size limited) | 330 |
| Max Sapling IOs/block | ~2,140 (block-size limited) | 300 |
| Orchard or Ironwood compact BW/day | ~105.2 MB | 168.79 MB |
...
The binding constraint is Orchard or Ironwood at 330 actions per block:
...
with their IVK assuming other sync trade-offs are taken, but current Sapling
and Orchard-protocol wallets always attempt both.

### Normal transaction throughput

For standard 2-action Orchard or Ironwood transactions, the action limit
of 330 allows $\lfloor 330 / 2 \rfloor = 165$ transactions per block,
giving:

$$\mathsf{orchard\_protocol\_tps} = 165 \;/\; 25 = 6.6 \textsf{ TPS}$$
...
A concern with per-pool limits is that a DoS attacker could fill one
pool's budget to crowd out transactions in another pool. The global
shielded budget prevents this from being worse than filling any single
pool, but it is worth examining whether fee incentives create an
asymmetry.
...
each Orchard or Ironwood action counts as one logical action. The marginal
fee per logical action is the same regardless of pool. Therefore, an
attacker gains no fee advantage from choosing one pool over another. The
cost per unit of shielded budget consumed is identical.

Furthermore, because the global shielded budget is shared, filling one
pool's budget necessarily reduces the remaining budget for all other
pools by the same amount. An attacker who spends their entire budget on
Sapling spam at 300 IOs leaves only 30 Orchard or Ironwood actions
available in that block. But this attack is no cheaper than filling 330
Orchard or Ironwood actions directly, since the per-action fee is the
same. The global budget ensures that the total shielded sync cost per
block is bounded to 330 units regardless of the attacker's pool choice.

The 330-action per-pool cap also allows either Orchard or Ironwood
transactions to use the full global budget when other shielded pools are
not used.
...
[^lightwallet-protocol]: [Zcash Light Wallet Protocol v0.5.0: `compact_formats.proto`](https://github.com/zcash/lightwallet-protocol/blob/v0.5.0/walletrpc/compact_formats.proto)
