...
Let $\mathsf{LockboxDisbursementAmount}$ be the zatoshi amount in the Deferred
Dev Fund Lockbox as of the end of the block preceding the activation height.
($\mathsf{LockboxDisbursementAmount}$ can be predicted in advance given that height.)

The additional coinbase output(s) MUST pay a sum of $\mathsf{LockboxDisbursementAmount}$
zatoshi to the above P2SH multisig address, using a standard P2SH script as
specified in [^Bitcoin-Multisig]. $\mathsf{LockboxDisbursementAmount}$ zatoshi
...
Note: The value $\mathsf{LockboxDisbursementAmount}$ needs to be precalculated
so that it is known at the point when the relevant consensus check is done in
node implementations.

$\mathsf{LockboxDisbursementAmount}$ MUST be 78,750 TAZ (7875000000 tazoshi)
on Testnet.

If this ZIP activates on Mainnet after the end of current dev fund as of
this writing and as specified in [^zip-1015-funding-streams],
$\mathsf{LockboxDisbursementAmount}$ MUST be 78,750 ZEC (7875000000 zatoshi)
on Mainnet.

If this ZIP activates on Mainnet before the end of the current dev fund,
$\mathsf{LockboxDisbursementAmount}$ MUST be calculated as:

$$
\begin{array}
          \mathsf{NumBlocksSinceNU6} &=& \mathsf{ZIP271ActivationHeight} - \mathsf{NU6MainnetActivationHeight}
  \mathsf{LockboxDisbursementAmount} &=& \mathsf{NumBlocksSinceNU6} \times 18750000 \mathsf{zatoshi}
\end{array}
$$

where $\mathsf{NU6MainnetActivationHeight}$ is 2726400 [^zip-0253] and 0.1875 ZEC
(18750000 zatoshi) is the per-block contribution to the deferred pool during the
current dev fund.
...
> Let $\mathsf{ZIP217ActivationHeight}$ and $\mathsf{LockboxDisbursementAmount}$
> be as defined in [ZIP-271].

and

> $\mathsf{totalDeferredInput}(\mathsf{height}) := \begin{cases}
>    \mathsf{LockboxDisbursementAmount},&\!\!\text{if } \mathsf{height} = \mathsf{ZIP217ActivationHeight} \\
>    0,&\!\!\text{otherwise}.
>  \end{cases}$

#### § 7.10 ‘Payment of Funding Streams’ [^protocol-fundingstreams]

Change the section title to ‘Payment of Funding Streams, Deferred Lockbox, and Lockbox
Disbursements’.
...
> Consensus rule: [Canopy onward] In each block with coinbase transaction $\mathsf{cb}$
> at \block height $\mathsf{height}$, $\mathsf{cb}$ MUST contain at least one distinct
...
>   an output that pays $\mathsf{fs.Value}(\mathsf{height})$ zatoshi in the prescribed way
>   to the address represented by that recipient identifier.
> * [NU6.1 onward] if the block height is $\mathsf{ZIP271ActivationHeight}, an output
>   that pays $\mathsf{LockboxDisbursementAmount}$ zatoshi in the prescribed way to the
>   Key-Holder Organization P2SH multisig address specified in [[ZIP-271]](zip-271).
\end{itemize}
...
>   consensus rule, as long as there is at least one distinct output that meets
>   it for each funding item.
...
[^protocol-subsidies]: [Zcash Protocol Specification, Version 2025.6.0 [NU6.1 proposal]. Section 7.8: Calculation of Block Subsidy, Funding Streams, and Founders’ Reward](protocol/protocol.pdf#subsidies)
...
[^zip-0253]: [ZIP 253: Deployment of the NU6 Network Upgrade](zip-0253.md)
