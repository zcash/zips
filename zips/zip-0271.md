...
The additional coinbase output(s) MUST pay a sum of
...
transaction value pool of the coinbase transaction to fund the output(s), and
...
Note: The value $v$ needs to be precalculated so that it is known at
the point when the relevant consensus check is done in node implementations.

$v$ MUST be 78,750 TAZ on Testnet.

If this ZIP activates on Mainnet after the end of current dev fund as of
this writing and as specified in [^zip-1015-funding-streams], $v$ MUST be:
78,750 ZEC on Mainnet.

If this ZIP activates on Mainnet before the end of the current dev fund,
$v$ MUST be calculated as:

```
NumBlocksSinceNU6 = ZIP271ActivationHeight - NU6MainnetActivationHeight
LockboxDisbursementAmount = NumBlocksSinceNU6 * 0.1875 ZEC
```
...
is extended by 1,260,000 blocks up to a maximum height of 4406400  on both Mainnet and Testnet.
If this ZIP activates before or on the first block after the end of current dev fund as of 
this writing and specified in [^zip-1015-funding-streams], this would extend ``FS_DEFERRED`` on
