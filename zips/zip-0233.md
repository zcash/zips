...
This ZIP proposes to register bundle type 6 variant 0 ("ZIP 233 NSM field") in
the V7 transaction bundle type registry defined in ZIP 248 [^zip-0248].
...
with `bundleType = 6` and `assetClass = 0` (ZEC).

## NSM Amount

When the `mValuePoolDeltas` map contains an entry with `bundleType = 6`, the
...
Since the NSM bundle (bundle type 6) has no effecting data and no authorizing
