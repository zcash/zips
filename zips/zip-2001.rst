...
"Deferred Development Fund Chain Value Pool Balance" and the following contents:
    ...
    :math:`\sum_{\mathsf{h} = 0}^{\mathsf{height}} \mathsf{totalDeferredOutput}(\mathsf{height})`.

    Note: :math:`\mathsf{totalDeferredOutput}(\mathsf{height})` is necessarily
    zero for heights prior to NU6 activation.
...
In section **7.1.2 Transaction Consensus Rules** [#protocol-txnconsensus]_, instead of:
    ...
    minus :math:`\mathsf{v^{balanceSapling}}`, minus :math:`\mathsf{v^{balanceOrchard}}`, MUST NOT
    be greater than the value in zatoshi of the block subsidy plus the transaction
    fees paid by transactions in this block.

it will be modified to read:

    For the block at height :math:`\mathsf{height}`, the total value in zatoshi of
    transparent outputs from its coinbase transaction, minus :math:`\mathsf{v^{balanceSapling}}`,
    minus :math:`\mathsf{v^{balanceOrchard}}`, plus :math:`\mathsf{totalDeferredOutput}(\mathsf{height})`,
    MUST NOT be greater than the value in zatoshi of block subsidy plus the transaction
    fees paid by transactions in this block.
...
is that it will read as follows:

    [Pre-NU6] For the block at height :math:`\mathsf{height}`, the total value
    in zatoshi of transparent outputs from its coinbase transaction, minus
    :math:`\mathsf{v^{balanceSapling}}`, minus :math:`\mathsf{v^{balanceOrchard}}`, plus
    :math:`\mathsf{totalDeferredOutput}(\mathsf{height})`, MUST NOT be greater
    than the value in zatoshi of block subsidy plus the transaction fees paid
    by transactions in this block.

    [NU6 onward] For the block at height :math:`\mathsf{height}`, the total value
    in zatoshi of transparent outputs from its coinbase transaction, minus
    :math:`\mathsf{v^{balanceSapling}}`, minus :math:`\mathsf{v^{balanceOrchard}}`, plus
    :math:`\mathsf{totalDeferredOutput}(\mathsf{height})`, MUST be equal to the
    value in zatoshi of block subsidy plus the transaction fees paid by transactions
    in this block.

where :math:`\mathsf{totalDeferredOutput}(\mathsf{height})` is defined in
section **7.8 Calculation of Block Subsidy, Funding Streams, and Founders’ Reward**
[#protocol-subsidies]_ consistently with ZIP 207.

In section 7.10 Payment of Funding Streams, ... (TODO finish)

(TODO do we need this?) In section **3.4 Transactions and Treestates** [#protocol-transactions]_,
add a definition of "total issued supply", such that the total issued supply as of a given
