...
A TLA+ formal specification of the connection lifecycle described in this
ZIP is available at [#formal-model]_. The model covers the handshake,
keepalive, block synchronization, and disconnection phases, verifying
liveness (all peers eventually reach the same block height) and safety
(message size bounds hold across all reachable states).
