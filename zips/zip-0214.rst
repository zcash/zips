...
As of `Revision 3`_, the $\mathsf{AddressIndex}$ and $\mathsf{Address}$
calculations for ``FS_FPF_ZCG_H3`` SHALL use the NU7
$\mathsf{AddressPeriod}$ definition in Revision 2 of ZIP 207
[#zip-0207]_. This triples the remaining block count if NU7 activates
partway through an address period, and each subsequent period lasts
105000 blocks. At the 25-second target spacing, each period retains
approximately its previous wall-clock duration and funding amount,
subject to per-block rounding.
