...
The Sender also determines a "sending protocol" —one of
...
sending protocol, of the full viewing key for that Account
(i.e. at the ZIP 32 Account level).

If the sending Account is undetermined, then the Sender SHOULD
choose one of the addresses from which funds are being sent
(for example, the first one specified), and then use the
external or internal :math:`\mathsf{ovk}` (according to the
type of transfer), as specified by the sending protocol, of
the full viewing key for that address.
