...
* Revision 1: This version adds support for `MUST-understand Typecodes`_
  and `Address Expiration Metadata`_
...
* “``ua``”, if this is a UA/UVK from `Revision 1`_ onward.
...
the range :math:`mathtt{0xE0}` to :math:`mathtt{0xEF}` inclusive are
...
recognize the Human-Readable Parts beginning with “``ua``” that mark a
