...
As of `Revision 1`_, typecodes :math:`\mathtt{0xE0}` and
:math:`\mathtt{0xE1}` are reserved for address expiry metadata.
...
Address Expiry Time is specified, then the Sender MUST choose a value for
...
attempt to obtain a currently-valid address for the intended recipient.

When deriving from a UFVK or UIVK containing typecodes :math:`\mathtt{0xE0}`
...
set nExpiryHeight field, this would leak the expiry information of the
destination address, which may then be identifiable.

When honoring an Address Expiry Time, the reason that a sender must choose a
``nExpiryHeight`` that is expected to occur within 24 hours of the time of
transaction construction is to ensure that the expiry time is respected to
within a day; since block times are not exact and computer clocks often
disagree, an Address Expiry Time is advisory and not a hard bound. Setting an
expiry height as specified ensures that the expiry time will be at least
loosely respected, and it is of course both permissible and perhaps advisable
to set this bound more tightly. A common expiry delta used by many wallets
is 40 blocks from the current chain tip.
