...
- An extension MAY request that arbitrary public information about the
  ...
  the integration point. Implmentations may restrict the information provided to
  that which may be provided in an efficient manner.
...
Introduction of TZEs produces a new transparent unspent outpoint set, such that indexes
into this set of outpoints may be referred to by TZE inpoints in spending transactions.

Encoding in transactions
------------------------

We define a new transaction format that is non-malleable and contains two new fields:
...
====================== ==================================== =====================
``tze_id``             extension ``type``                   ``uint64``    
``tze_mode``           extension ``mode``                   ``uint64``
...
- All ``amount`` field values of ``tze_output`` records MUST be nonnegative and less than
  MAX_MONEY.
