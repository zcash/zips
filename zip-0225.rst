...
* The fields ``flagsOrchard``, ``valueBalanceOrchard``, ``anchorOrchard``,
  ...
  only if ``nActionsOrchard > 0``. If ``valueBalanceOrchard`` is not present, then
...
in the hash::
   ...
   T.4c.i  : nullifier            (field encoding bytes)
   T.4c.ii : cmx                  (field encoding bytes)
   T.4c.iii: ephemeralKey         (field encoding bytes)
   T.4c.iv : encCiphertext[..52]  (First 52 bytes of field encoding)
...
in the hash::
   ...
   T.4d.i: encCiphertext[52..564] (contents of the encrypted memo field)
...
the following elements are included in the hash::
   ...
   T.4e.i  : cv                    (field encoding bytes)
   T.4e.ii : rk                    (field encoding bytes)
   T.4e.iii: encCiphertext[564..]  (post-memo suffix of field encoding)
   T.4e.iv : outCiphertext         (field encoding bytes)
...
``spendAuthSigsOrchard``, and ``bindingSigOrchard`` fields of the transaction::
   ...
   A.3a: zkproofsOrchard          (field encoding bytes)
   A.3b: spendAuthSigsOrchard     (field encoding bytes)
