...
Community Advisory Panel Membership Governance
----------------------------------------------

The ZCAP must have a mechanism to defend itself against members acting in bad faith
towards the Zcash project and its community.

If a ZCAP member is suspended from the `Zcash Community Forum`_ for a violation of
the Zcash Community Code of Conduct (CoC), that member shall be immediately
expelled from the ZCAP.

If a ZCAP member is found to be hostile or act in unconstructive behavior toward the Zcash project,
this 3 steps procedure must be followed :

1. Community Petition
  - Create a post in the `Zcash Community Forum`_ to recommend a ZCAP member
    be expelled from ZCAP for hostile or unconstructive behavior.
  - Provide support (e.g. tweets, forum posts, etc.) that demonstrates the member has persistently
    acted in a hostile or unconstructive manner.
  - Encourage feedback from community members

2. Review Process
  - The petition will be reviewed by the ZF Executive Director (ED) or their delegate to determine
    whether to reject or accept the petition based on the allegations made, support provided, and community input.
    Note: The ED or their delegate are not deciding whether the member should or should not be expelled,
    but whether or not the allegations and support warrant the ZCAP being polled.
  - Reject – The ED or their delegate will briefly explain their reasoning for rejecting the petition
    in the relevant ZCF thread.
